...
(the predecessor tools helm descends from) and applies idempotently — externals
...
from . import freetext, pk, registry

# THE SEED IS OWNER DATA, SO IT LIVES IN THE OWNER'S HOME — not in the repo.
#
# lineage_seed.json carries the owner-CONFIRMED founding ancestry of their whole
# portfolio: which private projects descend from which, what composes what, and
# the advisor decisions behind each edge. Shipped in the tree it was a map of the
# owner's private portfolio — which projects exist, how they descend from and
# compose one another — sitting in a repo whose whole 0.2 posture is
# "one flip from public". Found in the 0.2 council and verified: still
# tracked on main, 25 lines, named by an earlier as-public adjudication as the
# single most damaging file, and unread by three certification passes.
#
# It also contradicted helm's own first principle, which the README states as
# "helm references authoritative homes — it never copies them". Every other piece
# of owner truth already lives under ~/.helm. This one did not.
#
...
# Rels rendered as components under the SOURCE node (a composes b -> b
# indents under a).
...
def cmd_lineage(args):
    ...
    if verb == "add":
        if len(rest) < 3:
            ...
            return 2
        note, rc = freetext.tail("helm lineage", "add", rest[3:], "a note")
        if rc is not None:
            return rc
        _, err = registry.add_edge(rest[0], rest[1], rest[2], note or "")
        if err:
    ...
    if verb == "external":
        ...
        note, rc = freetext.tail("helm lineage", "external", rest[2:], "a note")
        if rc is not None:
            return rc
        registry.add_external(rest[0], path, note or "")
