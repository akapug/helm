...
fixed EIGHT DAYS earlier (react-digest: REACT_TAG landed 2026-07-21 as
18374bd). Both arms correctly refuted it — which means the run measured
...
def arm_config(seat_claude, rdir):
    ...
    for name in sorted(os.listdir(seat_claude)):
        ...
        if name in SETTINGS_FILES:
            try:
                with pk.open_regular(src, encoding="utf-8") as f:
                    body = json.load(f)
