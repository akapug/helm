...
actual goal of the whisper system since [its start]. there is tons of planning for
...
RULES = (
    {
        ...
        # THE PAYLOAD IS THE ROUTE, not the argument for it: a seat mid-write
        # needs the verb, and why a private memory file is the wrong home is
        # the `learn` skill's job and this module's docstring's. What stays is
        # the one clause that stops a false positive — a genuinely private
        # note is allowed to be a private note.
        "say": ("that path is a private memory file: no other seat can read "
                "it. A durable lesson belongs in helm's store, which injects "
                "fleet-wide — load the `learn` skill, then `helm store add "
                "premise '<id> | <statement> | <keywords>'` and prove it "
                "fires with `helm store resolve \"<a sentence someone would "
                "type>\"`. Keep the file only if the note is genuinely "
                "private to this session."),
    },
    {
        "id": "helmese-register-edit",
        "match": lambda p: bool(_REGISTER_RE.search(p)),
        # The decoder-refusal explanation — why an unknown version refuses
        # rather than guessing with a newer table — is draft-2 amendment 3 and
        # is in this module's LAWS above. The instruction and its check are
        # what a seat mid-edit can act on.
        "say": ("that is the helmese seed register. If you changed what a "
                "seeded entry means, added one, or retired one, bump "
                "`helmese.VERSION` in the same commit — a changed entry under "
                "a stale version ships a register that reads as the old one "
                "and is not. Check with `python3 -c \"from helm import "
                "helmese; print(helmese.stamp())\"`."),
    },
)
...
def prepare_for_pair(session):
    """Compute a candidate WITHOUT latching. Installed caller owns fail-open.

    Unlike the standalone entry, failed reads must reach its timed preparatory
    stage rather than masquerading as no matching rule. Missing state is quiet.
    """
    if not session:
        return None
    path = os.path.join(record.session_dir(session), "edit-paths.log")
    try:
        with open(path, encoding="utf-8", errors="replace") as f:
            lines = [line.strip() for line in f.read().splitlines() if line.strip()]
    except FileNotFoundError:
        return None
    rule = classify(lines[-1]) if lines else None
    if not rule:
        return None
    latch = pk.read_json(_latch_path(session), {}, strict=True)
    if not isinstance(latch, dict):
        raise ValueError("toolwhisper latch is not an object")
    if latch.get(rule["id"]):
        return None
    return {"session": session, "rule": rule["id"],
            "text": "[helm toolwhisper] %s" % rule["say"]}


def commit_for_pair(candidate):
    """Latch only a completely published candidate; merge CURRENT rule state.

    Pair writers serialize this read/merge/write, not the preparation or output.
    Standalone for_hook keeps its existing behavior (and does not take this
    pair-only lock). The caller bounds lock and file I/O with its delivery timer.
    """
    import fcntl
    lp = _latch_path(candidate["session"])
    os.makedirs(os.path.dirname(lp), exist_ok=True)
    with open(lp + ".pair-lock", "a", encoding="utf-8") as lock:
        from . import hooklatency
        hooklatency.flock(lock, fcntl.LOCK_EX, "lock-whisper")
        try:
