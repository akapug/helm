"""Route one whole-suite gate to a consented box from the inventory (#225).
...
def _job_consent_err(name, row):
    ...
    never a display/lookup alias: authorising host=box-a cannot ship code to
...
def _remote_script(head, tree, label, timeout, challenge,
...
if ! stage "$cmd_git" -C "$dir/tree" fetch -q "$dir/ship.bundle" \
    'refs/helm-job/*:refs/helm-job/*' 2>"$dir/git.err"; then
  gitdiag; err "clone-failed:${git_diag:-no git diagnostic}"
fi
...
cd "$dir/tree" || err cd-failed
# The gate's cgroup guard needs a USER-DELEGATED cgroup: a bare ssh session
# scope is root-owned (measured on a build host 2026-08-05: mkdir denied), so the
...
def _bundle(repo, head, challenge):
    """-> (path, err): the CAPTURED sha, packed for transit via the vcs seam.
    ...
    rc, _out, err = git.text(repo, "update-ref", ref, head)
    if rc != 0:
        _bundle_cleanup(path)
        return None, "cannot pin the job ref: %s" % (
            (err or "").strip()[:240] or "exit %s" % rc)
    try:
        rc, _out, err = git.text(repo, "bundle", "create", path, ref,
        ...
        git.text(repo, "update-ref", "-d", ref)
    return path, None
...
def route(repo=None, box=None, label=None, timeout=None, transport=None,
          nodes=None):
    """Run this repo's whole-suite gate ON `box` and import the receipt.
    ...
    row, err = pick_box(box, nodes=nodes)
    if err:
        return None, err
    ...
    bundle_path, err = _bundle(repo, head, challenge)
    if err:
        return None, err
    ...
    try:
        rc, out, errout, err = transport.run(
            _remote_script(head, tree, label, timeout, challenge,
    ...
    if not verdict_json.get("minted"):
        return None, "the remote gate on box %r refused: %s" % (
...
def cmd_route(box, repo=None, label=None, timeout=None, as_json=False):
    """The `helm gate run --box` arm: route, then report exactly the way the
    local arm does — evidence line, bind verdict, bind-derived exit."""
    result, err = route(repo=repo, box=box, label=label, timeout=timeout)
    if err:
        if as_json:
            print(json.dumps({"minted": False, "routed": box, "reason": err},
                             indent=1))
            return 1
    ...
    if as_json:
        ...
        state, _rid, why = gate.bind(evidence, receipt.get("head") or "")
        if state != "VERIFIED":
    ...
    return 0 if gate.bind(evidence, receipt.get("head") or "")[0] \
        == "VERIFIED" else 1
