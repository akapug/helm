...
# Per-process anti-CSRF bearer (MUTATION_TOKEN): a hostile page
...
def code_drift():
    ...
    and there is no way to tell from the page. Measured live: the
...
def default_room():
    ...
    the one room the fleet does not talk in — the owner had to be TOLD where a
    ...
    (seats.derive_home_room), asked TWICE:
      ...
      2. THIS FILE'S OWN DIRECTORY — because a systemd unit with no
         ...
         timer units — a fix that depends on a hand-installed unit property
         WILL be found missing on the next surface out.
    ...
    if not _DEFAULT_ROOM:
        ...
        try:
            from . import seats
            room = (seats.derive_home_room(seats.safe_cwd())
                    or seats.derive_home_room(PACKAGE_DIR))
        except Exception:
...
# ABSORBED contracts from the predecessor cockpit's route handlers: same query
...
#
# NO TAB READS THESE ANY MORE. The cave tab that did was retired:
# its board carried the fleet's notes to the owner (they live in
