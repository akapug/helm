...
agent harness, or a future remote bridge can share one adapter contract
...
from . import chat, freetext, home, pk
...
def multiplayer_dir():
    # Same law as chat_dir (#117): explicit env wins, a redirected HELM_HOME
...
def adapters(name=None):
    ...
    and future remote bridges register another factory without changing
    the consumer."""
...
def cmd_multiplayer(args, adapter_factory=adapters):
    ...
    if not args:
        print("usage: helm multiplayer set <doc> <key> <value> [--cave C] [--actor A]\n"
              ...
              "       helm multiplayer presence [--cave C] [--actor A] [--connection ID] [--state S] [--ttl N[s|m|h|d]]\n"
    ...
    try:
        ...
        if verb == "set" and len(rest) >= 3:
            ...
            value, rc = freetext.tail("helm multiplayer", "set", rest[2:],
                                      "a cell value")
            if rc is not None:
                return rc
            update = multiplayer_demo.encode(rest[1], value or "", actor)
            ...
            # THE ONE --ttl READER, shared with the lease verbs; the
            # heartbeat still holds its own 1..MAX_TTL bound after it.
            from .seats_common import ttl_flag
            ttl, bad = ttl_flag(args[1:], DEFAULT_TTL)
            if bad:
