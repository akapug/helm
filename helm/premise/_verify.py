...
def verify_chain(recs=None):
    ...
    `recs` IS THE RECORDS THE CALLER ALREADY READ, and passing them is how a
    caller that ALSO reports a count avoids building one answer from two
    readings of an append-only file. Both live callers do exactly that — read
    the chain for the count and the head, then call this — so a record appended
    between the two calls made a card say N records and "chain verified (N+1
    records)". The default re-reads and is every other caller's behaviour
    unchanged."""
    recs = chain_records() if recs is None else recs
    prev = ""
...
def verify_record(rec_hash, expect=None, recs=None):
    ...
    changed, is NOT this premise's proof. (ok, detail). `recs` is the chain
    the caller already read (verify_chain's own contract): a caller checking
    many records on one pass reads the file once instead of once per record."""
    recs = chain_records() if recs is None else recs
    for i, rec in enumerate(recs):
