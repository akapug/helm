...
def records_from(data):
    """Every native record in ONE READ'S BYTES, in append order.

    THE PARSE, SPLIT OUT FROM THE OPEN. A caller that must be able to say WHICH
    FILE it read — the land projection's read-set, which puts the path, the
    inode and the content identity into a single witness term — cannot call
    `chain_records`, because that resolves the chain path AGAIN and opens
    whatever it names at that instant. It opens once, fstats that handle, reads
    those bytes and parses them here, so the record and the body describe one
    file description by construction.

    A torn/unparseable line (a crash mid-append) is skipped, never a wedge — and
    that now includes a line that is not valid UTF-8, which the line-by-line
    text read raised out of `chain_records` instead of skipping. One malformed
    byte in an append-only file must not take the whole chain dark.
    """
    rows = []
