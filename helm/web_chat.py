...
# 1:1 with the predecessor chat's getRecent(...,50)); deeper history comes from the
...
def _api_chat_older(qs):
    """Older-history page (the analog of an earlier chat client's
    getRecent(before, n)): the immutable body of rows[max(0,before-win):before]
    for a room, plus the absolute `base` of the first row returned and the live
    `total`. BODY ONLY — no transport, no rooms, no roster, no signal, no
    presence. Signed history is immutable, so this slice is cache-safe; the
    signing STATUS is never in this response and is always recomputed live on
    the poll path."""
    from . import chat
...
def _api_chat_ids(qs):
    """Batch id hydration (parity with an earlier chat client's
    handleMessagesByIds, cap 100): the rows whose id is in ?ids=<csv>, BODY
    ONLY (no transport/rooms/roster/signal).
...
def _api_chat(qs):
    ...
    try:
        ...
        out = {"room": room,
               ...
               # THE OWNER'S MINIMAL INCOMPLETE SIGNAL. An unreadable dm/ dir
               # used to render as an owner with no DMs — on his own surface,
               # which is the worst place to fail open. It rides the envelope
               # rather than the room rows because a namespace is not a room.
               "dm_incomplete": _dm_lanes()[1] is not None,
        ...
        if not incremental:
            ...
            out["base"] = start
            # THE NAME AN UNNAMED OWNER POST WILL CARRY, said on the same open.
            # The page ships no person's name: its name box holds only what
            # the owner typed, and a post without one is named by the post
            # handlers with seats.owner_name(). This is that same resolver, so
            # the name the page shows and marks his rows with is the name his
            # posts land under. A resolver that raises leaves the key out, and
            # the page reads "owner" rather than guessing at someone.
            try:
                me = _s.owner_name()
            except Exception:
                me = None
            if me:
                out["owner_name"] = chat._dsan(me)
        out.update(_owner_signal(room, rows))
        # AGES AT RESPONSE TIME, ONTO FRESH ROW DICTS — the same law the
        # recent-lands rows follow in _api_lr, and for the same two reasons:
        # the browser's clock must never be the one comparison that decides
        # whether a reply is fresh, and stamping in place would let two
        # concurrent reads tear each other's numbers. A row whose stamp cannot
        # be parsed carries age_s None, which the card renders as "age
        # unknown" rather than as zero.
        answers = out.get("owner_answers")
        if isinstance(answers, list):
            from . import pk
            now = time.time()
            out["owner_answers"] = [
                dict(a, age_s=(None if _ts is None
                               else max(0, int(now - _ts))))
                for a, _ts in ((a, pk.parse_ts_epoch(str(a.get("ts") or "")))
...
def _api_chat_post(payload):
    ...
    if room.startswith(chat.DM_PREFIX):
        # A post into an open DM channel IS a DM (task/62): route it through
...
def _api_chat_dm(payload):
    ...
    row, err = seats.dm(str(payload.get("to") or ""), text,
                        ...
                        # it here silently flattened the reply (a FIX
                        # verdict on the dm-channels-web review row)
                        reply_to=str(payload.get("reply_to") or "") or None)
    if err:
