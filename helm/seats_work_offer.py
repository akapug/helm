...
def _offer_landing_state(row):
    ...
    if proof == "absent":
        return False, pinned
    return None, pinned
def _finalize_work_offer(session, seat, offer, line):
    ...
    if landed is not False:
        return None
    try:
        ok, _m, _l = claim("dispatch:" + rid, seat, session=session)
    except Exception:
        ok = False
...
def _work_offer_candidate(session, seat, ask, dsp, pending, inbox_blocked):
    ...
    (ds4pro measured at 108 undelivered rows), so the gate was permanently
...
def _session_delegated(session):
    ...
    for name in names:
        ...
        if name.startswith(_ACTIVITY_PREFIX) and name.endswith(suffix):
            live.append(name)
    for name in live:            # bounded by leases held, never by fleet size
        raw = pk.read_json(os.path.join(d, name), {}) or {}
...
def _whisper_candidates(session, seat, pending, inbox_blocked, cwd=None):
    ...
    if dirty >= DIRTY_AT:
        ...
        #
        # I wrote the false version in "whisper: fixed one rung's
...
def _stop_whisper(session, room, seat, pending, inbox_blocked, cwd=None):
    ...
    cands = _whisper_candidates(session, seat, pending, inbox_blocked, cwd)
    if not cands:
        return None
    ...
    if not hit:
        return None
    fp, line = hit[:2]
    if fp.startswith("autoclaim:"):
        # PROVE-AND-MINT-ON-WIN. The offer layer owns its domain policy; this
        # generic selector only hands the winning candidate back for finalizing.
