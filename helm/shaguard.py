...
def findings(text, root=None):
    ...
    for tok in toks:
        ...
        # a real blob sha as missing.
        #
        # AND IT ASKS THROUGH probe_outcome BECAUSE THIS IS THE ACCUSING SITE.
        # `_probe` folds a completed-nonzero, a missing binary and a TIMEOUT
        # into one None, and this branch reads None as "the object is not
        # here" — so a transient git failure mid-sweep publishes a VALID sha
        # as dangling, with the same confidence as a real finding. The repo
        # probe above already refuses to speak when git cannot speak about
        # the directory at all; this is the same rule applied per token,
        # which is where a timeout actually lands.
        ran, rc, _text = be.probe_outcome(root, "cat-file", "-e", tok,
                                          timeout=_TIMEOUT)
        if not ran:
            continue                  # nothing ran: unreadable, never absent
        if rc == 0:
            continue                  # the object is here
