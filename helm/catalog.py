...
def _pk():
    """`pk`, whichever way this module was reached.

    IT IS BOTH A PACKAGE MODULE AND A SCRIPT. The entry point under
    `__main__` below is documented and used, and executing a file by pathname
    leaves it with no package parent, so a module-level `from . import pk`
    raises ImportError before argv is read. Resolving it here, at the reader,
    keeps both ways in: the relative import when there is a package, and the
    bare one when `sys.path[0]` is this file's own directory. pk is
    stdlib-only, so neither spelling pulls anything else in."""
    try:
        from . import pk
...
def _localnames():
    """`localnames`, the same two ways in as `_pk()`."""
    try:
...
CLAUDE_ROOTS = [f"{HOME}/.claude/projects", f"{HOME}/.claude-homes/*/projects"] + \
    [r for r in os.environ.get("HELM_CLAUDE_ROOTS", _localnames().legacy_env(
        "CLAUDE_ROOTS", "")).split(":") if r]
CODEX_ROOTS = [f"{HOME}/.codex/sessions", f"{HOME}/.codex-homes"] + \
    [r for r in os.environ.get("HELM_CODEX_ROOTS", _localnames().legacy_env(
...
def _load_cache():
    try:
        with _pk().open_regular(CACHE) as fh:
            return json.load(fh)
    except Exception:
        return {}
...
def _backfill_syn(rows):
    ...
    try:
        with _pk().open_regular(SYN_CACHE) as fh:
            cache = json.load(fh)
    except Exception:
        cache = {}
    todo, fresh = [], {}
...
def _build_from_cv():
    ...
    if (os.environ.get("HELM_CATALOG")
            or _localnames().legacy_env("CATALOG")) == "scanner":
        return None
    import subprocess
    try:
        # SAME TWO WAYS IN as `_pk()`: this module is also a script, and there
        # the relative import has no parent. It was outside the except list
        # below, so an ImportError escaped and killed the build branch instead
        # of falling back to the scanner.
        try:
            from . import home
        except ImportError:           # run by pathname: no package parent
            import home
    ...
    # scanned by harnesses.pi_observations() for the auto-map. The older form of
    # this comment said "the harnesses helm mints resume commands for", which was
    # false, and it got copied verbatim into three operator-facing surfaces
    # before anyone checked it against pi.py. Keep the reason mechanical.
