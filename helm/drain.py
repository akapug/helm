...
from . import eventledger, home, pk, registry
...
# an owner correction whose directive ("never clear ... mid-task") followed a
# long preamble was stored ending inside "[CORRECTION" — the entry could only
# ever inject his disappointment, never the directive.
...
def _alias_err(msg):
    ...
    # copy-then-delete (finding #2 of a review)
...
def _route_target(base, blob, project_names, aliases):
    ...
    `old-api` whichever side of `old` the operator listed it (review #1).
...
def classify(mem=None):
    ...
    try:
        ...
        reg = pk.read_json(home.registry_path(), {"version": 1, "projects": {}}) or {"version": 1, "projects": {}}
...
# Minted examples: a laughing aside that a job was idle "as always", a report
# that the speaker "never saw" a promised check, and five verbatim
# <task-notification> XML blobs. Three of the 43 were genuine. The three failures, in order of damage:
#
...
#   2. CONTAINS-A-MARKER != IS-A-DIRECTIVE.  "i never saw the confirmation"
#      reports the past; "we should always track the newest tag" states a rule.
...
_PROMOTE_REFUSALS = (
    ...
    # laughter/emoticons mark banter. "finished early as always ... haha"
    ("banter", re.compile(r"\b(?:ha(?:ha)+|hehe+|lol|lmao|rofl)\b|:-?[)D]|!!!", re.I)),
    # fleet seat addresses (w9:p1), handoff tags and commit reports are
...
# comma/dash boundaries: "sure, always the cheaper plan, but each tenant does
# need to see its own invoice total" put a bare "always" and a distant "need
# to" in one sentence, and only clause-level scope keeps them apart.
#
...
# clause ("...updated so the dashboard is always clear") into a bare normative
# copula (the corpus keeps a stand-in with both shapes):
#   - sentence punctuation only counts at a word boundary, or "status.page"
#     splits mid-token;
#   - a newline is WHITESPACE, not a boundary (the text wrapped between "so"
#     and "the"), so the caller collapses runs of whitespace first.
...
_NOT_DIRECTIVE = (
    ...
    # the speaker reporting on HIMSELF ("i never got...", "im just always
    # anxious"). Only copulas and adverbs may sit between the pronoun and the
    ...
    re.compile(r"\b(?:i|im|i'm)\b"
               ...
               r"\s+(?:always|never)\b"),
    # past-tense narration ("that was never migrated", "the ticket was never
...
def _repoint_index_locked(mem, renames):
    """Re-point MEMORY.md while its stable sibling lock is already held."""
...
def _repoint_index(mem, renames):
    ...
    idx = os.path.join(mem, "MEMORY.md")
    with eventledger.locked(idx) as held:
        return _repoint_index_locked(mem, renames) if held else 0
...
def apply(plan, mem=None, sweep_dups=False, limit=None, project=None,
    ...
    with eventledger.locked(idx) as held:
        if not held:
            raise RuntimeError(
                "drain: MEMORY.md projection lock unavailable — ABORTING, "
                "nothing mutated")
        for a in doable:
        ...
        repointed = _repoint_index_locked(mem, renames)
