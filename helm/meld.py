...
Protocol (an earlier prototype's scars kept):
  ...
  * the invite's protocol head (room + join command) comes FIRST so the
    200-byte delivery clip can never eat the join instruction (prior-harness
    incident,
  ...
  * state is keyed room × ACTOR (`<room>.meld.<seat-key>.json`) — helm's two
    ...
    the prototype's live dogfood refuted (test-fixtures-isolated-what-production-shares).
...
v1 is 2-party (prototype precedent); 3+ minds use a plain room + discipline.
...
from . import chat, eventledger, home, pk
...
def _valid_transition(kind, before, after, cause):
    ...
    # transition's identity check cannot see the stream (codex r4: presence
...
def _reduce(events, room):
    ...
    (codex r3: the mixed history was one _repair_from_ram away from gone)."""
...
def _room_projection(unique):
    ...
    the room-level CAUSALITY the per-transition check cannot see (codex r4).
    ...
    Clock laws (codex r4 generated-clock class): an unparseable or FUTURE
...
def _repair_from_ram(room, seat):
    ...
    if unique and not _lock_held(room):
        # codex r4 MIGRATION: a parent-shaped room (r2 actor bits on the
        ...
        # status=UNKNOWN reason=folded (codex r6b exact review).
...
def _parent_actor_bits(room):
    ...
    (restored, reopened, disagreement). codex r4 MIGRATION: the immediate
...
def _transition(room, seat, kind, st, cause=None):
    with chat._room_lock(room):
        ...
        # actor-local _reopened bit died with the snapshot (codex r3 HIGH).
...
def join(room, seat=None, via="meld"):
    ...
    lands silently strands GO forever, prior-harness live-incident), state
...
def _floor_line(marker, frm, accept, st, room, via):
    ...
    # who have DONE (codex-3 re-gate): a member who already YIELDed their view
...
def replay_room(room, apply=True):
    with chat._room_lock(room):
        # codex r4 LOCK: _transition holds this SAME lock; replay_room used
        # to run bare, so a live transition racing a restore could interleave
        ...
        if restored:
            ...
            # with the process/snapshot (codex r3 HIGH). The marker restates
            ...
            if mwhy == "folded":
                ...
                # what the journals actually hold (codex r6). A migration
            ...
            # report replayed (codex r6 second probe: a normal live room
            # read replayed=true).
            for actor in states:
...
def _room_name_epoch(room):
    ...
    any journaled room the event stream is the sole authority (codex r3 MED:
...
def _last_activity_epoch(room):
    ...
    if not unavailable and events:
        ...
        if roomproj["clock_unknown"]:
            ...
            # off a claim. Silence, never a guess (codex r3 MED).
            return None
