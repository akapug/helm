...
from .seats_common import (MAX_BYTES, _BROADCAST, _canonical_recipient, _clip,
                           _scrub, _seat_key, dm_lane, own_name,
                           recipient_matches,
                           roster, roster_path)
...
def identity_disagreement(session=None):
    ...
    WAS opus-integrator — it armed OI's beacon and drained ~60 of OI's DMs.
...
def _assert_own_seat(claimed):
    ...
    if claimed and foreign_seat(claimed):
        return None, ("helm chat: --seat %r cannot receive for another seat: "
                      ...
                      "`helm chat seats`." % (claimed, own_name()))
    return claimed, None
_FOREIGN_WARNED = set()
...
# keyed on (authored path, cwd), and @codex then found FOUR narrowings of that
# key in sequence: git's documented config FILES missed command-scope
...
def _git_owner_handle(cwd=_CWD_UNSET):
    ...
    the second could see a directory the first did not: @codex-3's probe
...
def mentions(text, name):
    ...
    exists to prevent: it matches @ownerson and @owner-extra for the name
    owner, spending attention on rows that address someone else."""
...
def _delivery_guard(seat, session=None):
    """Order a pause decision with the cursor/presence mutation it permits."""
    from . import proxywatch
    with proxywatch.delivery_state_guard():
        yield _delivery_pause(seat, session)
def deliverable(m, seat, room="main", scope=None, ambient=True):
    ...
    owner-posts-not-all), top to bottom:
      ...
      * the seat's HOME room (roster home_room): EVERY remaining row — the
        team channel is full-surface for its own team — but ONLY under
        ambient=True (the boundary/pending default). ambient=False is the
        idle BEACON's default (wait --follow): it drops exactly this tier,
        so a plain home-room row wakes nobody while everything above (and
        the @all rule below, home room included) still does.
    ...
    if m.get("restored"):
        ...
        # from Jul 21 woke opus-integrator a step from acting on a 12-day-old
    ...
    if m.get("react"):
        ...
        return not m.get("un") and not recipient_matches(frm, seat) \
            and recipient_matches(m.get("tfrom"), seat)
    text = m.get("text")
    if not text:
        return False
    ...
    if recipient_matches(frm, seat):
        ...
        # protects (codex + codex-2 xrev of "chat: close the three
        # reply-wake FIX findings", 2026-07-21).
        return False
    if m.get("dm"):
    ...
    if rf and recipient_matches(rf, seat):
    ...
    if room in sc["mute"]:
        return False
    home_r = sc.get("home")
    if ambient and home_r and room == home_r:
        ...
        # below, so a home-room @all broadcast still wakes.
        return True
