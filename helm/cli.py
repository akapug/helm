...
_VERB_HELP = {
    ...
    "dispatch": "dispatch send <recipient> <lane> <message...> --ref TIP [--key K]|add <recipient> <lane> [--ref TIP]|ack <id> <ref>|retarget <id> <old-tip> <new-tip>|verdict <id> <reviewed-tip> <evidence>|list [--open|--overdue|--needs-retry] [--json] — atomic idempotent handoff + durable DISPATCH ledger. POSTING/ACK remain PENDING; only a verdict bound to the exact current tip closes. Retarget is forward-only and preserves original history. Overdue means NEEDS CHECK-IN, never auto-reassign",
