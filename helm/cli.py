...
_VERB_HELP = {
    ...
    "capabilities": "capabilities [--public] [--json] — the wired-substrate SELF-INDEX: what helm gives you (verb -> what -> wired-via -> live/absent), grouped core vs powerpack. The SAME index JIT-surfaces one line at the reasoning moment via inject (a2a/converge -> meld+chat-deliver; 'solved this before' -> recall; deep code analysis -> polyana when wired). --public withholds private-hold powerpacks",
    ...
    "todos": "todos [--all] [--json] — the seat todo mirror: what each agent is working on right now (seat | in-progress | done/total | age). PULL-only surface; the recorder captures TodoWrite/Task* into per-session state and pushes at most one rate-capped room line per meaningful transition (never a mention, never a DM)",
    ...
    "dispatch": "dispatch send <recipient> <lane> <message...> --ref TIP [--key K]|add <recipient> <lane> --ref TIP|verdict <id> <full-reviewed-tip> <evidence>|cancel <id> <reason...>|list [--open|--overdue] [--json] — durable DISPATCH ledger. A dispatch persists before delivery; one operation sends AT MOST ONCE (a retry never re-DMs — confirm at the recipient); ambiguous delivery stays open NEEDS CONFIRMATION; a matching exact-tip verdict closes it, or `cancel` honestly abandons a stranded one with a reason (recipient gone / work moot — never a substitute for a real verdict). Historical ref-less rows read NEEDS REDISPATCH (redispatch with --ref; no bind/retarget/ack verbs exist). Unavailable storage means obligations UNKNOWN",

    "lr": "lr list [--all] [--json]|show <id> [--json]|stalls [--json] — the LAND REQUEST lifecycle as a read-only VIEW over the dispatch ledger + one git observation of trunk (no second ledger, no new event). One dispatch = one land loop binding author + lane/branch + exact review commit (dispatch tip) + reviewer verdict (dispatch verdict) + landed evidence (tip observed on trunk); a CANCELLED dispatch is abandoned, not a land loop (excluded). States OPEN->AWAITING_REVIEW->READY->MERGED_LOCAL->LANDED; each wait is a named state with a DWELL time, so 'stalls' surfaces any loop past its per-stage threshold (the workflow-gap-finder). Landing authority is the existing integrator merge; the LR only OBSERVES it. Unavailable storage means land loops UNKNOWN",
    ...
    "work": "work claim <lane>|release [<lane>] --lease ID [--park]|gc [--apply]|list|install-guard [--apply] — worktree lifecycle on the claims lane: private room <repo>-wt/<lane> per lane (lease = room key, git worktree lock = do-not-disturb), gc rescue-commits dirty lease-less rooms to their branch (never discards), install-guard dry-runs/installs composed deterministic shared-checkout guards",
    ...
    "cred": "cred [list|backup [--all] [--apply]|switch-guard [--install] [--apply]|heal [--apply]] — the SAFE /login: content identity plus transactional 0600 snapshots/restore; every mutation is dry-run without --apply and live-session uncertainty refuses",
