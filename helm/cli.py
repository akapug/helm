...
_VERB_HELP = {
    ...
    "cred": "cred [list|backup [--all]|switch-guard [--install]|heal [--apply]] — the SAFE /login: which account each credhome ACTUALLY holds (read from .claude.json content, never the dir name), 0600 credential snapshots that make a /login reversible, the pre-login guard, and the drift repair (dry-run default; refuses while a live session holds the home)",
