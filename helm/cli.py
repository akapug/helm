...
_VERB_HELP = {
    ...
    "dispatch": "dispatch send <recipient> <lane> <message...> --ref TIP [--key K]|add <recipient> <lane> --ref TIP|ack <id> <ref>|retarget <id> <old-tip> <new-tip>|verdict <id> <reviewed-tip> <evidence>|list [--open|--overdue|--needs-retry] [--json] — atomic idempotent handoff + durable DISPATCH ledger. Every new row is exact-tip-bound and closable; POSTING/ACK remain PENDING; only a matching-tip verdict closes. NEEDS DELIVERY RETRY outranks overdue CHECK-IN; unavailable storage means obligations UNKNOWN, never zero",
