...
_VERB_HELP = {
    ...
    "reviewers": "reviewers <row-id> [--all-seats] [--json] — WHO CAN REVIEW "
                 ...
                 "from a vendor WALL and from a gone pane — and, as that door "
                 "files a row for a seat whose ONLY refusal is DEAF, such a "
                 "seat stays ELIGIBLE with a 'DEAF (nudge pending)' note and "
                 "ranks below a clean one; the pooled family "
    ...
    "train": "train [--repo PATH] [--trunk REF] [--name TRAIN] [--max-behind N] [--apply] — THE LANDING WINDOW COMPOSES ITSELF. A DRY RUN by default: lists this project's approve-ready rows (the land-request projection's own READY word on a LIVE row, never a closed row that still stores it; READY-SELF-REVIEW and READY-CONTESTED are excluded because they are the READY word's own door-caution rungs (landreq.ready_rung: nobody independent looked, or an unanswered FIX stands on this tip); no land verb enforces them, so this verb does; a READY-UNVERIFIED row whose independence is UNKNOWN is excluded too, because this train's gate cannot answer that, and every READY-UNVERIFIED row kept prints the rung's reason), their reviewed tips, how many commits each is behind trunk, and the merge order (oldest approve first), and the rows it excludes with the reason. THE DRIFT CAP: a car whose reviewed tip is more than --max-behind N commits behind the trunk the train stands on (git rev-list --count <tip>..<trunk>; default 200, floor 10, any other value refuses with exit 2) is skipped and named with its count: rebase it or close it; a count git cannot give is UNKNOWN and skipped too. The header prints the declared trunk authority (helm.trunkRef/helm.trunkRemote), observed now, beside the local trunk snapshot the plan stands on; --apply REFUSES when they differ (fetch first) or the authority is UNKNOWN or undeclared, and mints nothing. --apply mints ONE detached room under <repo>-wt/compose/<train>, MERGES each reviewed tip by its exact sha (git -c rerere.enabled=false merge --no-ff, message \"<train>: merge lane <lane>\"), asserting before every merge that git resolves the room to itself, a linked worktree of this project on a detached HEAD, then launches ONE whole-suite gate through `helm gate window launch`, never around it. Merge, not cherry-pick: the reviewed sha stays a parent, so once the train lands it is an ANCESTOR of trunk and foldcheck proves the reviewed content landed by ancestry alone (`helm lr compose` keeps its cherry-pick for its own callers). A conflict is aborted, never resolved: the row and lane are named, the other rows still compose, and nothing else is removed. The train is numbered one past the highest trainN on trunk unless --name names it. Exit 0 every car merged and the door dispatched; the door's own code (2, 3, 4) when the door refuses, unchanged; 1 when a car was refused, the train stopped, the trunk authority refused, or nothing was approve-ready",
