...
_VERB_HELP = {
    ...
    "dispatch": "dispatch add <recipient> <lane> [--ref R] [--deadline SECONDS]|ack <id> <ref>|verdict <id> <ref>|list [--open] [--overdue] [--json] — the durable DISPATCH ledger: every hand-off to another seat is an obligation with a deadline, outliving the session that made it (per-session watchdogs evaporate at compaction). POSTING IS NOT DONE — only a VERDICT closes a row; 'ack' records pickup and keeps it open. Overdue means CHECK IN, never auto-reassign: a long turn looks identical to a dead seat",
