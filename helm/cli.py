...
_VERB_HELP = {
    ...
    "dispatch": "dispatch send <recipient> <lane> <message...> --ref TIP [--key K]|add <recipient> <lane> --ref TIP|ack <id> <ref>|bind <id> <tip>|retarget <id> <old-tip> <new-tip>|verdict <id> <reviewed-tip> <evidence>|list [--open|--overdue|--needs-retry] [--json] — atomic idempotent handoff + durable DISPATCH ledger. New rows are exact-tip-bound; `bind` migrates the short-lived v2 ref-less schema. Retarget replay revalidates repository ancestry. POSTING/ACK remain PENDING; only a matching-tip verdict closes. Retry/binding needs outrank overdue CHECK-IN; unavailable storage means obligations UNKNOWN",
