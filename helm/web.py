...
recognized files only), the chat post (an append to the RAM room — the
owner's side of the groupchat) and the owner's away card (his away flag and
his fleet notice, written through away.declare/lift and ownernotice with an
OwnerDoor). All of it localhost-only, and every mutation
...
API = {
    ...
    "/api/friction": _api_friction,
    "/api/owner/posture": _api_posture,
    ...
    "/api/flags": _api_flags,
    "/api/board": _api_board,
...
QUERY_API = {  # GET endpoints that take query params; fn(qs) -> (obj, status)
    ...
    "/api/project/detail": _api_project_detail,
...
POST_API = {  # fn(payload_dict) -> (obj, status); ALL demand the mutation token
    ...
    "/api/accounts": _api_accounts_post,
    "/api/cwd": _api_cwd_post,
    "/api/prune": _api_prune_post,
    "/api/inject/act": _api_inject_act,
...
def _seed_impl_modules():
    bridge = {"_WEB_FANOUT_NAMES", "_WEB_IMPL_MODULES", "_WebModule",
              "_seed_impl_modules"}
    namespace = {name: value for name, value in globals().items()
                 if not (name.startswith("__") and name.endswith("__"))
                 and name not in bridge}
