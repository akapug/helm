    ...
    if row is not None:
        ...
        try:
            ...
            return 1, {"error": "helm work: cannot refresh peek activity %s — %s"
                                % (path, exc)}
        # A REUSED ROOM IS SOMEBODY ELSE'S ROOM AND THE CALLER MUST BE TOLD.
        # Returning the same {path, sha} for a fresh mint and for a converge
        # makes a GUEST read as an owner: the documented contract is "no
        # lease, no branch, no lock", so a second caller mutates a tree a
        # first caller is measuring and neither learns they shared it.
        # The concrete cost: a cherry-pick in a reused room writes conflict
        # markers into the files of a worktree another caller has live
        # processes in, and neither side is told.
        #
        # THE ASYMMETRY IS WHAT MAKES THIS A DEFECT RATHER THAN A GAP. The
        # DROP path already probes occupancy and refuses — so the machinery
        # exists and is wired to the door where being wrong costs a refused
        # cleanup, and is absent from the door where being wrong costs
        # another seat's measurement.
        ...
        return 0, {"path": path, "sha": sha, "reused": True,
                   ...
                   # as ONE PROCESS with pid 'unknown', and only a prior read
                   # of `occupancy` would tell it otherwise. That is the same
                   # collapse the tri-state exists to prevent, arriving as a
                   # non-empty list meaning NOBODY WAS COUNTED rather than an
                   # empty one meaning nobody is here.
                   "occupants": occ_map[path] if census else None,
                   # TRI-STATE, NOT TWO: measured-and-occupied, measured-and-
                   # empty, and not measured at all. The third is the one a
                   # reader must never receive as either of the first two.
                   "occupancy": "measured" if census else "unmeasured",
                   # `dirty()` reads an UNREADABLE checkout as dirty by
                   # documented contract. That collapse is right for a
                   # disclosure: it errs toward telling the caller to look.
                   "dirty": bool(v.dirty(path))}
