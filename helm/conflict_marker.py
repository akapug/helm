...
# The same sentence about a merge, derived rather than written twice: on a
# merge "already in HEAD" is false for a line that came from the other side.
...
def _sibling(name):
    ...
    dual-mode loader helm/world_prose_guard.py and helm/seatname_guard.py
    already use to reach each other's hardened seams."""
    if __package__:
        try:
...
def _parents(root):
    ...
    ONE implementation of "who are this commit's parents" in the guard family,
    which carries the measurement of what a first-parent base does to a merge. nevertrack.py is snapshotted beside this scanner under every
    guard profile; an absent neighbour is a broken installation and falls back
    to HEAD alone (see the body)."""
    nt = _sibling("nevertrack")
    if nt is None:
        # ONE LAW'S ABSENCE ISOLATES TO ITSELF — the rung law the composed hook
        # is built on, and two arms pin it (a DELETED never-track snapshot
        # warns loudly and the commit still lands). Raising here made this rung
        # refuse every commit in a repo whose never-track snapshot was missing,
        # measured red on tests/test_conflict_marker.py's own wiring arm. HEAD
        # alone is this rung's OLD base: it over-blocks a merge, which is the
        # safe direction, and the hook is already shouting about the snapshot.
        return ["HEAD"]
    # EACH SEAM IN ITS OWN CURRENCY: this scanner's root is literal BYTES (that
    # is its NUL-safety) and nevertrack's takes the decoded name, so the
    # decode happens HERE, at the crossing. MEASURED by a probe before this
    # line existed: the raw bytes root reached os.path.join inside
    # commit_parents and every commit died with a TypeError that a fail-closed
    # rung reports as a refusal — the exact hazard helm/seatname_guard.py
    # documents for this pair.
    return nt.commit_parents(os.fsdecode(root)) or ["HEAD"]
...
# THE ALGORITHM IS PINNED, because an added-line ATTRIBUTION cannot be left to
# a tie-break or to the operator's ~/.gitconfig. Myers minimises the edit
# COUNT, and on a large deletion it buys a shorter script by deleting a few
# more lines here and RE-ADDING the identical text there; the surviving lines
# come back as `+`. Measured on a generated fixture whose new blob is a
# byte-exact SUBSEQUENCE of the old one -- a commit that adds nothing at all --
# myers reported 162 added lines and histogram, patience and minimal reported
# none. Every guard on this seam then judges text the commit did not write, and
# refuses a correct commit, which is the direction that gets a rung switched
# off. Histogram anchors on lines that are unique to one side instead of
# shortening a script, so it does not synthesise that addition, and it is
# linear where `--minimal` is not -- a 60s timeout here RAISES, so the slow
# option's failure mode is worse than the one it cures. Pinning also makes the
# answer independent of ambient `diff.algorithm`, which helm/gitfacts.py
# already documents as hostile state a measurement must neutralise. The two
# sibling readers -- helm/silent_cap.py and helm/vacuous_assertion.py -- carry
# the same pin for the same reason; tests/test_conflict_marker.py asks all
# three at once so a cure here cannot leave a sibling behind.
def _ranges_against(root, old, path, base=()):
    """Inclusive new-side line ranges the index adds to `path` relative to
    `base` (no base = HEAD, git's own default), or None when git calls the
    change binary — no line structure to attribute, so the caller over-blocks
    rather than under-sees. `:(literal)` is load-bearing: the bare path of a
    file NAMED '*.py' glob-expands, and this hunk parser has no per-file
    attribution, so a neighbor's added range would be blamed on this path.
    `--no-textconv` is load-bearing the same way: a textconv filter that
    strips marker lines would erase exactly the added ranges that must refuse.
    A git failure RAISES: an empty answer here would read as "adds nothing"."""
    ...
    rc, out, err = _git(root, "-c", "diff.algorithm=histogram",
                        ...
                        "--no-ext-diff", "--no-textconv", "-M", *base,
                        "--", *spec)
    if rc != 0:
...
def _overlap(a, b):
    """The ranges in both lists — the line set two parents BOTH lack."""
    out = []
    for lo1, hi1 in a:
        for lo2, hi2 in b:
...
def _added_ranges(root, status, old, path, parents=None):
    """Inclusive new-side line ranges this commit ADDS, or None when EVERY
    line is this commit's (a new path, or a binary change). A pure rename
    yields NO ranges — its markers are inherited.

    ADDED MEANS ABSENT FROM EVERY PARENT. A single-parent commit
    is unchanged, to the byte: status 'A' is every line, anything else is
    git's default HEAD base. A MERGE is the case this existed to get wrong —
    HEAD is one side of two or more, so a line the merge took from the other
    side read as this commit's, and `status` itself is first-parent talk ('A'
    for a path the other side has carried for months). So on a merge the
    status is not consulted at all and the ranges are the INTERSECTION of the
    per-parent added ranges: a line every parent lacks is this commit's, a
    line any parent already has is not, and a parent that lacks the path
    contributes the whole file, exactly as status 'A' did. A parent whose diff
    reads binary contributes NO restriction rather than an empty set — an
    unreadable diff must never shrink what is scanned — and when no parent
    yields a readable diff the answer is None, the caller's over-block.

    `parents` is measured by the caller once per scan where there is a scan to
    amortise it over; the CONSUMERS of this seam (helm/seatname_guard.py,
    helm/world_prose_guard.py) pass nothing and it is measured here per path,
    so a consumer cannot be merge-blind by forgetting an argument."""
    parents = _parents(root) if parents is None else parents
    if len(parents) < 2:
        if status == "A":
            return None
        return _ranges_against(root, old, path)
    merged = None
    for ref in parents:
        ranges = _ranges_against(root, old, path, (ref,))
        if ranges is None:
            continue
        merged = ranges if merged is None else _overlap(merged, ranges)
    return merged
...
def scan_staged(root):
    ...
    pre_existing = _PRE_EXISTING if len(parents) < 2 else _PRE_EXISTING_MERGE
    ...
    for status, old, path in entries:
        ...
        if not hits:
            continue
        ranges = _added_ranges(root, status, old, path, parents)
