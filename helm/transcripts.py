...
ABSORBED from the predecessor (its server module), behavior-preserving, per
the dissolve-into-helm law.
...
itself once from a declared predecessor's cache, so cwd-overrides and mints
carry over). Thread-safe: the same single-flight cache the server used, so the CLI
...
def _load_cwd_overrides():
    try:
        with pk.open_regular(OVERRIDES_PATH) as f:
            d = json.load(f)
...
def _grep_sessions(query, rows, cap=60):
    ...
    for a scoped set. The result keeps a proven empty distinct from a subprocess
    failure, and marks a hit whose snippet could not be read as incomplete."""
    import subprocess as _sp, os as _os

    def issue(stage, exc=None, timeout=None, sid=None, rc=None):
        if isinstance(exc, _sp.TimeoutExpired):
            kind = "timeout"
            message = "scoped grep %s timed out after %ss" % (stage.replace("-", " "), timeout)
        elif exc is not None:
            kind = "exec"
            message = "scoped grep %s unavailable (%s)" % (
                stage.replace("-", " "), type(exc).__name__)
        else:
            kind = "exit"
            message = "scoped grep %s failed (exit %s)" % (stage.replace("-", " "), rc)
        out = {"stage": stage, "kind": kind, "message": message}
        if sid:
            out["sid"] = sid
        if rc is not None:
            out["returncode"] = rc
    ...
    by_path = {r["p"]: r for r in rows if r.get("p")}
    for r in rows:
        if not r.get("p") or not _os.path.exists(r["p"]):
            unavailable.append({"stage": "session-list", "kind": "missing",
                                "message": "scoped transcript unavailable",
                                "sid": r["i"]})
    if not paths:
    ...
    try:
        ...
        found = _sp.run(["/usr/bin/grep", "-rilF", "--", query, *paths],
                        capture_output=True, text=True, timeout=60)
    except (_sp.TimeoutExpired, OSError) as e:
        return {"hits": hits,
                "unavailable": unavailable + [issue("session-list", e, 60)]}
    if found.returncode not in (0, 1):
        return {"hits": hits,
                "unavailable": unavailable + [issue("session-list", rc=found.returncode)]}
    for path in found.stdout.splitlines():
        ...
        snip_issue = None
        try:
            match = _sp.run(["/usr/bin/grep", "-iF", "-m1", "--", query, path],
                            capture_output=True, text=True, timeout=10)
            if match.returncode not in (0, 1):
                snip_issue = issue("snippet", sid=r["i"], rc=match.returncode)
            else:
                line = match.stdout.strip()
                i = line.lower().find(query.lower())
                if i >= 0:
                    snip = line[max(0, i - 90): i + 120]
        except (_sp.TimeoutExpired, OSError) as e:
            snip_issue = issue("snippet", e, 10, r["i"])
        hit = {"harness": r["h"], "id8": r["i"][:8], "date": r["u"],
               "title": r["t"][:120],
               "snippet": snip or ("(snippet unavailable)" if snip_issue
                                    else "(match in transcript)"),
               "sid": r["i"], "cvid": r["i"], "syn": r.get("syn", False)}
        if snip_issue:
            hit["snippet_unavailable"] = snip_issue
            unavailable.append(snip_issue)
        hits.append(hit)
        if len(hits) >= cap:
            break
    return {"hits": hits, "unavailable": unavailable}
...
def deep_search(query, limit=40, scope=None, include_synthetic=False):
    ...
    if scope:
        ...
        searched = _grep_sessions(query, scoped, cap=limit)
        out = {"hits": searched["hits"],
               "source": f"scoped-grep ({len(scoped)} sessions in {scope})",
               "scope": scope}
        if searched["unavailable"]:
            out["unavailable"] = searched["unavailable"]
            stages = {u["stage"] for u in searched["unavailable"]}
            out["error"] = ("scoped transcript search unavailable"
                            if "session-list" in stages
                            else "scoped transcript search incomplete: snippet unavailable")
        return out
...
def cmd_search(args):
    ...
    if not args:
        ...
        return 2
    query, _qrc = freetext.tail("helm", "search", args, "search text")
    if _qrc is not None:
        return _qrc
    query = query or ""
