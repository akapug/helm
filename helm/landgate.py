...
whether that is an unnecessary single point of failure and a design review
answered YES for the MECHANISM and NO for the ROLE:
...
(iv) IS THE CLAUSE THE FIRST THREE DRAFTS MISSED. (i)-(iii)
...
def _git(repo, *args):
    ...
    The audit is right: a direct spawn here would be a second
...
def gate_binds_tree(receipt, tree, gates=None, repo=None, tip=None):
    ...
    THE TREE IS DERIVED, NEVER TRUSTED. The first cut compared the
    ...
    if derived is not None and tree:
        ...
        # (a cross-family review fix): compared raw, an abbreviated --tree
...
def disjoint_from_queue(repo, base, tip, queue, landed=None):
    ...
    LANDED IS A LADDER, NEVER ANCESTRY ALONE. `landed`, when given,
    ...
    a new sha), so it blocks its files FOREVER — a recorded trap. A competitor
...
def cmd_landgate(args):
    ...
    WIRED DELIBERATELY, and the stop-guard is why. It refuses a module that
    NOTHING can reach — built, not wired — and it is right: a predicate
    shipped with no caller is not a clean separation.
    ...
    # (a gate fed its own answer is not a gate). Supplying it asks
    ...
    # one — which is the ordinary state of a claude-direct seat.
    ...
    #
    # Measured on a live seat with HELM_CHAT_NAME unset:
    ...
    # explaining a branch that could never run (kimi, gate 79b508b87b1ca6c5).
