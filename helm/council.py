...
WHY EMBARGO (the load-bearing idea, design-locked with a cross-family reviewer):
...
# DECISION vocabulary, not gate vocabulary (a scoping refutation in a
# meld): a council RATIFIES — release go/no-go, adopt-this-design,
...
# reason that decides the fork between a least-surprise ALL and a liveness
# 2-of-3). Majority is a canonical rule rather than a fleet-size guess, and
# it is PRINTED on every surface so it is never silent — the actual concern
# behind least-surprise ALL.
def default_threshold(n):
    return n // 2 + 1


def registry_path(room):
    """ONE append-only EVENT LEDGER per council, on DISK (a re-gate).
...
def _valid_import(ev, room=None):
    ...
    # THE ENVELOPE IS PART OF THE EVENT (r8). A writer that validates
    # BEFORE _emit stamps v/ts/room/id checks something other than
    # what it writes — planted imports with v missing, v=2, or id=OTHER
...
def _apply(state, ev, room=None):
    """Fold ONE event. Strict by construction (a re-gate found probes
    ...
    if t in ("convene", "import"):
        ...
        if state is not None:
            return state
        # THE AUTHORITATIVE FIRST EVENT IS BOUND TOO (a migration attack):
        ...
        fresh = {"v": 1, "room": ev.get("room"), "epoch": ev.get("epoch"),
                 ...
                 # code did the opposite (r6). A planted import
    ...
    # EVERY event must belong to THIS council (a re-gate: a planted row
    ...
    if t == "reveal" and len(state["signals"]) >= state["threshold"]:
        ...
        # ACTOR-GATED IN THE REDUCER TOO (a re-gate: the VERB checked
...
def read(room):
    """(state, unavailable) — the TRI-STATE every verb must carry (a
...
def _normalize_legacy(old, room):
    ...
    THE REDESIGN the fourth pass earned. Three consecutive rounds of
    ...
    if not isinstance(tip, str) or not tip.strip():
        return None, "tip"
    # VALIDATE THE SNAPSHOT'S OWN CLAIMS — never silently rewrite them (r5,
    # the sharpest of the migration findings). An importer that IGNORES
    # `old.room` and takes the council's identity from the path, and maps
    # each signal by its dict KEY while ignoring the embedded `seat` and
    # `tip`, launders both: a body claiming room=OTHER imported as this
    ...
    # REQUIRED MEANS REQUIRED (r6). Writing
    # `if x is not None and x != expected` — validate-IF-PRESENT — and then
    # filling the absence from the path, the dict key, or the top level, is
    # the same laundering one level down: a v1 snapshot ALWAYS wrote a
    # top-level room and a seat+tip on every signal (the 8dab216/ffbf6e7
    # council lineage — "cash the 0.3 deferral" + its xrev fix),
    ...
    for seat, s in raw.items():
        ...
        # PROVENANCE CARRIED, not restamped (r5): the old `ts` is
...
def _import_legacy(room):
    ...
    (a re-gate). Councils that ran before the ledger left snapshots the
    ...
    # EXISTENCE IS NOT VALID STATE — again, one function over (a re-gate).
    ...
    if existing is not None:
        return "present"
    # TRI-STATE ON THE LEGACY FILE TOO (r5 — the FOURTH time this
    ...
    # VALIDATE THE WHOLE SNAPSHOT BEFORE ANY APPEND (a migration attack).
    ...
    if why:
        # TERMINAL-BUT-INVALID IS *FAILED*, NEVER "ABSENT" (r4).
...
def question_subject(question):
    ...
    THE SAME NAMED THING, FIXED BEFORE ANYONE SIGNALS. The tip was made
    required because of one hazard: an optional tip "fell back to first-signal
...
def evidence_digest(tip, verdict, evidence, room="", epoch="", seat=""):
    """The signal's FULL BINDING (the design-lock, now honoured whole):
...
def _valid(reg, room):
    """(reg, err) — STRICT schema on every read (a re-gate: a registry
...
def _held(room):
    """The WHOLE lifecycle serializes on one lock (a re-gate: only
...
def convene(room, members, epoch, threshold=None, convener=None,
    ...
    # A NAMED SUBJECT IS REQUIRED, not an artifact specifically. A re-gate
    # made the TIP required and its reason survives intact: an
    ...
    # EXISTENCE, not parseability (xrev: registry() returns None for a
    ...
    with _held(room) as held:
        ...
        # EXISTENCE IS NOT VALID STATE (a re-gate). Keying the refusal on
        ...
        # ACT ON THE MIGRATION'S RESULT (a re-gate): ignoring it made a
        ...
        # first (a re-gate): members must know what they are judging
...
def signal(room, seat, verdict, tip, evidence):
    ...
    # re-deriving the subject here would reintroduce exactly what the
    ...
    tip = str(tip or "").strip()
    # READ-MODIFY-WRITE UNDER THE LOCK (xrev @ 8dab216: the unlocked
    ...
    with _held(room) as held:
        ...
        # TRI-STATE, never binary (a re-gate): "could not read the
        ...
        if reg.get("status") == "revealed":
            # TERMINAL (xrev): a judgment cast once the others are
        ...
        # ONE COUNCIL, ONE ARTIFACT (xrev: signals binding DIFFERENT
...
def outcome(signals):
    ...
    Quorum is a REVEAL bar, not a decision (a re-gate: YES+NO reached
...
def abort(room, seat, reason):
    ...
    with _held(room) as held:
        ...
        if not reg:
            return None, "no council convened for room %s" % room
        # TERMINAL TRUTH IS WRITE-ONCE (a re-gate: abort-after-reveal and
...
def status_lines(room, via="council"):
    ...
    # ZERO-WHO ON THE PULL SURFACE TOO (a re-gate). Deleting the pushed
