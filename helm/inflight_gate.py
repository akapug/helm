...
one. Three seats hit that inside one hour on 2026-08-03: @kimi committed r2
...
measured by @codex in review: from the snapshot's own directory
...
def inflight(root):
    ...
    for name in names:
        ...
        if isinstance(row, dict) and _owner_live(row):
            live.append(row)
    try:
        with open(os.path.join(gitdir, INFLIGHT), encoding="utf-8") as fh:
            row = json.load(fh)
        if isinstance(row, dict) and _owner_live(row):
            live.append(row)
    except (OSError, ValueError, TypeError):
