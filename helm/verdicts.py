...
FOUR POLARITIES, AND THE FOURTH BY ITSELF AUTHORIZES NOTHING.

The separately opted-in prospective compose-land proof v3 is owned by
compose_contract. It does not widen this vocabulary or any global allowlist;
historical rows and bare CONCUR retain their original meaning.
...
WHY IT IS POWERLESS BY DEFAULT RATHER THAN BY A GUARD. Ordinary close doors
read `_CLOSE_POLARITY[reason]` as a strict allowlist (`polarity not in ...`
refuses). `concur` remains absent from every tuple, and the suite pins that
absence rather than trusting this sentence. The prospective compose exception
requires its own whole proof, not membership in this global authorization set.
"""
...
# is a detectable lie. Applying the marker to a whole ruling cost about
