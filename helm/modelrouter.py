...
NOT THIS VERB? `helm route <kind>` is the other one — it answers WHO should
take a piece of work right now, and shares five of these six subverb names.""" \
    % ROUTER_PORT_DEFAULT
...
def build_table():
    ...
    for fam, info in seat.FAMILIES.items():
        ...
        if not token:
            continue
        # EVERY ALIAS THE FAMILY'S PROXY SERVES, not just its default. A
        # per-model family (openrouter) maps several claude-side aliases onto
        # one proxy, and a table built from `model` alone would route `or-code`
        # nowhere while the proxy sitting on this port answers it.
        models = set(seat.family_route_aliases(info)) \
            | set(info.get("probe_models") or ())
