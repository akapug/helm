...
def _reflex_props(rows, project=None):
    ...
    from . import reflex
    # a nested-spawn reflex is said at SubagentStart and an act reflex by
    # argv-guard at the act, never through the inject ledger, so the ledger
    # cannot call either dead (task/2971, task/2980)
    live = {pk.slug(str(e["id"])): e for e in reflex.load_all(project)
            if e.get("signal") not in (reflex.SPAWN_SIGNAL,
                                       reflex.ACT_SIGNAL)}
