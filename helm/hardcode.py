...
# The round-5 bar: both real warnings tonight (wiring's injected
...
# OVERRIDE must be RELATIONAL, not existential (the round-6 bar): an
...
# A MENTION BAKED INTO A SEND. The seat-literal rung above catches a fleet
# seat name written in quotes, which means it catches the spellings that WORK
# and is blind to the one that does not: a ROLE token like `@integrator` is
# not a seat name, matches no alternative, and sails through — so the broken
# addressee is exactly the one the guard cannot see. That is the wrong way
# round, because a literal that no longer resolves is worse than a literal
# that does: the send door accepts any token and reports success, so the post
# is written, reaches nobody, and is reported as delivered.
#
# This fires on the ACT rather than the name — a mention literal handed to a
# post or a dm, in python or in the shell text of an installed hook. The
# addressee of a send is resolved at the moment of sending or it is a bet on
# a roster nobody re-read. Prose is not scanned (the payload is _code_only),
# so a comment naming a seat is untouched; a SEND is not prose.
_MENTION_SEND = re.compile(
    rb'(?:\.?(?:post|dm)\s*\(\s*|helm chat (?:post|dm)[^"\x27]{0,80})'
    rb'["\x27]@[A-Za-z0-9_-]')
...
def _parse_diff_blocks(diff_bytes):
    ...
    Factoring the parser is the blocker-3 requirement — the control
...
def _code_only(payload):
    ...
    The semantic line the fixtures draw: a hardcode is identity baked
...
def _constructed_home_fires(body):
    ...
    must not silence it (the round-6 bar).
...
def _hits(payload, rel=""):
    ...
    (blocker 2 — prose filtering applied only to constructed-home
    ...
    m = _MENTION_SEND.search(body)
    if m:
        out.append(("baked-addressee", "a mention spelled into a send — the "
                    "door accepts any token and reports success, so a name "
                    "the roster stopped carrying posts to nobody and reads "
                    "as delivered; resolve the addressee at send time",
                    m.group(0)))
    return out
...
def _control_ok(root):
    ...
    parser forever (blocker 3). This control is one the parser can
