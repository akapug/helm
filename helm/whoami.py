...
Store: ~/.helm/_global/know-your-user/
  profile.json   the one operator profile (schema v2, superset of the v1
                 scaffold an external profile manager keeps; merge_scaffold()
                 imports that store in place — content wins over emptiness in
                 both directions).
...
#: Names the external profile manager's home directly, ahead of the adopted
#: homes. A new knob, so no legacy spelling is read.
PROFILE_HOME_ENV = "HELM_PROFILE_HOME"


def scaffold_profile_path():
    """Path to an external user-profile helm merges from, or None when there is
    none. Source order: HELM_PROFILE_HOME (that manager's home), else the first
    adopted home (registry `adopt`) that actually carries a
    user-profile/profile.json. EMPTY in the public tree — no env and no adopted
    profile home means a fresh profile, so the merge is a no-op. No
    site-specific home path lives in this code."""
    raw = os.environ.get(PROFILE_HOME_ENV)
    if raw:
        return os.path.join(os.path.expanduser(raw), "user-profile", "profile.json")
    from . import registry
    for home_dir in registry.adopted_homes().values():
        cand = os.path.join(home_dir, "user-profile", "profile.json")
...
def load_profile():
    ...
    schema carry through so a re-save never drops them (the scaffold's forward-compat law)."""
...
def merge_scaffold(scaffold_path=None):
    """Import the external know-your-user scaffold (content AND bookkeeping)
    into the helm profile. Content wins over emptiness in both directions: a
    scaffold field never clobbers non-empty helm content, an empty scaffold
    field never erases anything, interview_status only ever ranks UP (offered never downgrades
    done). Idempotent — a no-change merge does not rewrite the store."""
    p = load_profile()
    before = _content_key(p)
    ext = pk.read_json(scaffold_path if scaffold_path is not None
                       else scaffold_profile_path())
...
def add_note(text, topic="", supersedes=""):
    ...
    desc = pk.description_line(text.split("\n")[0], 120)
