...
from . import eventledger, fleetnotes, freetext, home, pk

DEFAULT = os.path.expanduser("~/.helm/_global/integration-board.json")


def path():
    """The board this process writes. HELM_BOARD wins; otherwise it is THIS
    PROCESS'S helm home rather than the literal path resolved at import, so
    one env var isolates the board exactly as it isolates every other surface
    under the home. With nothing set the two are the same file."""
    return os.environ.get("HELM_BOARD") or os.path.join(
        home.helm_home(), home.GLOBAL, "integration-board.json")
...
#
# The meld (the owner, the integrator and a builder seat) replaced it
# with writer-per-KEY, and one sharpening from it is the load-bearing part:
...
def add_landed(name, sha, note, board_path=None, seat=None):
    ...
    still held. This is the owner-surface law `headlines-click-to-detail`
    made compositional rather than a second list an agent must remember to
    maintain. A matching projection is skipped, so an ordinary retry cannot
    refresh its timestamp; a missing projection IS repaired.
...
def set_value(key, value, allow_new=False, board_path=None, seat=None):
    ...
    board's own `_contract` names it as another seat's console generator, and
...
def cmd_board(args):
    ...
    if verb == "set":
        ...
        if len(rest) < 2:
            ...
            return 2
        key, value = rest[0], None
        value, rc = freetext.tail("helm board", "set", rest[1:], "a value")
        if rc is not None:
            return rc
    ...
    if verb == "note":
        ...
        if len(rest) < 2:
            print(_USAGE, file=sys.stderr)
            return 2
        key, text = rest[0], None
        text, rc = freetext.tail("helm board", "note", rest[1:], "note text")
        if rc is not None:
            return rc
    ...
    if verb == "landed":
        ...
        if len(rest) < 3:
            print(_USAGE, file=sys.stderr)
            return 2
        name, sha = rest[0], rest[1]
        note, rc = freetext.tail("helm board", "landed", rest[2:], "a note")
        if rc is not None:
            return rc
        ok, err = add_landed(name, sha, note or "", seat=_acting_seat())
        if not ok:
            print("helm board: " + err, file=sys.stderr)
            return 1
        if err:
