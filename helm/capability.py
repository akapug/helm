...
CAPABILITIES = (
    ...
    },
    {
        ...
        "what": "see whether your @mention/DM was SEEN or ACTED on, which "
                "addressees are STRANDED, and which @names this roster could "
                "not resolve — the SENT/SEEN/ACTED consume ladder, so a "
                "silent reply is never mistaken for lost",
...
def _authored_powerpacks():
    ...
    than let it read as 'none configured' (OI cross-family finding 2026-07-30)."""
...
def _effective_mcps():
    ...
    try:
        import json
        with pk.open_regular(os.path.join(os.getcwd(), ".mcp.json"),
                             encoding="utf-8") as f:
            data = json.load(f)
...
def public_set(env=None):
    ...
    powerpack catalog (owner packaging note, 2026-07-23)."""
