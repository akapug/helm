...
WINDOW: FAMILIES[family]["max_context"], which the launch line teaches CC
...
def _transcript_ctx(path):
    ...
    for ln in reversed(_tail_lines(path)):
        ...
        # codex read 0.0% of 360k with a 33-minute-old transcript of 2363 lines
        # and 755 usage-bearing assistant records — thirty-three minutes after
        # it had been rescued from 102%. kimi read 0.0% against 19341 lines.
        # Meanwhile codex-3, whose newest record carried real numbers, read
        # 41.9% correctly.
        #
...
def read(seat_name):
    ...
    row = {"seat": seat_name, "family": family, "ctx_tokens": None,
           ...
           "pane_handle": owned.get("handle"), "age_s": None}
...
def _live_seat_names():
    ...
    try:
        ...
        return None if blind else names
...
def _dead_holding_state(row, thr):
    ...
    codex-2 (93.8%, 6.2d) and codex-3 (103.0%, 23.7h) were reported as hot for
...
def _hot_blocked(row, thr):
    ...
    Live at the moment this landed: codex-2 at 93.8% (session-unbound, 6.1d)
    and codex-3 at 103.0% (stale, 22.7h), neither fired on, neither reported.
...
def _wedge_text(row, thr):
    ...
    return (
        "⚠️ WATCHDOG: seat %s is WEDGED BEHIND AUTOCOMPACT'S OWN SAFETY REFUSAL "
        ...
        "open turn — but a turn that never lands never yields an empty composer, "
        "so this seat can NEVER become eligible on its own and will keep "
        "climbing until it 400s. A busy seat's percentage drops when its turn "
        "lands; this one's has not. Nothing automatic will act on it: esc the "
        "pane by hand and /compact it, or rebind/respawn if the pane is gone. "
        "[watchdogs-correct-composition-holed]"
        % (row["seat"], w.get("n") or 0, w.get("state") or "?",
           "CLIMBING" if last > first else "FLAT", first, last,
           "{:,}".format(row.get("window") or 0),
           _age(max(0.0, time.time() - (w.get("since") or time.time()))), thr))
...
def _refusal_alarm_line(row):
    ...
    if alarm.get("state") == "active":
        ...
        return ("  REFUSAL ALARM [ACTIVE] %s %s against %d%% trigger; "
