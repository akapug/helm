...
Row schema (every snapshot carries the full shape):
  {id, ts, ask, source, hold_kind: owner-input|usage-reset,
   status: open|done|reported, done_ref, report_ref, last_updated}
...
WHO WROTE A DECISION ROW (task/2997) is resolved, never defaulted: the acting
seat through the identity layer, or the owner through an OwnerDoor that only
the web queue's handlers mint (see the WHO WROTE A ROW block below).
...
names the mint passes), which closes the routes a seat takes by
ACCIDENT. A same-uid agent that DELIBERATELY bypasses them, for example with a
script file or a spelling the guard does not read, can still reach the web
door or mint the capability, and since the away card's flag and notice reach
every seat as his word, that forgery is seen fleet-wide. That is outside this
ledger's threat model (accident, not adversary) and belongs to the owner's
own terminal; no further string check here pretends otherwise.
"""
import json
import os
import sys

from . import eventledger, freetext, home, pk

STATUSES = ("open", "done", "reported")
ASK_HOLD_KINDS = ("owner-input", "usage-reset")
...
def add(ask, source=None, needs=None, hold_kind="owner-input"):
    ...
    rather than defending it. (a review bound FIX on exactly this:
    ...
    THE GUARD SITS ON THE WRITE, NOT ON ONE DOOR (task/373, a forward risk
    raised on task/353). This ledger has add|done|report|list and NO edit
    ...
    if hold_kind not in ASK_HOLD_KINDS:
        return None, "hold kind must be one of: %s" % ", ".join(ASK_HOLD_KINDS)
    ...
    row = {"id": rid, "ts": ts, "ask": ask,
           ...
           # The ask ledger is the typed human-input primitive. Prose in
           # ask/needs is never a classifier.
           "hold_kind": hold_kind,
...
# ---------------------------------------------------------------------------
# WHO WROTE A ROW (task/2997). This ledger exists to carry the OWNER'S voice,
# so its `by` fields are testimony. The failure it prevents: every writer here
# defaulted a missing author to "owner", so a seat running `helm decide
# comment` was recorded as the owner, and the relay DM'd the asker "owner
# comment on ..." from his handle. Nobody reading the card could tell the
# seat's words from his.
#
# THE THREAT MODEL IS THE SAME UNIX USER. Every agent runs as the owner's own
# uid, so nothing on this box can PROVE a process is the owner. What this
# ledger prevents is ACCIDENTAL owner attribution, and it makes a deliberate
# one an explicit act. Two rules follow:
#
#   * THE OWNER IS NEVER INFERRED. No absence of evidence (no seat name, no
#     harness stamp) makes a process the owner: a seat's child with those
#     variables unset looks exactly like that. The CLI is never his door;
#     the owner does not type helm verbs. A missing author resolves to the
#     acting seat through the identity layer (`actors.resolve_actor`, the
#     door `helm chat dm` acts through), and an actor that does not resolve
#     is UNKNOWN and writes nothing.
#   * THE OWNER IS A CAPABILITY, NOT A STRING. `OwnerDoor` is minted by
...
OWNER_DOORS = ("the owner's doors are the web queue (helm web, the work tab) "
               "and his phone")


def is_owner(name):
    """Is `name` one of the owner's own spellings: the OWNER token, his derived
    handle, or a name the owner rails post under. Empty is nobody. It
    READS a recorded author and refuses his name as a return address; it
    ...
    if s == OWNER:
        return True
    from . import seats
    return s in {str(n).casefold()
                 for n in set(seats.owner_names()) | {seats.owner_name()}}
...
class OwnerDoor:
    """The owner's authority on this ledger: which of HIS doors a write came
    through. Only `owner_door()` can build one, so a stray string can never
    become one by accident."""
    __slots__ = ("door",)

    def __init__(self, door, mint=None):
        if mint is not _DOOR_MINT:
            raise TypeError("an OwnerDoor is minted by ownerasks.owner_door() "
                            "inside the owner's own web handlers; it is never "
                            "constructed")
        object.__setattr__(self, "door", str(door))

    def __setattr__(self, *_a):
        raise AttributeError("an OwnerDoor is immutable")

    def __repr__(self):
        return "<OwnerDoor %s>" % self.door


def owner_door(door):
    """Mint the owner's capability for `door` (e.g. "web"). Call it ONLY where
    the owner himself acts; the census arm names the one module allowed."""
    return OwnerDoor(door, mint=_DOOR_MINT)


def acting_author(act):
    """(by, who, err) — the SEAT this process writes a decision row as.

      by   the admitted seat's canonical name
      who  what a relay DM is sent as: the admitted actor itself (the
           capability, never a bare name)
      err  set when no seat resolves: the author is UNKNOWN and nothing may
           be written. Never promoted to the owner."""
    from . import actors
    actor, err, _reason = actors.resolve_actor_reason(home.session_id(),
                                                      act=act)
    if actor is None:
        return None, None, ("author UNKNOWN — %s. Nothing was written"
                            % str(err).rstrip(". "))
    if is_owner(actor.canonical_name):
        return None, None, (
            "refusing to %s: this agent session is rostered under the owner's "
            "own name %r, and an agent never writes in his voice. Run it from "
            "a seat with its own HELM_CHAT_NAME" % (act, actor.canonical_name))
    return actor.canonical_name, actor, None


def _author(by, act):
    """(by, who, door, err) for a write whose caller passed `by`.

    An OwnerDoor is the owner, through the door it names. None is THIS
    process, resolved to a seat or refused UNKNOWN. Anything else, the string
    "owner" included, is a caller-stated name and is refused."""
    if isinstance(by, OwnerDoor):
        from . import seats
        return OWNER, seats.owner_name(), by.door, None
    if by is None:
        name, who, err = acting_author(act)
        return name, who, None, err
    return None, None, None, (
        "refusing to %s as %r: an author is never a caller-stated string. "
        "Omit it and the acting seat is recorded" % (act, by))


VERDICT_DOOR = ("A verdict on an owner decision card is the OWNER's ruling, "
                "and a seat never casts it; %s. A seat adds to a card with "
                "`helm decide comment <id> <text>`." % OWNER_DOORS)
...
def file_decision(title, context, options, asker, refs=None, source=None):
    ...
    # delivered-looking card (found in review). Exact canonical seat
    # token or no card.
    from . import seats
    ...
    if aerr:
        return None, ("asker %r is not a deliverable seat address — %s "
                      ...
                      % (asker, aerr))
    # THE OWNER IS NEVER THE ASKER (task/2997). The room line announcing a
    # card is posted AS the asker, and the verdict DMs him from the owner:
    # under one of the owner's names the card speaks in his voice to the room
    # and addresses his own ruling to himself, which never delivers.
    if is_owner(asker):
        return None, ("asker %r is one of the owner's own names — the asker "
                      "is the SEAT the verdict returns to; filed under his "
                      "name, the card would announce itself in his voice and "
                      "DM his ruling back to himself" % asker)
    ts = pk.now_ts()
    rid = os.urandom(4).hex()
...
def _push_body(row, stale=(), total=None):
    ...
    if stale:
        ...
        # (found in review). The caller now slices ONCE and stamps only what
...
def _owner_reach(row):
    ...
    THE TAIL HAS A REAL EDGE, and stating the bound was NOT enough (a second
    pass said so, correctly): the carry rides the NEXT card's push, so the LAST
    ...
    # in no message at all. `named` is now the single source for both:
    ...
    # If the channel is gone now, we cannot PROVE the headline
...
def flush_unreached():
    ...
    WHY THIS EXISTS AND WHY STATING THE BOUND WAS NOT ENOUGH (a
    review pushed back, rightly): the carry rides the NEXT card's push, so if
...
def decide(rid, choice, comment="", by=None):
    ...
    ONLY THE OWNER RULES, AND ONLY THROUGH HIS DOOR (task/2997). `by` must
    be an OwnerDoor; the verdict records "owner" and the door it came
    through. Omitted, it resolves this process, which is a seat (refused) or
    UNKNOWN (refused); a string, "owner" included, is refused. Before this,
    `helm decide verdict` from any seat recorded the owner's ruling."""
    author, _who, door, err = _author(by, "cast a verdict")
    if err:
        return None, "%s. %s" % (err.rstrip(". "), VERDICT_DOOR)
    if door is None:
        return None, ("refusing to cast a verdict as %r. %s"
                      % (author, VERDICT_DOOR))
    path = decisions_path()
    with eventledger.locked(path) as held:
        if not held:
...
def _verdict_author(row):
    """The recorded verdict author, or None when it is the owner. `decide`
    has always written `by` and its old default was the owner, so a row
    without one reads as his."""
    by = (row.get("verdict") or {}).get("by")
    return None if not by or is_owner(by) else str(by)
...
def deliver_verdict(rid):
    ...
    read 'delivered' while the ruling was unreachable after a reboot
    (found in review). So `delivered` binds to DURABLE state — the
    ...
    with eventledger.locked(path) as held:
        ...
        try:
            from . import seats
            # THE RELAY SPEAKS AS THE RECORDED AUTHOR, never a hard-coded
            # owner (task/2997): the row is the testimony, the DM repeats it.
            dmrow, err = seats.dm(r["asker"], _verdict_text(r),
                                  who=_verdict_author(r) or seats.owner_name())
...
def comment_decision(rid, text, by=None):
    """A NON-CLOSING note -> (row, problem). Appends to the card's comments
    and best-effort DMs the asker (row+problem = recorded, DM failed —
    advisory). The card's status never moves: comment is the owner's 'not
    yet / tell me more' lane and any seat's way to add to a card; verdict
    remains the closer.

    THE ROW AND THE RELAY NAME THE AUTHOR (task/2997). `by` omitted is THIS
    process: the acting seat is recorded, and the DM reads "<seat> comment
    on", sent as that seat; an actor that does not resolve is UNKNOWN and
    writes nothing. "owner" is written only through an OwnerDoor, with the
    door beside it; a string, "owner" included, is refused. The DM is sent
    as the recorded author, never as a caller-supplied name. A seat
    commenting on its own card is not DM'd to itself: it wrote the words."""
    text = str(text or "").strip()
    if not text:
        return None, "an empty comment says nothing"
    by, who, door, err = _author(by, "comment on a decision card")
    if err:
        return None, "%s; %s." % (err.rstrip(". "), OWNER_DOORS)
    from . import seats
    path = decisions_path()
    with eventledger.locked(path) as held:
        if not held:
    ...
    if door is None and seats.recipient_matches(by, row["asker"]):
        return row, None
    try:
        _dmrow, err = seats.dm(row["asker"],
                               "[decision %s] %s comment on %r: %s"
                               % (rid, by, row.get("title"), text),
                               who=who)
        if err:
...
def sync_board():
    ...
    the derived list over the key, and a review proved that as live
...
USAGE = ("usage: helm asks add <text> --needs \"<what only the owner can "
         "supply>\" [--kind owner-input|usage-reset] [--source S] | "
         "done <id> <evidence> | "
...
def cmd_asks(args):
    ...
    if verb == "add":
        ...
        if "--kind" in rest:
            i = rest.index("--kind")
            hold_kind = rest[i + 1] if len(rest) > i + 1 else None
            del rest[i:i + (2 if hold_kind else 1)]
        text, rc = freetext.tail("helm asks", "add", rest, "the ask text")
        if rc is not None:
            return rc
    ...
    if verb in ("done", "report"):
        if len(rest) < 2:
            ...
            return 2
        rid = rest[0]
        ref, rc = freetext.tail("helm asks", verb, rest[1:], "the evidence")
        if rc is not None:
            return rc
        row, why = (mark_done if verb == "done" else mark_report)(
            rid, (ref or "").strip())
        if not row:
...
DECIDE_USAGE = """usage: helm decide file <title...> [--asker SEAT] [--ref R]... [--source S]
  ...
  the CLI is never the owner's door, so `verdict` here always refuses and
  names his doors (the web queue and his phone). `comment` records the
  acting seat, the DM to the asker names it, and an actor that does not
  resolve is UNKNOWN and writes nothing. The verdict is a DURABLE ledger row first and
...
def cmd_decide(args):
    ...
    if verb == "file":
        ...
        while "--ref" in rest:
            ...
            if r:
                refs.append(r)
        title, rc = freetext.tail("helm decide", "new", rest, "the card title")
        if rc is not None:
            return rc
    ...
    if verb == "verdict":
        ...
        if len(rest) < 2:
            ...
            return 2
        rid = rest[0]
        choice, rc = freetext.tail("helm decide", "decide", rest[1:],
                                   "the chosen option")
        if rc is not None:
            return rc
        row, err = decide(rid, choice or "", comment=comment)
        if not row:
            print("helm decide: " + err, file=sys.stderr)
            return 1
    ...
    if verb == "comment":
        if len(rest) < 2:
            print(DECIDE_USAGE, file=sys.stderr)
            return 2
        text, rc = freetext.tail("helm decide", "comment", rest[1:],
                                 "comment text")
        if rc is not None:
            return rc
        row, problem = comment_decision(rest[0], text or "")
        if not row:
            print("helm decide: " + problem, file=sys.stderr)
            return 1
