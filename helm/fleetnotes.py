...
def _clean_goto(raw):
    ...
    Owner-surface law `headlines-click-to-detail`: the headline tells the
    owner WHAT; the pointer says WHERE to act. Validate here because a
    hand-planted `javascript:` href stays dangerous no matter how perfectly
    HTML-escaped it is."""
    if raw is None:
        return None
...
def _project(key, row, now=None):
    ...
    Existing notes are prose blobs. The owner-surface law
    `headlines-click-to-detail` means a long or multiline legacy body must
    improve on its NEXT read: derive one bounded headline and collapse the untouched
    ...
    # boundary and report age 47h59m59s alongside stale=True (found on
...
def rows(notes_path=None, now=None):
    ...
    order and the rendered order disagreed (found on the first #234 tip).
    ...
    # feed and in the bottom section (r2). My no-reorder test did
...
def set_note(key, text, by=None, ts=None, notes_path=None, *, detail=None,
    ...
    `headlines-click-to-detail` owner-surface law, enforced at the one write
    seam rather than improvised by the browser.
...
def retire(key, retired=True, notes_path=None):
    ...
    (found on the first #234 tip; the defect was my CLAIM, and the
