...
from .. import localnames, physics
...
# HELM_* preferred, the predecessor's spelling accepted where this host's local
# names declare one — the env2 pattern catalog.py uses).
CWD_ROOTS = [r for r in os.environ.get(
    "HELM_CONFIG_ROOTS", localnames.legacy_env("CONFIG_ROOTS", f"{HOME}/dev")).split(":") if r]
