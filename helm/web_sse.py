...
# ---------- the SSE doorbell (poll -> push) ----------
# The lineage's settled pattern:
...
# every 20s + no-cache/no-transform (the standard anti-proxy-buffering pair);
# EventSource gives the client auto-reconnect for free.
#
# LIFECYCLE IS PER-SERVER (meld-converged; design + concurrency review
# clear): each _Server owns its OWN watcher thread + state, so
# the three reviewed races are UNEXPRESSIBLE rather than guarded — no
...
# content always arrives via the client's normal cursor poll (dissolve
# beats mechanize). The rooms-summary cache stays GLOBAL —
# it is orthogonal, lock-coupled, and TTL-bounded.
_SSE_WATCH_S = 0.25
_SSE_IDLE_WATCH_S = 10.0  # poll cadence when no client is connected — background
                           # watcher burns 54% of a core at 250ms forever
...
def _log_names(base):
    ...
    on /dev/shm/helm-chat: 8318 entries at the top, of which 103
...
def _chat_fingerprint():
    ...
    (cross-family review, both E2E-reproduced). The file NAME is folded into each
...
def _sse_tick(srv, boot=None):
    ...
    server's in-flight tick is a no-op (running checked
    under the server's own cond — pressure-tested), and so is a
...
def _sse_ensure_watcher(srv):
    ...
    per-server cond serializes racing streams (pressure-tested); the
