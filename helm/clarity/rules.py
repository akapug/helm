...
# A SENTENCE MAY OPEN WITH A LOWERCASE WORD AND THE SPLITTER CANNOT SEE IT.
# `_SENT_SPLIT` requires a capital, a digit or a quote after the terminator,
# so "...the plain form. helm canon wants..." is ONE sentence here and its
# word count is the SUM of two. helm's own nouns are lowercase — helm, task,
# every seat name — so the case is common and the refusal it produces quotes
# a span the author never wrote as one sentence. MEASURED 2026-09-09: six
# consecutive refusals on one compliant owner-ask body, each naming a
# different rule, none of them this. Naming the boundary costs one clause and
# ends that loop; it changes no verdict.
_ABBREV = frozenset((
    "e.g", "i.e", "vs", "etc", "cf", "no", "al", "fig", "approx", "ca",
    "incl", "resp", "dr", "mr", "mrs", "ms", "st", "inc", "jr", "sr",
))
_MERGED = re.compile(r"([A-Za-z0-9'\-.]*?)([.!?])\s+([a-z][A-Za-z0-9'\-]*)")


def merged_boundary(sentence):
    """The lowercase word the splitter refused to open a sentence at, or None.

    None ALSO for an abbreviation or an ellipsis, where not breaking is the
    RIGHT answer. A wrong hint is worse than no hint: it sends the author to
    a boundary that is not one, and this rule exists because a message that
    points at the wrong half costs rounds.
    """
    for m in _MERGED.finditer(sentence):
        at = m.start(2)
        if at and sentence[at - 1] == ".":
            continue                      # an ellipsis is not a boundary
        left = m.group(1).rstrip(".").lower()
        # AN INTERNAL DOT MEANS THE TERMINATOR IS PART OF THE TOKEN, not the
        # end of a sentence — U.S., a.m., Ph.D., config.py. Found on
        # "...in the U.S. today.", where the hint named 'today' and would have
        # sent the author to a boundary that does not exist. This is a RULE
        # rather than more blocklist entries because the blocklist can only
        # hold the initialisms someone thought of.
        if "." in left:
            continue
        if left in _ABBREV or (len(left) == 1 and left.isalpha()):
            continue
        return m.group(3)
    return None
...
def _check_sentence_length(doc, ctx):
    ...
    for para in doc:
        for line, s in para:
            n = len(words(s))
            if n > limit:
                msg = ("%d words (max %d in %s mode): %s"
                       % (n, limit, ctx["mode"], _clip(s)))
                word = merged_boundary(s)
                if word is not None:
                    msg += (" — COUNTED AS ONE SENTENCE: a sentence must open"
                            " with a capital, so the break before '%s' is"
                            " invisible here and this is the SUM of two."
                            " Capitalise it or reword." % word)
                out.append((line, msg))
    return out
