...
def account_dir(email):
    ...
    same fold homes.py names homes with (ann@example.com -> ann-example-com)."""
...
def _prune(email, keep=KEEP):
    ...
    the newest; older home-specific pre-images may be pruned. Best-effort."""
    snaps = snapshots(email)
...
def restore(snapshot_path, config_dir, require_free=False):
    ...
    if require_free:
        ...
        # re-probe, here), the launcher never launches into DRIFT homes, the occupant
