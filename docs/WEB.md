...
| `/api/todos` | — | the fleet todo mirror: `{seats, orphans, now}` — every roster seat with the task it is on right now (`active`, `done`/`total`, `ts`), plus mirrored sessions no seat claims. Read-only, pull-only; any surprise answers `{"unavailable": true}` at 200. Feeds the ledger tab's **fleet todos** panel (`helm todos --all` is the same read) |
