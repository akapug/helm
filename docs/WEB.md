...
| `/api/chat` | `room=` (default `main`) `since=` (rows already seen) | the chat poll read: `{room, lines, total, transport, rooms, roster}` — rows include reaction rows AND reply rows (`{reply_to, rts, rfrom}`; the client aggregates both); signed rows carry `{turn, receipt, chain, payload}`; `rooms[]` carries each channel's `{total, last, owner_unread, owner_mentions, seats[{seat, presence, last_seen}]}` (the sidebar's badges + ages); `roster` is the live seat names for @mention completion; `transport` = `{mode: signed|unsigned, url, head}`; a `since` past the end (the room rotated) resends everything |
...
| `/api/chat` | `{"text": ..., "room": "main", "name": "owner", "reply_to": <parent row id, optional>}` | the owner's chat post (`reply_to` threads it under that row, wakes the parent row's author mention-tier — replying replaces typing the @mention — and, when signed, binds the parent into the digest): shortcodes expand, the digest rides a signed turn when the room node answers (server-side, as the server's `HELM_CELL_PROFILE`, default `owner`), append to the RAM room + drop the `owner-unread` marker the shipped reflex fires on until an agent's `helm chat read` consumes it |
| `/api/chat/react` | `{"emoji": ":tada:", "tts": <target ts>, "tfrom": <target from>, "room": "main", "name": "owner"}` | the owner's click-to-react: a typed reaction row referencing the target message, signed like a post |
