...
Helm owns **local human + agent multiplayer inside one cave**. A separate hosted
product owns the remote/web side. The boundary between them is an adapter
contract, not a shared implementation, so neither side needs to know whether the
local operator uses Orca, herdr, a bare shell, a browser, or another
metaharness.
...
- A hosted product can bridge remote/web caves by implementing the relay/presence
