...
[![CI](https://github.com/akapug/helm/actions/workflows/ci.yml/badge.svg)](https://github.com/akapug/helm/actions/workflows/ci.yml)
...
Then it runs the fleet: several coding agents from **different model
families** working the same repo at once, each in its own pane, talking in one
room, reviewing each other's work before anything lands.

**Try it — no install, standard library only:**

```console
$ ./bin/helm sync     # discover your projects across every harness
$ ./bin/helm web      # the whole cockpit, warm, in a browser
```
...
**Requires Python 3.9+ on Linux**, and nothing else. CI runs the full suite on
3.9 through 3.13, so that floor is measured rather than asserted; `tomllib`
(3.11+) is used when present and config validation degrades to a warning
without it.
...
Optional PATH install — the entry script resolves through symlinks, so putting
...
- **Environment** — every `HELM_*` variable (all optional):
  ...
  default `~/.helm`; legacy `MELD_*` spellings are accepted as fallbacks
  forever.
