...
class _SeatBase(unittest.TestCase):
    ...
    def _exec_launch(self, launch_sh, extra_parent=None):
        ...
        # THIS ARM INTENDS A COMPLETE RUNNABLE CONTRACT: it EXECUTES launch.sh,
        # whose generated preflight refuses to start a session while a required
        # external guard cannot run. The env here is deliberately scrubbed, so
        # the harness-owned pin has to be carried into it explicitly — without
        # this the arm measures the preflight instead of the config-dir pinning
        # it exists to check.
        env["HELM_SUITE_GUARD"] = os.environ["HELM_SUITE_GUARD"]
        # NO HELM_HOME HERE, DELIBERATELY. Injecting it made these arms pass
        # while MASKING a durable launch regression: the generated script must
        # carry its own minting home, so a seat launches from an ordinary
        # shell. A fixture that needs an extra variable to pass is evidence
        # about the code, not a fact of life.
...
class RebootChecklistTest(unittest.TestCase):
    ...
    It is an operator document the release export omits; there this skips
    with that reason, and everywhere else a missing checklist still fails."""

    REL = "docs/REBOOT_CHECKLIST_1.4.149.md"
    PATH = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))),
                        *REL.split("/"))

    def test_checklist_exists_and_names_exact_checks(self):
        why = _release.omitted(self.REL)
        if why:
            self.skipTest(why)
