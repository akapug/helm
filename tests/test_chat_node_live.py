...
class LiveNodeTest(unittest.TestCase):
    ...
    def test_05_reflex_loop_through_the_signed_room(self):
        ...
        chat.post("fleet, morning", who="daria")
        ...
        self.assertIn("daria: fleet, morning", out.getvalue())
...
class LiveNodeLifecycleTest(unittest.TestCase):
    def test_failed_class_setup_restores_cwd_and_environment(self):  # noqa: VACUOUS_ASSERTION — the mocked node is asserted terminated and waited, its log closed, and its tmp tree removed after the injected skip
        ...
        try:
            ...
            # THE KEYS THAT MOVED, NEVER THE MAPPING (task/2370). Comparing
            # two environment dicts renders both on failure, values included.
            # Both dicts are keyed by ENV_KEYS exactly, so an empty drift list
            # is the same contract as dict equality was.
            self.assertIn("HELM_HOME", ENV_KEYS)   # the list walked is real
            self.assertEqual([k for k in ENV_KEYS
                              if os.environ.get(k) != prior[k]], [],
                             "class setup failure left these env KEYS unrestored")
