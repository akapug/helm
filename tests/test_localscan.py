...
class ScanTest(unittest.TestCase):
    ...
    def test_shared_store_collapses_isolated_store_stays_separate(self):
        ...
        accounts = [
            {"name": "a@x.com", "provider": "anthropic", "home": os.path.join(self.tmp, "home-a")},
            {"name": "b@x.com", "provider": "anthropic", "home": os.path.join(self.tmp, "home-b")},
            {"name": "iso@x.com", "provider": "anthropic", "home": os.path.join(self.tmp, "iso-home")},
...
class CrosscheckTest(unittest.TestCase):
    ...
    def test_agreeing_sources_are_plausible_and_probe_happens_once(self):
        ...
        prov = StubProvider([self.acct("busy@x.com", home)],
                            [hist("busy@x.com", 0.34, 0.12, 0.08)])
    ...
    def test_header_active_scan_empty_is_drift(self):
        ...
        prov = StubProvider([self.acct("ghost@x.com", home)],
                            [hist("ghost@x.com", 0.55)])
    ...
    def test_scan_active_header_idle_is_drift(self):
        ...
        prov = StubProvider([self.acct("local@x.com", home)],
                            [hist("local@x.com", 0.0)])
        rows = localscan.crosscheck(prov, now)
    ...
    def test_freshest_history_row_wins(self):
        ...
        prov = StubProvider(
            [self.acct("h@x.com", home)],
            [hist("h@x.com", 0.0, probed_at="2026-01-01T00:00:00Z"),
             hist("h@x.com", 0.20, probed_at="2026-01-02T00:00:00Z")])
        rows = localscan.crosscheck(prov, now)
    ...
    def test_shared_store_is_commingled_never_a_per_account_verdict(self):
        ...
        prov = StubProvider([self.acct("a@x.com", homes[0]),
                             self.acct("b@x.com", homes[1])],
                            [hist("a@x.com", 0.9), hist("b@x.com", 0.9)])
        rows = localscan.crosscheck(prov, now)
    ...
    def test_no_header_observation(self):
        ...
        prov = StubProvider([self.acct("h@x.com", home)], [])
        rows = localscan.crosscheck(prov, now)
...
class CmdTest(unittest.TestCase):
    ...
    def test_cmd_renders_scan_header_and_signal(self):
        ...
        prov = StubProvider(
            [{"name": "h@x.com", "provider": "anthropic", "home": os.path.join(tmp, "h")}],
            [hist("h@x.com", 0.34, 0.12, 0.08)])
        rc, out = self.run_cmd(prov)
        self.assertEqual(rc, 0)
        self.assertIn("helm creds crosscheck", out)
        self.assertIn("h@x.com", out)
        ...
        self.assertEqual(rc, 0)
        self.assertEqual(json.loads(out)[0]["accounts"], ["h@x.com"])
