...
WHAT HAPPENED (2026-07-24). A personal, privacy-sensitive owner skill under
`agents/claudecode/skills/` was TRACKED, and had already been pushed to
`origin/main`. Its files carry material about the owner personally, including
verbatim private quotes. The repo is private, so nothing was publicly disclosed —
but a private repo is still a REMOTE, and this repo's whole as-public posture is
that it stays ONE VISIBILITY FLIP away from public. A flip would have published
it instantly.

THIS DOCSTRING IS DELIBERATELY UNSPECIFIC, and that is itself the second lesson
(2026-07-25). The first version of this file explained the hazard by DESCRIBING
...
file is the leak. Three independent reviewers (a from-clone CERTIFY, a re-cert
that produced 7 named classes, and my own certification) all passed the tree
while these sat in it. Adding one more content pattern would not have caught it;
...
THE HISTORY IS NOT FIXED BY THIS. These blobs remain in the commits already on
`origin/main`, and removing them from history is a rewrite of a shared remote —
an owner decision, surfaced, not something a test can undo. This file only
guarantees they stop entering NEW trees.

THE THIRD LESSON (2026-07-29): A SUITE-RUN GUARD CANNOT CLOSE A TIMING GAP. A
...
def _standalone_hit(hay, needle):
    ...
    WHY THIS IS NOT A PLAIN SUBSTRING TEST, measured on this estate: 13 needles
    are configured and THREE ARE UNDER 8 CHARACTERS (min 6). A 6-byte string
...
class NeverTrackTest(unittest.TestCase):
    ...
    def test_no_never_track_path_is_in_the_index(self):  # noqa: VACUOUS_ASSERTION — emptiness IS the claim; the unconditional _tracked() control above the loop proves the index reader lists a tracked file
        ...
        # POSITIVE CONTROL on the same observable: the index reader DOES list
        # a tracked file, so the empty result below is a reading, not a stub.
        self.assertEqual(_tracked("helm/nevertrack.py"), ["helm/nevertrack.py"])
        leaked = {}
        for path, why in nevertrack.never_track_set().items():
            found = _tracked(path)
            if found:
    ...
    def test_untracking_did_not_EMPTY_a_path_that_is_present(self):  # noqa: VACUOUS_ASSERTION — a clone holds none of these paths by construction (the docstring's first lesson); the check applies only where one is present
        """Untracked is NOT deleted — but that is only checkable where the files
        exist, and CONDITIONING it is the whole point.

        FIRST VERSION OF THIS TEST WAS WRONG, caught by kimi on a fresh clone: it
        ...
        commit exists to protect, plus the fab gate on every node.
        ...
        for path, _why in nevertrack.never_track_set().items():
    ...
    def test_a_needle_buried_in_a_longer_word_is_not_a_leak(self):
        ...
        for hay, needle, want, why in (
                ...
                (b"x user@ex.example y", b"user@ex.example", True, "email standalone"),
                ...
                (b"myuser@ex.example", b"user@ex.example", False, "a DIFFERENT address"),
    ...
    def test_no_tracked_file_carries_a_real_owner_account(self):
        ...
        anywhere" rule would fire on example.com and on any throwaway test
        identity and get switched off, and a guard people switch off protects
        nothing.
    ...
    def test_thin_needles_missing_address_shape_triggers_census_note(self):
        """12 needles, none containing @ — armed but blind to email/account-id
        shapes. The census must say what class is absent."""
        from helm import nevertrack
        note, classes = nevertrack._needle_census(
    ...
    def test_ls_files_already_sees_the_staged_set_so_a_union_adds_nothing(self):
        """THE PREMISE PIN for the 2026-07-29 incident, so the disproved fix
    ...
    def test_gitignore_also_covers_them_so_a_re_add_is_hard(self):  # noqa: VACUOUS_ASSERTION — the check-ignore control on a shipped prefix runs unconditionally before the local loop; a checkout with no local list has nothing local to judge
        ...
        A MACHINE-LOCAL prefix is ignored on that machine, not in the tracked
        .gitignore — naming it there would publish the name the local list
        exists to keep out. So it is judged by git's own ignore reading
        (.git/info/exclude, core.excludesFile), and a checkout with no local
        list has nothing to judge."""
        with open(os.path.join(ROOT, ".gitignore"), encoding="utf-8") as f:
            ignored = f.read()
        for path in NEVER_TRACK:
            self.assertIn(path, ignored,
                          "%s should also be in .gitignore" % path)
        # POSITIVE CONTROL on the check-ignore observable, unconditional: a
        # shipped prefix IS ignored, so a local prefix's rc below is a reading.
        first = sorted(NEVER_TRACK)[0]
        self.assertEqual(subprocess.run(
            ["git", "check-ignore", "-q", "--no-index", "--", first],
            cwd=ROOT, timeout=60).returncode, 0, first)
        local, where = nevertrack.load_local_never_track()
        for path in local:
            p = subprocess.run(["git", "check-ignore", "-q", "--no-index",
                                "--", path], cwd=ROOT, timeout=60)
            self.assertEqual(p.returncode, 0,
                             "%s (from %s) is not ignored on this machine — "
                             "add it to .git/info/exclude" % (path, where))

    def test_the_tracked_gitignore_names_no_machine_local_prefix(self):
        """The other half of the same law: whatever the local list names must
        stay OUT of the tracked .gitignore, or the name ships anyway."""
        with open(os.path.join(ROOT, ".gitignore"), encoding="utf-8") as f:
            ignored = f.read()
        # POSITIVE CONTROL on the same membership test, unconditional: the
        # shipped prefixes ARE found in the text it reads.
        self.assertEqual([p for p in NEVER_TRACK if p not in ignored], [])
        self.assertTrue(NEVER_TRACK)
        local, where = nevertrack.load_local_never_track()
        if not local:
            self.skipTest("no machine-local never-track list (%s)" % where)
        shipped = [p for p in local if p in ignored and p not in NEVER_TRACK]
        self.assertEqual(shipped, [], "machine-local prefixes named in the "
                                      "tracked .gitignore: %r" % shipped)


class LocalNeverTrackTest(unittest.TestCase):
    """THE MACHINE-LOCAL LIST. A path whose NAME is itself private cannot be
    a key in the shipped NEVER_TRACK without publishing that name, so such
    prefixes are listed outside the tree ($HELM_NEVER_TRACK_LOCAL, else
    ~/.helm/_global/never-track.txt) and the staged scan enforces them exactly
    like the shipped ones. Every case here plants its own list and repo."""

    def setUp(self):
        self.tmp = tempfile.mkdtemp(prefix="helm-test-local-never-track-")
        self.addCleanup(shutil.rmtree, self.tmp, ignore_errors=True)
        self.listfile = os.path.join(self.tmp, "never-track.txt")
        self.absent = os.path.join(self.tmp, "absent.txt")

    def env(self, listfile):
        return mock.patch.dict(os.environ, {"HELM_NEVER_TRACK_LOCAL": listfile,
                                            "HELM_PRIVATE_NEEDLES": self.absent})

    def test_the_local_list_is_read_from_outside_the_tree(self):
        with open(self.listfile, "w", encoding="utf-8") as f:
            f.write("# a comment line\n\nlocal/private-skill/\n  other/thing  \n")
        with self.env(self.listfile):
            local, path = nevertrack.load_local_never_track()
            merged = nevertrack.never_track_set()
        self.assertEqual(path, self.listfile)
        self.assertEqual(sorted(local), ["local/private-skill/", "other/thing"])
        self.assertIn(self.listfile, local["local/private-skill/"],
                      "a local entry's reason names the file it came from")
        self.assertIn("local/private-skill/", merged)
    ...
    def test_an_absent_local_list_is_empty_and_keeps_the_shipped_law(self):
        with self.env(self.absent):
            local, path = nevertrack.load_local_never_track()
            merged = nevertrack.never_track_set()
        self.assertEqual((local, path), ({}, self.absent))
        self.assertEqual(merged, NEVER_TRACK)
        self.assertTrue(merged, "control: the shipped law is not empty")

    def test_the_staged_scan_refuses_a_path_under_a_local_prefix(self):  # noqa: VACUOUS_ASSERTION — the empty control reading is bracketed by the refusal of the SAME staged file under a listed prefix, asserted non-empty just above it
        repo = os.path.join(self.tmp, "repo")
        os.makedirs(os.path.join(repo, "local", "private-skill"))
        ...
        git("add", "seed.txt")
        git("commit", "-qm", "seed")
        with open(os.path.join(repo, "local", "private-skill", "SKILL.md"), "w") as f:
            f.write("plain text\n")
        self.assertEqual(git("add", "local/private-skill/SKILL.md").returncode, 0)
        with open(self.listfile, "w", encoding="utf-8") as f:
