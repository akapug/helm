"""Hermetic tests for helm.silent_swallow — the empty-completion loud-fail.
...
signature proven on the live codex transcript (75/75 swallows, 0 false
...
from helm import seat, silent_swallow
...
class SilentSwallowTest(unittest.TestCase):
    def setUp(self):
    ...
    def test_swallow_signature_detected(self):
        # end_turn + empty content + output_tokens>0 = drop-after-generate
        self.plant([asst([], ot=103)])
        f = silent_swallow.scan_seat("codex")
        self.assertIsNotNone(f)
        self.assertEqual(f["output_tokens"], 103)
        self.assertEqual(f["session"], SID)

    def test_empty_text_block_is_a_swallow(self):
        # the proxy translates a swallow into an empty text block + end_turn
        self.plant([asst([{"type": "text", "text": ""}], ot=49)])
        self.assertIsNotNone(silent_swallow.scan_seat("codex"))

    def test_empty_thinking_wrapper_is_a_swallow(self):
        # the line464 form: empty thinking wrapper, real text dropped
        self.plant([asst([{"type": "thinking", "thinking": "",
                            "signature": "gAAAA"}], ot=57)])
        self.assertIsNotNone(silent_swallow.scan_seat("codex"))

    # -- non-swallows (false-positive guards) ------------------------------

    def test_real_text_is_not_a_swallow(self):
        self.plant([asst([{"type": "text", "text": "an answer"}], ot=50)])
        self.assertIsNone(silent_swallow.scan_seat("codex"))

    def test_tool_use_is_not_a_swallow(self):
        self.plant([asst([{"type": "tool_use", "name": "Bash"}], ot=50)])
        self.assertIsNone(silent_swallow.scan_seat("codex"))

    def test_deliberate_thinking_park_is_not_a_swallow(self):
        self.plant([asst([{"type": "thinking", "thinking": "no action"}],
                         ot=30)])
        self.assertIsNone(silent_swallow.scan_seat("codex"))

    def test_zero_output_tokens_is_not_a_swallow(self):
        # a genuine refusal/classifier produces ~0 output — not our class
        self.plant([asst([], ot=0)])
        self.assertIsNone(silent_swallow.scan_seat("codex"))

    def test_tool_use_stop_reason_is_not_a_swallow(self):
    ...
    def test_reports_the_newest_swallow(self):
    ...
    def test_latch_suppresses_repeat_same_ts(self):
        ...
        r1 = silent_swallow.check(seats=["codex"], post=False)
        self.assertEqual(len(r1["alerted"]), 1)
        r2 = silent_swallow.check(seats=["codex"], post=False)
    ...
    def test_alert_text_names_seat_tokens_and_integrator(self):
        txt = silent_swallow._alert_text(
        ...
        self.assertIn("SILENT-SWALLOW", txt)
