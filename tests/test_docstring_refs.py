...
WHAT THIS FILE HOLDS:
  ...
  * WHY THE VERDICT IS REPO-LOCAL, learned the long way. Two earlier rounds
    ...
    and retired 19 exemptions on their strength. @codex-3's #88 P1 measured
...
OAuth client id, "0123456789abcdef") are code, not citations, and stay out of
...
# the resolver is required to pass. Cited in helm/docref_guard.py's module
# docstring: the repository's root commit, resolvable from every clone.
MUST_HIT = "99fd8406"
...
# MUST-HIT patch identity: recomputed from the repository's first commit
# pair. Unlike a Git object id, cat-file must reject it while
# patch-id --stable verifies it.
MUST_HIT_PATCH = "51c087e2d51143dbf08015a8f34598f31a146879"
...
# snapshot-owned rather than read out of the tree being judged (@codex F2,
...
class DocstringCommitRefsTest(unittest.TestCase):
    ...
    def test_must_miss_control_fails_the_resolver(self):
        ...
        # yes to a real commit, so its no below is a verdict, not a stub.
        self.assertTrue(_resolves(MUST_HIT),
                        "resolver rejected the known-live control commit")
...
class DocrefStagedRungTest(unittest.TestCase):
    ...
    def test_the_category_split_bootstraps_under_the_installed_flat_snapshot(self):  # noqa: VACUOUS_ASSERTION — the same historical snapshot immediately refuses a fabricated citation after admitting the self-only category migration
        ...
        """
        # The pre-split evaluator, vendored verbatim as a fixture: the
        # bootstrap property is about the rung a machine may still have
        # INSTALLED, so the subject must be the real old source — and a
        # fixture keeps it reachable from every clone, where the historical
        # commit is not.
        flat = os.path.join(ROOT, "tests", "fixtures",
                            "docref_flat_snapshot.py")
    ...
    def test_a_new_key_no_ledger_vouches_for_is_REFUSED(self):
        """@codex's repro, closed. A fabricated LEDGER_CITED key plus its
    ...
    def test_an_unreadable_ledger_fails_CLOSED(self):
        """I had this backwards and @codex corrected it. I argued fail-OPEN
    ...
    def test_the_authority_evaluator_never_executes_the_judged_tree(self):
        """@codex F2, pinned. My first cure shelled out to the judged tree's
        own tests/test_docstring_refs.py — and not even its staged copy. Their
    ...
    def test_the_trusted_root_is_the_snapshots_own_checkout(self):  # noqa: VACUOUS_ASSERTION — both arms assert an EQUALITY against a constructed path, never an absence; the run-from-source arm is the unconditional positive control on the same _trusted_root() call, and the paths are deliberately synthetic (they need not exist for a pure path computation)
        """The evaluator must be SNAPSHOT-OWNED (@codex F2). Under the suite
