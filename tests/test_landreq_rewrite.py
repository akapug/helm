...
class RewriteBase(unittest.TestCase):
    ...
    def rewrite(self):
        ...
        # FILTER_BRANCH_SQUELCH_WARNING SILENCES A `sleep 10`, NOT A CHECK.
        # git-filter-branch prints a deprecation warning and then sleeps ten
        # seconds so a human reads it (git's own source, line 96). Two tests
        # call this helper, so the module paid twenty seconds per run for a
        # message no CI reader has ever seen. Scoped to this one call rather
        # than the environment, so nothing else in the suite is silenced.
...
class RewriteSquelchTest(RewriteBase):
    """PINS THE SQUELCH, because without this arm DELETING IT LEAVES THE SUITE
    GREEN and silently restores twenty seconds per run — codex named exactly
    that hole on row 37f557c04ffa.

    git-filter-branch prints a deprecation warning and then `sleep 10` unless
    FILTER_BRANCH_SQUELCH_WARNING is set (git's own source, line 96). Two tests
    call rewrite(), so the module paid twenty seconds for a message no CI
    reader has ever seen. The cost is invisible to every assertion in this
    file, which is why it needs a pin rather than a comment."""

    def test_the_rewrite_squelches_gits_ten_second_nag(self):
        seen = []
        real = globals()["_git"]

        def spy(cwd, *args, **kw):
            seen.append((args, dict(kw.get("env") or {})))
            return real(cwd, *args, **kw)

        globals()["_git"] = spy
        self.addCleanup(lambda: globals().__setitem__("_git", real))
        self.rewrite()

        fb = [(a, e) for a, e in seen if a and a[0] == "filter-branch"]
        # POSITIVE CONTROL on the same observable: the spy saw the real call,
        # so an empty env below means "not squelched", never "never ran".
        self.assertEqual(len(fb), 1, "rewrite() must invoke filter-branch once")
        self.assertEqual(fb[0][1].get("FILTER_BRANCH_SQUELCH_WARNING"), "1",
                         "filter-branch must be squelched at the call site — "
                         "without it git sleeps ten seconds per invocation and "
