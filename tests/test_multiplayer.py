...
class MultiplayerTest(unittest.TestCase):
    ...
    def test_cli_two_actor_dogfood_contract(self):
        rc, out, _ = self._run(["presence", "--cave", "demo", "--actor", "daria",
                                "--connection", "phone", "--state", "editing"])
        self.assertEqual(rc, 0)
        self.assertEqual(json.loads(out)["actor"], "daria")
        self.assertEqual(self._run(["publish", "board", "--stdin", "--cave", "demo",
                                    "--actor", "daria"], stdin="opaque-a")[0], 0)
        self.assertEqual(self._run(["publish", "board", "--stdin", "--cave", "demo",
    ...
    def presence_ttl(self, value):
        return self._run(["presence", "--cave", "demo", "--actor", "daria",
                          "--ttl", value])

    def test_cli_presence_ttl_takes_a_unit_suffix(self):
        rc, out, err = self.presence_ttl("1m")
        self.assertEqual(0, rc, err)
        row = json.loads(out)
        self.assertEqual(60, round(row["expires"] - row["seen"]))

    def test_cli_presence_ttl_bare_seconds_still_work(self):
        """CONTROL: the spelling that always worked keeps its meaning."""
        rc, out, err = self.presence_ttl("30")
        self.assertEqual(0, rc, err)
        row = json.loads(out)
        self.assertEqual(30, round(row["expires"] - row["seen"]))

    def test_cli_presence_ttl_refuses_an_unknown_suffix_naming_the_forms(self):
        rc, out, err = self.presence_ttl("4x")
    ...
    def test_cli_presence_ttl_keeps_its_ceiling_under_units(self):
        """A suffix is a spelling, not a way past the heartbeat's bound."""
        rc, out, err = self.presence_ttl("6m")
        self.assertEqual(2, rc, err)
        self.assertEqual("", out)
        self.assertIn("between 1 and %d seconds" % multiplayer.MAX_TTL, err)
