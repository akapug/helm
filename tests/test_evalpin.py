...
class ArmCommandTest(unittest.TestCase):
    ...
    def test_the_ONLY_env_the_rig_adds_is_the_config_override(self):
        ...
        # KEY NAMES, NEVER VALUES (task/2370). `assertEqual(cmd["env"], base)`
        # renders BOTH mappings on failure, and both are copies of this
        # process's environment — on a build node that is real credentials in
        # the unittest report and in the fab artifact. These two arms are the
        # same contract: identical key sets, and no key whose value differs.
        self.assertIn(evalrun.CONFIG_OVERRIDE_ENV,
                      tuple(base))                  # base is populated
        self.assertEqual(sorted(cmd["env"]), sorted(base),
                         "the child env KEY SET is not the rig's plus the override")
        self.assertEqual([k for k in sorted(base)
                          if cmd["env"].get(k) != base.get(k)], [],
                         "the child env differs from the rig env at these KEYS")
        self.assertNotIn(evalrun.CONFIG_OVERRIDE_ENV, tuple(os.environ),
