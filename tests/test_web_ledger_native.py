...
from helm import chat, web, web_ledger, web_ui_loader  # noqa: E402
...
class NativeBase(unittest.TestCase):
    ...
    def setUp(self):
        # THE ROOM WALK IS CACHED PROCESS-WIDE, AND EVERY CLASS HERE SEEDS ITS
        # OWN HELM_HOME. Without this, the empty-home class reads the seeded
        # class's pulse — a stale body crossing a fixture boundary, which is
...
def _seed_chat():
    ...
    rows = [{"ts": "2026-07-21T10:00:00Z", "from": "kimi", "text": "hi"},
            ...
            {"ts": "2026-07-21T10:00:09Z", "from": "daria", "react": "🔥",
...
class TestNativeLedger(NativeBase):
    ...
    def test_chat_pulse_counts_msgs_not_reacts(self):
        ...
        self.assertEqual((p["last_from"], p["last_room"]), ("daria", "main"))
...
class TestNativeLedgerEmptyHome(NativeBase):
    def test_everything_fails_open_empty(self):
        ...
        # age_s is the pulse's OWN age, not a projection leg — a quiet pulse
        # still has to say when it was read, or "quiet" is a claim about now
...
class TestNativeChatPulseIsCached(NativeBase):
    """task/2703: /api/ledger/native read EVERY room on EVERY request.

    Measured on a live corpus of 459 rooms / 18,082 rows, `_native_chat_pulse`
    costs 162 ms per call, of which `chat.read` over the rooms is 181 ms of CPU
    by cProfile, and the console polls this endpoint on a 3000 ms timer from
    every open socket. The arms below bind the two halves
    ...
    def test_the_room_walk_runs_ONCE_for_many_requests_in_one_window(self):
        # UNCONDITIONAL POSITIVE CONTROL. The counter is a patched callable and
        ...
        for _ in range(6):
            status, d = self.req("/api/ledger/native")
            self.assertEqual(status, 200)
            self.assertEqual(d["chat"]["msgs"], 2)
        self.assertEqual(len(self.calls), 1,
                         "six requests inside one TTL window walked the rooms "
                         "%d times; the cache is not holding" % len(self.calls))

    def test_the_window_ENDS_and_a_new_post_reaches_the_pulse(self):
        _, first = self.req("/api/ledger/native")
        self.assertEqual(first["chat"]["msgs"], 2)

        # the room is CLASS fixture state — the sibling arms count on two msgs
        ...
        # INSIDE the window the new post is invisible — which is the cache
        # working, and exactly why the age below has to be on the wire.
        _, held = self.req("/api/ledger/native")
        self.assertEqual(held["chat"]["msgs"], 2)

        ts, body = web._qstate["ledger-native-pulse"]
        web._qstate["ledger-native-pulse"] = (
            ts - web_ledger._LEDGER_WALK_TTL_S - 1, body)
        _, after = self.req("/api/ledger/native")
        self.assertEqual(after["chat"]["msgs"], 3,
                         "the TTL expired and the pulse still held the old walk")
        self.assertEqual(after["chat"]["last_ts"], "2026-07-21T10:00:20Z")

    def test_the_pulse_states_its_own_age_and_a_held_one_says_so(self):
        """A CONFIDENT STALE BOARD IS WORSE THAN THE TIMEOUT IT REPLACES.

        The owner reads these counts as "is the fleet chattering". `last_ts` is
        the POST's own stamp and keeps ticking identically whether the walk
        behind it is a second or a whole window old, so age_s is the only thing
        on this payload that can tell a held reading from a live one."""
        _, fresh = self.req("/api/ledger/native")
        # UNCONDITIONAL POSITIVE CONTROL on the SAME observable: a near-zero
        # age is the reading this arm wants, and it is also what an empty or
        # defaulted pulse would show. `msgs` says this payload is the seeded
        # walk, so the small age below is freshness rather than vacancy.
        self.assertEqual(fresh["chat"]["msgs"], 2)
        self.assertLess(fresh["chat"]["age_s"], 5)

        # THE AGE COMES FROM THE BODY'S OWN read_ts, NOT FROM THE CACHE SLOT.
        # Backdating only the slot's timestamp left age_s at 0.0 here, and that
        # was the payload telling the truth: the walk really had just run. The
        # stamp the reader sees is the instant the ROOMS WERE READ, so ageing
        # the reading means ageing that — the slot moves with it only so the
        # entry stays inside its window and the held body is what gets served.
        ts, body = web._qstate["ledger-native-pulse"]
        body["read_ts"] -= 9
        web._qstate["ledger-native-pulse"] = (ts - 9, body)
        _, held = self.req("/api/ledger/native")
        self.assertGreaterEqual(held["chat"]["age_s"], 9)
        # the served body is COPIED, never stamped — the next reader inside the
        # window gets its own age, not the one this request computed.
        # UNCONDITIONAL POSITIVE CONTROL on the SAME observable: age_s is
        # absent from this dict, and that proves nothing unless the dict is the
        # real cached pulse. `msgs` is a key the pulse always carries, so its
        # presence establishes what the absence beside it is about.
        cached_pulse = web._qstate["ledger-native-pulse"][1]["pulse"]
        self.assertEqual(cached_pulse["msgs"], 2)
        self.assertNotIn("age_s", cached_pulse)
...
class TestRosterSingleFlightCache(unittest.TestCase):
    ...
    def tearDown(self):
        ...
        # planted data, not our leaked {seat: agent-deadbeef} (kimi xrev).
        web._ROSTER_REP_CACHE.clear()

    # THE DOUBLE OWES THE WHOLE CALL, INCLUDING THE HALF THIS CLASS IS NOT
    # ABOUT. The report takes the render's ONE availability snapshot from its
    # caller, and a double carrying the older arity raises TypeError at
    # ARGUMENT BINDING — before its body runs, so the compute counter stays 0
    # and every arm about counting reports a count for a call that never
    # happened. The parameter is EXPLICIT rather than a **kwargs sink: a sink
    # would keep these arms green across the next signature change too, which
    # is exactly how they came to stand over an impossible call.
    def _fake_report(self, room, availability=None):
        self.calls["n"] += 1
        self.handed.append(availability)
        return {"seats": [{"seat": "agent-deadbeef", "home_room": None,
                           "cwd": "/tmp/claude-x/s"}], "claims": []}

    def _assert_handed_a_snapshot(self, n):
        """The positive control under every arm here: the computes really went
        through the shipped door, and each was handed ONE acquired record —
        {upstream, age, error, stale_s}, a dict upstream, and the console's own
        staleness bar riding with the bytes — never None."""
        self.assertEqual(n, len(self.handed))
        for rec in self.handed:
            self.assertIsInstance(rec, dict)
            for key in ("upstream", "age", "error", "stale_s"):
                self.assertIn(key, rec)
            self.assertIsInstance(rec["upstream"], dict)
            self.assertEqual(web._UPSTREAM_STALE_S, rec["stale_s"])

    def test_concurrent_pollers_share_one_compute(self):
        # A RAISE INSIDE A POLLER IS NOT A COUNT. threading prints a target's
        # exception to stderr and joins cleanly, so a rebuild that could not
        # run AT ALL arrived here as "0 computes" — this arm failing on the
        # single-flight sentence for a reason that has nothing to do with
        # locking, while the real defect stayed in a stream nobody reads. The
        # pollers report what happened to them and that is read FIRST.
        blew = []
        ...
        def poll():
            try:
                ...
                blew.append(traceback.format_exc())

        # THE COLLECTOR IS PROVED ON THIS OBSERVABLE BEFORE IT IS BELIEVED.
        # An empty `blew` is only news if a poller that DOES raise lands in it,
        # so one planted raise runs through the same wrapper first.
        def boom(room, availability=None):
            raise RuntimeError("planted rebuild failure")

        with mock.patch("helm.seats.roster_report", boom):
            probe = threading.Thread(target=poll)
            probe.start()
            probe.join()
        self.assertEqual(1, len(blew))
        self.assertIn("planted rebuild failure", blew[0])
        del blew[:]
        web._ROSTER_REP_CACHE.clear()
        ...
        with mock.patch("helm.seats.roster_report", self._fake_report):
            ...
            for t in threads:
                t.join()
        self.assertEqual([], blew, "a poller raised: %s" % "".join(blew)[-400:])
        self.assertEqual(self.calls["n"], 1)   # single-flight, not 8 stacked
        self._assert_handed_a_snapshot(1)
    ...
    def test_ttl_expiry_recomputes_and_freshness_serves_cache(self):
        ...
        self._assert_handed_a_snapshot(2)
...
class TestRosterCacheIsBoundToItsHome(unittest.TestCase):
    """THE KEY MUST NAME THE SUBJECT. `main` is a room, not a world: a cached
    report describes ONE roster file, and while the key said only the room, a
    process that moved to another helm home was served the previous home's
    seats for a whole freshness window — confidently, with nothing on the
    payload saying the rows came from somewhere else.

    MEASURED, not imagined. In a suite composition this served one module's
    empty roster to the next module's identical call 0.6s later, standing in a
    different HELM_CHAT_DIR, so a seat that had just been written was absent
    from the payload meant to carry it. That reading is indistinguishable from
    a seat that never joined, which is why the cure belongs in the cache and
    not in every fixture that remembers to empty it — several already do, and
    that is the tell that the substrate owed this.

    NO DOUBLE HERE, ON PURPOSE. The arms above mock `roster_report` to count
    computes; this one asks the whole shipped path for the ROWS, because the
    property is about which home's bytes come back and a double has no home."""
    ...
    def _home(self, seat):
        """A fresh helm home carrying exactly one seat. -> that seat's name."""
        from helm import seats
        tmp = tempfile.mkdtemp(prefix="helm-test-roster-home-")
        self.dirs.append(tmp)
        os.environ["HELM_HOME"] = os.path.join(tmp, "helm")
        os.environ["HELM_CHAT_DIR"] = os.path.join(tmp, "chat")
        seats.write_roster(seat, session=seat[0] * 32, cwd=tmp)
        return seat

    def _seats(self):
        return sorted(s.get("seat") for s in
                      web._roster_cached("main").get("seats", []))

    def test_a_report_built_under_another_home_is_never_served_here(self):
        first = self._home("alpha")
        # POSITIVE CONTROL on the same observable: the first home's own rows
        # really do come back through the cached door, so the second read's
        # answer cannot be read as "this path returns nothing here".
        self.assertEqual(self._seats(), [first])
        self.assertIn("main", web._ROSTER_REP_CACHE)
        warm = web._ROSTER_REP_CACHE["main"][1]

        second = self._home("beta")            # same room, another world
        got = self._seats()
        self.assertEqual(got, [second],
                         "the roster cache served another home's seats: "
                         "expected %r, got %r" % ([second], got))
        self.assertIsNot(web._ROSTER_REP_CACHE["main"][1], warm,
                         "the same rep object came back after the home "
                         "changed — the entry outlived its subject")
...
class TestRoomsSummaryPerRoomParseMemo(unittest.TestCase):
    """Brick #2.5: the TTL cache above bounds how OFTEN the summary
...
class TestSseDoorbellWatcher(unittest.TestCase):
    ...
    def test_the_listing_is_not_repeated_while_the_directory_is_unchanged(self):  # noqa: VACUOUS_ASSERTION — the call RESULT is bound, not discarded: assertIsNotNone(fp0) then 20 assertEqual(_chat_fingerprint(), fp0) inside the spy, then an APPEND asserted to CHANGE it. The analyzer cannot credit them because the result is compared to a local rather than a literal; the counted-call assertions are the SECONDARY claim
        ...
        on the live chat dir 2026-08-06: 8318 entries, 103 of them .jsonl, and
    ...
    def test_the_ledger_poll_stops_when_nobody_is_looking(self):
        """task/281's title is "whether or not anyone is looking", and this is
        ...
        MEASURED 2026-08-06: /api/ledger costs 1160ms of server CPU per
        ...
        self.assertIn("view-history", tick)
...
class TestSseDoorbellWire(unittest.TestCase):
    ...
    def test_viewer_connect_wakes_watcher_through_real_handler(self):  # noqa: VACUOUS_ASSERTION — the event:chat delivery through the real Handler._sse connect path is the positive control
        ...
        with mock.patch.object(web, "_SSE_IDLE_WATCH_S", 2.0), \
            ...
            # 0.10 IS TIGHT ON PURPOSE AND I BRIEFLY BROKE IT. I retuned this
            # to _SSE_IDLE_WATCH_S / 3, reasoning the cadence was the only
            # ceiling. TWO THINGS WERE WRONG. The socket above is 0.50
            # ("< idle cadence — timeout proves stale"), so the read loop
            # decides failure first and the derived bound was unreachable —
            # a live discrimination swapped for a dead one. And this method
            # PATCHES _SSE_IDLE_WATCH_S TO 2.0, so the derived bound was
            # 0.667s, not the 3.33s I claimed when I described the change:
            # I read the module constant while the test was running on a
            # mocked one. Conclusion right, arithmetic invented.
            # This arm was never among those that reddened; I retuned it on
            # the size of its number, without tracing what bounds it.
...
class UpstreamStarvationReachesTheRosterTest(unittest.TestCase):
    ...
    it. Measured 2026-08-02 against raw proxy log tails: ds4pro RATE-LIMITED,
    ...
    def test_the_join_is_WIRED_into_the_cached_roster_not_merely_defined(self):
        ...
        # THE REPORT IS HANDED THE SAME RECORD THE JOIN READS. The rebuild
        # acquires the availability snapshot ONCE and passes it to the report,
        # so the double records what it received and this arm reads it: two
        # producers writing one row off two acquisitions is the defect the
        # single hand-down exists to remove, and a double that merely tolerated
        # the argument would witness nothing about it.
        handed = []

        def report(room, availability=None):
            handed.append(availability)
            return fake

        with mock.patch.object(web, "_ROSTER_REP_TTL", 0), \
             mock.patch("helm.seats.roster_report", report), \
        ...
        self.assertIsInstance(handed[0], dict, "the report was handed no "
                                               "availability record at all")
        starved = fake["seats"][0]["seat"]      # this arm's own fixture, never
        self.assertEqual("RATE-LIMITED",        # a second spelling of the name
                         handed[0]["upstream"][starved]["state"],
                         "the report was handed the planted record the join "
                         "then published, or the row has two acquisitions")

    # ── #230: the wall verdict and the reset horizon are the RECORD's ──────
    # The owner learned about gemini's 429 daily-quota wall from pane errors,
    ...
    # non-healthy blip cannot say "this seat is refused, not idle" (#163:
    # a wall is a fact, not unreadiness). Both fields are PASS-THROUGH from
    ...
    def test_a_walled_family_carries_the_records_own_dark_verdict(self):
        ...
        self.assertEqual(row["upstream_restart"], "UNKNOWN")
        self.assertNotIn("upstream_remediation_action", row)

    def test_a_due_local_cooldown_carries_the_derived_action(self):
        from helm import proxywatch
        row = self._row("codex", {"codex": {
            "state": proxywatch._PROXY_COOLDOWN, "dark": True,
            "falsification_bar_s": 900, "seats": {"codex": {
                "state": proxywatch._PROXY_COOLDOWN, "dark": True,
                "falsification_due": True, "falsification_age_s": 901,
                "falsification_seat": "codex"}}}}, family="codex")
        self.assertEqual(row["upstream_restart"], "HELPFUL")
        self.assertEqual(row["upstream_restart_target"], "proxy")
        self.assertIn("PRESCRIBES: restart this exact proxy",
                      row["upstream_remediation_action"])

    def test_a_fresh_local_cooldown_never_carries_restart_advice(self):
        from helm import proxywatch
        row = self._row("codex", {"codex": {
            "state": proxywatch._PROXY_COOLDOWN, "dark": True,
            "falsification_bar_s": 900, "seats": {"codex": {
                "state": proxywatch._PROXY_COOLDOWN, "dark": True,
                "falsification_due": False, "falsification_age_s": 899,
                "falsification_seat": "codex"}}}}, family="codex")
        self.assertEqual(row["upstream_restart"], "UNKNOWN")
        self.assertNotIn("upstream_remediation_action", row)
    ...
    def test_a_recorded_reset_horizon_reaches_the_row_as_epoch_ms(self):
        ...
        the kimi standing-config (#45) will — so this pins the SEAM: the day
    ...
    def test_a_non_finite_horizon_is_dropped_and_the_seat_keeps_its_wall(self):
        """The FIX round, blocker 1, parse half. float("nan") passes an
        ...
        exactly the silence #230 closed."""
    ...
    def test_a_corrupt_dark_value_maps_to_None_never_True(self):
        """The FIX round, blocker 2, server half. bool("false") is True
    ...
    def test_a_poisoned_record_degrades_one_seat_never_the_payload(self):
        """The FIX round, blocker 1, endpoint half. The general law
        ...
        # the degrade is DOWNSTREAM of one acquisition: the report still got
        # the record, so a fail-open field is a join verdict about this seat
        # and never a missing read for the whole render
        self.assertEqual(1, len(handed))
        self.assertIsInstance(handed[0], dict, "the report was handed no "
                                               "availability record at all")
        healthy_join = fake["seats"][1]["seat"]   # the sibling that annotates
        self.assertEqual("RATE-LIMITED",          # normally, off the fixture
                         handed[0]["upstream"][healthy_join]["state"])
