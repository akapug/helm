...
// THE SERVER'S SENTENCE IS ONE TIER OF THREE. post() renders `url -> status:
// error`, so an EXACT comparison against FORBIDDEN alone pins the one-tier
// shape these arms were written under and goes red the moment the path and
// the status are added -- which is exactly what happened (task/2650). What
// these scenarios are about is WHICH failure surfaced and whether a retry
// ran, never how the sentence is drawn, so they ask for all three tiers and
// stop asserting the whole string.
function msgHas(m, url, status, sentence) {
  return typeof m === "string" && m.indexOf(sentence) !== -1
    && m.indexOf(String(status)) !== -1 && m.indexOf(url) !== -1;
}
function forbiddenMsg(m, url) { return msgHas(m, url, 403, FORBIDDEN); }
...
async function scenario_stale_bearer_first_post_recovers() {
  ...
  const r = await threw(post("/api/chat", {room: "main", text: "hi", name: "daria"}));
...
async function scenario_next_post_rides_the_adopted_token() {
  ...
  const r = await threw(post("/api/chat", {room: "main", text: "again", name: "daria"}));
...
async function scenario_genuine_forbidden_still_loud() {
  ...
  return {name: "genuine_forbidden_still_loud",
    pass: r.threw && forbiddenMsg(r.message, "/api/chat")
...
async function scenario_refresh_down_original_403_surfaces() {
  ...
  return {name: "refresh_down_original_403_surfaces",
    pass: r.threw && forbiddenMsg(r.message, "/api/chat")
      && SERVER.posts.length === 1 && TOKEN === "T1",
...
async function scenario_non_403_never_refreshes() {
  ...
  return {name: "non_403_never_refreshes",
    pass: r.threw && msgHas(r.message, "/api/chat", 400,
                            "body wants a JSON object")
...
async function scenario_unsubstituted_page_never_adopts_the_marker() {
  ...
  return {name: "unsubstituted_page_never_adopts_the_marker",
    pass: r.threw && forbiddenMsg(r.message, "/api/chat")
      && TOKEN === "T1" && SERVER.posts.length === 1,
...
async function scenario_homepost_error_object_contract() {
  ...
  const badOk = !bad.threw && bad.value
    && forbiddenMsg(bad.value.error, "/api/homes");
