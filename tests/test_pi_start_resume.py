...
class PiCmdTest(unittest.TestCase):
    ...
    def test_run_pins_model_registers_real_roster_row_and_execs_key_in_env(self):  # noqa: VACUOUS_ASSERTION — exact argv/env/roster tuples are positive controls; the key's argv absence is the secrecy contract
        ...
        with tempfile.TemporaryDirectory(prefix="helm-test-pi-run-") as d, \
                ...
                mock.patch.object(pi.seat_launch_owner, "exec_attached",
    ...
    def test_run_routes_the_exact_child_through_one_owner(self):
        with mock.patch.object(pi, "pi_binary", return_value="/usr/bin/pi"), \
                mock.patch.object(pi, "_pi_api_key", return_value="secret-key"), \
                mock.patch.object(pi, "provider_name", return_value="helm-seat-a"), \
                mock.patch.object(pi, "_proxy_descriptor", return_value=({
                    "seat": "pi-seat-a", "model": "helm-seat-a/model-a",
                    "runtime": {"agent_harness": "pi", "family": "seat-a",
                                "backend": "proxy"}}, None)), \
                mock.patch("helm.seats.write_roster"), \
                mock.patch.object(pi.seat_launch_owner, "exec_attached",
                                  return_value=23) as owner:
            rc = pi.pi_run("seat-a", ["--print", "probe"])
    ...
    def test_run_refuses_a_model_from_another_provider_before_reading_key(self):  # noqa: VACUOUS_ASSERTION — the emitted provider error and status 2 are positive controls; the three untouched boundaries prove refusal ordering
        ...
        with mock.patch.object(pi, "pi_binary", return_value="/usr/bin/pi"), \
                ...
                mock.patch.object(pi.seat_launch_owner, "exec_attached") as execvpe, \
                mock.patch.object(sys, "stderr", new=err):
            rc = pi.pi_run("codex", ["--model", "openrouter/x"])
        self.assertEqual(rc, 2)
        self.assertIn("--model must use helm-codex", err.getvalue())
