...
class TestIdentity(MeldBase):
    def test_self_seat_is_env_first_like_whoname(self):
        """The identity trap: roster-first _self_seat
...
class TestStandupMultiParty(MeldBase):
    """standup = the informal 2+ species (owner canon): the pinned PAIR
    generalized to a pinned SET. The set is the anti-hijack allowlist —
    it admits every member and refuses every outsider (the hijack
    ...
    def test_pair_floor_wording_is_unchanged(self):
        ...
        there and must not drift (the 2+ change leaves the pair untouched)."""
    ...
    def test_multiparty_floor_is_honest_not_exclusive(self):
        """In a 2+ standup the floor is NOT exclusive — any member
    ...
    def test_still_to_hear_drops_a_member_who_already_spoke(self):
        """A re-review found: the set tracked DONE only, so a member who had
...
class TestLifecycleJournal(MeldBase):
    ...
    def test_flush_wipe_replay_restores_state_and_room_history(self):
        ...
        # room CONTENT restores too — a live reboot proved the
...
class ReplayedStatusRenderTest(MeldBase):
    """meld-replayed-reads-as-active (P0-misleading). A REPLAYED meld
    ...
    def test_answering_a_ghost_marks_it_REOPENED_never_plain_active(self):
        """A live ghost's exact shape, HIGH: one live say into a
    ...
    def test_a_malformed_room_epoch_renders_no_age_not_55_years(self):
        """The room-name epoch is a CLAIM, and a garbage one is
        not a clock. meld-1-name must render silence on the age suffix, never
        age=20667d; a plausible epoch still resolves. The validator later moved
...
class RoomOwnedProvenanceTest(MeldBase):
    ...
    def test_age_comes_from_the_event_stream_only(self):
        ...
        self.assertEqual(ep, epoch)              # the stream's epoch, not the row's
        delta = int(time.time()) - ep
        self.assertLess(delta, 120)              # fresh, not 200+ days
        out = "\n".join(meld.status(seat="seat-a"))
        self.assertIn("age=", out)
        self.assertNotIn("age=2", out.split("age=")[1][:3])  # not 200d-scale

    def test_the_stream_outvotes_the_name_epoch(self):
        """A journaled room whose NAME epoch lies (stamped a year back, but
        ...
        self.assertEqual(meld._last_activity_epoch(renamed),
                         epoch)                              # the stream WINS
...
class CouncilJudgesANamedSubjectTest(unittest.TestCase):
    ...
    codex-3 made the tip required and named the hazard exactly: an optional tip
