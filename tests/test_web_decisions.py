...
ENV_KEYS = ("HELM_HOME", "MELD_HOME", "HELM_CHAT_DIR", "MELD_CHAT_DIR",
            ...
            # WHO IS WRITING is read from these on the library path
            # (task/2997): an inherited harness stamp would make this suite's
            # direct ownerasks.decide calls run as an agent, and an inherited
            # actor-store override would point it at a real identity file.
            "HELM_ACTORS", "MELD_ACTORS", "PI_CODING_AGENT", "CLAUDECODE",
...
class TestWebDecisions(unittest.TestCase):
    ...
    def test_delivered_cards_leave_the_queue(self):
        ownerasks.decide(self.rid, "1", by=ownerasks.owner_door("web"))
    ...
    def test_lost_dm_returns_to_the_queue_and_web_retry_recovers(self):  # noqa: VACUOUS_ASSERTION — the final empty-entries absence has its unconditional positive control earlier in this SAME test: the same GET returned the dm_lost entry before the recovery POST
        """Finding [1000]: a reboot-eaten DM must be DISCOVERABLE in
        ...
        ownerasks.decide(self.rid, "2", by=ownerasks.owner_door("web"))
    ...
    def test_verdict_closes_and_DMs_the_asker(self):
        ...
        self.assertEqual(lane[0]["from"], "daria")   # the OWNER's word
    ...
    def test_the_browser_door_records_the_owner_BY_DESIGN(self):
        """task/2997. These POSTs are the owner's browser door, so they
        record "owner" BY DESIGN, with the door beside it, even when the
        server process itself carries a seat identity (`helm web` launched
        from a seat's pane). The bearer they demand is CSRF protection and
        proves a browser on loopback, NOT the owner: every agent runs as his
        uid. What stops an agent posting here is the argv-guard's owner-door
        rung (tests/test_chat_argv_guard.py, OwnerDoorPostGuardTest); a
        deliberate same-uid bypass is outside the threat model, as the
        ownerasks module docstring states."""
        sid = "sid-builder-2"
        seats.write_roster("builder-2", session=sid, cwd=self.tmp)
        with mock.patch.dict(os.environ, {"HELM_CHAT_NAME": "builder-2",
                                          "CLAUDE_CODE_SESSION_ID": sid}):
            status, d = self.req("/api/decisions/comment",
                                 {"id": self.rid, "text": "rebase cost?"})
            vstatus, vd = self.req("/api/decisions/verdict",
                                   {"id": self.rid, "choice": "2"})
        self.assertEqual(status, 200, d)
        self.assertEqual(vstatus, 200, vd)
        card = ownerasks.decision_rows()[self.rid]
        self.assertEqual((card["comments"][-1]["by"],
                          card["comments"][-1]["door"]), ("owner", "web"))
        self.assertEqual((card["verdict"]["by"], card["verdict"]["door"]),
                         ("owner", "web"))
        lane = self.dm_lane("builder-9")
    ...
    def test_ui_decision_markup_and_js(self):
        ...
        for marker in ('id="odq"', 'id="odqlist"', 'id="odqmeta"',
                       'id="odqReload"', 'class="odqcard', 'id="onyouline"'):
    ...
    def test_ui_queue_renders_first_on_the_work_page(self):  # noqa: VACUOUS_ASSERTION — body.index() RAISES on any missing marker, so every marker is asserted present before the order comparison can run
        ...
        self.assertIn('"work"', body.split("const VIEWS")[1][:120],
                      "the VIEWS array does not route the Work page")
        # no link off the page to a tab that is this page
        onyou = body[body.index('id="onyou"'):body.index('id="board"')]
        self.assertNotIn('href="#work"', onyou)
