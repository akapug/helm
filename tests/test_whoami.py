#!/usr/bin/env python3
"""whoami tests — hermetic: tempfile + HELM_HOME/HELM_PROFILE_HOME env overrides;
never touches the real ~/.helm, an external profile home, or ~/.claude."""
import contextlib
import io
import os
import sys
...
class WhoamiBase(unittest.TestCase):
    def setUp(self):
        ...
        self.scaffold_dir = os.path.join(self.tmp.name, "profile-home")
        self.envp = mock.patch.dict(os.environ, {
            "HELM_HOME": os.path.join(self.tmp.name, "helm-home"),
            "HELM_PROFILE_HOME": self.scaffold_dir,
        })
        self.envp.start()
...
class TestMergeScaffold(WhoamiBase):
    def test_imports_scaffold_content_and_bookkeeping(self):
        path = self.write_scaffold({"schema_version": 1, "technical_level": "expert",
                             ...
                             path=os.path.join(self.tmp.name, "fake-scaffold", "profile.json"))
...
class TestInterviewConfirm(WhoamiBase):
    ...
    POP = {"schema_version": 2, "technical_level": "expert founder-operator",
           ...
           "source": "derived-from-an-older-corpus 2026-07-19"}
...
class ANoteDescriptionSaysWhenItIsShortTest(WhoamiBase):
    """The third writer of this shape. The note's whole text is the body
    below, so the description is a glance and now says when it is one."""

    def desc(self, text):
        name = whoami.add_note(text, topic="voice")
        path = os.path.join(whoami.notes_dir(), name + ".md")
        return pk.parse_simple_frontmatter(
            path, {"name": "", "description": ""})["description"], path

    def test_a_long_lead_line_is_cut_in_the_description_and_says_so(self):
        # `add_note` strips, so the fixture must be what actually lands in
        # the body — a trailing space made this arm assert about a string the
        # writer never wrote.
        text = ("he wants the number beside the claim " * 10).strip()
        desc, path = self.desc(text)
        self.assertIn("[cut: 120 of ", desc)
        with open(path, encoding="utf-8") as fh:
            self.assertIn(text, fh.read())   # the whole note is the body

    def test_a_short_lead_line_carries_no_mark(self):
        desc, _path = self.desc("crisp is kind")
        self.assertEqual(desc, "crisp is kind")
