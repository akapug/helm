...
class AlarmTest(MixBase):
    ...
    def test_an_EQUAL_COUNT_TIE_breaks_by_name_deterministically(self):
        """THE MUTATION THAT SURVIVED, found in review. The sort is
    ...
    def test_the_CLI_indents_EVERY_line_not_just_the_first(self):
        """ALSO FROM REVIEW. `cmd_mix` did `print("\\n  " + alarm)`, which
        ...
        # EXACT PREFIXES, NOT "starts with two spaces". A review caught the
...
class UsageAdvertisesEveryVerbTest(unittest.TestCase):
    ...
    was caught by a different accident: the stdin body (review), --kind
    ...
    def test_every_routed_verb_is_named_in_usage(self):
        import inspect
        src = inspect.getsource(D._cmd_dispatch)
        routed = set(re.findall(r'verb == "([a-z-]+)"', src))
        routed |= set(re.findall(r'verb in \(([^)]*)\)', src)
                      and re.findall(r'"([a-z-]+)"',
                                     " ".join(re.findall(r'verb in \(([^)]*)\)', src))))
        # The READ verbs are also named at the door, in their own set, so a
        # verb routed only there would escape a scan of the body alone.
        routed |= set(D.DISPATCH_READ_VERBS)
        self.assertTrue(routed, "no verbs found — the scan itself broke")
        # ASSERTED POSITIVELY, against a NON-EMPTY observable: the verbs USAGE
        # names must be ALL of them. The empty-`missing` form proves the same
        # property only while the scan is working, and a scan that silently
        # found nothing renders identically to a fully advertised table.
        advertised = sorted(v for v in routed if v in D.USAGE)
        self.assertEqual(advertised, sorted(routed),
                         "verbs routed but absent from USAGE: %s"
                         % sorted(set(routed) - set(advertised)))
...
class RawEventsAreUntrustedTest(MixBase):
    ...
    A re-gate: "raw events unvalidated". Both cases below produced a
    ...
    def fresh_home(self):
        """An empty ledger for ONE case inside a test: a scope, never a second
        setUp. Re-running setUp re-saves `self.prior` from the environment
        this fixture has already rewritten, so tearDown writes a temporary
        HELM_HOME back as the original and every later test in the process
        reads and writes a stray home."""
        tmp = tempfile.mkdtemp(prefix="helm-test-mix-case-")
        try:
    ...
    def test_rows_that_pass_identity_but_FAIL_GENESIS_are_not_counted(self):
        ...
        # POSITIVE CONTROL, UNCONDITIONAL and ahead of the loop, in the same
        # scope the cases use: the same number of CANONICAL rows is counted by
        # both readers and does raise the alarm. Without it, an empty result
        # below could mean a scope whose ledger no reader looks at.
        n = D.MIX_NO_BUILD_ALARM + 1
        with self.fresh_home():
            for _ in range(n):
                self.plant_raw(self.genesis())
            self.assertEqual(len(D.snapshot()[0]), n)
            self.assertEqual(sum(v["total"] for v in D.mix(hours=8)[0].values()),
                             n)
            self.assertIsNotNone(D.mix_alarm(hours=8)[0])
        for bad in ({"tip": "not-a-tip"}, {"status": "cancelled"},
                    {"seq": 3}, {"v": 99}):
            with self.subTest(**bad), self.fresh_home():
                for _ in range(n):
...
class OwnerLayerWiringTest(unittest.TestCase):
    """The three wiring defects a re-gate named. Asserted against the
    ...
    def test_cmd_dispatch_passes_kind_to_add(self):
        ...
        import inspect
        src = inspect.getsource(D._cmd_dispatch)
        # Spelled through the ledger module since the verb table moved
        # to its satellite; the call is the subject, not its prefix.
        call = src[src.index("row, why = dispatches.add("):]
