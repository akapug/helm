...
class ShapeTest(unittest.TestCase):
    ...
    def test_an_identity_env_var_FIRES_but_a_config_env_var_does_not(self):
        ...
        USER/HOST are not — DATABASE_HOST must NOT fire (codex-3's blocker 4)."""
...
class ScanWiringTest(unittest.TestCase):
    def test_the_must_hit_control_is_wired_into_scan(self):
        ...
        # probe of the constant — codex-3's blocker-3 requirement
...
class _StagedRepoMixin(unittest.TestCase):
    ...
    codex-3's round-3 verdict named exactly this: the committed tests fed
...
class BlockerReproTest(_StagedRepoMixin):
    """codex-3's four truth blockers, each pinned against the REAL pipeline."""
    ...
    def test_unrelated_home_source_and_hidden_join_do_NOT_co_fire(self):  # noqa: VACUOUS_ASSERTION — absence IS the blocker; the positive one-hunk control is asserted below
        """Blocker 1 (codex-3's exact fixture): ONE tracked file, 120 baseline
    ...
    def test_a_docstring_example_is_NOT_a_hardcode(self):  # noqa: VACUOUS_ASSERTION — absence IS the blocker; positive arms (real assignments fire) asserted inline below
        """Blocker 2 (codex-3's exact fixture): prose is classified before ALL
    ...
    def test_codex3_round4_prose_spans_not_lines(self):
        """codex-3's round-4 EXACT repros, pinned verbatim: prose is a lexical
    ...
    def test_codex3_round5_no_override_is_measured(self):
        """codex-3's round-5 bar: the warning asserts "with no override", so
    ...
    def test_codex3_round6_override_is_relational_not_existential(self):
        """codex-3's round-6 bar: measured EXISTENCE of an override is not
        ...
        control payloads (codex-3's exact probes) must FIRE; both real
        override fixtures (round 5) must stay SILENT. Mutation: revert to a
        hunk-global `_OVERRIDE_FALLBACK.search(body)` guard -> the two FIRE
        payloads go silent (the false negative returns)."""
        # codex-3's exact round-6 probes: an unrelated override anywhere in
        ...
        # and the related overrides (round 5) still stay SILENT — the
    ...
    def test_the_must_hit_control_traverses_real_extraction(self):
        """Blocker 3 (codex-3's exact fixture): ONE raw-diff->added-blocks
