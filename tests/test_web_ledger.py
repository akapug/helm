...
from helm import chat, pk, web, web_ui_loader  # noqa: E402
...
def _reply(h, body):
    """404 for None, else the JSON — shared by every mock-node handler."""
    if body is None:
        h.send_response(404)
...
class EmptyAnchorNode(BaseHTTPRequestHandler):
    """A live incident's shape, generalized: a healthy node with real
...
class TestWebLedgerSplitBrain(LedgerBase):
    ...
    default). In a live incident the ledger tab read the anchor node while the
    ...
    def test_turn_status_proxies_the_same_node(self):
        ...
        status, d = self.req("/api/ledger/turn?hash=" + HEAD_HASH)
        self.assertEqual(status, 200)
        self.assertEqual(d.get("consensus_final"), True)


class LedgerEmptyStateMirror(unittest.TestCase):
    """Python mirror of the assembled web UI's ledgerEmptyClass() — the empty-state
    classifier that keeps an empty signed-entries feed honest. Three empties,
    because they answer three different owner questions: mismatch = "entries
    exist but this read missed them" (the wrong-node incident),
    fresh = "zero is normal, and here is what creates one", off = "zero
    because nothing is being created while signing is off"."""

    def _classify(self, signed, head, nturns):
        if nturns > 0:
            return "rows"
        if signed and head is not None:
            return "mismatch"
        if signed:
            return "fresh"
        return "off"
    ...
    def test_the_states(self):
        for signed, head, nturns, want in (
                (True, 33, 3, "rows"),
                (False, None, 1, "rows"),
                (True, 33, 0, "mismatch"),  # the incident: head #33, read none
                (True, 0, 0, "mismatch"),   # head #0 IS one entry, not zero
                (True, None, 0, "fresh"),
                (False, None, 0, "off"),
                (False, 12, 0, "off"),      # unsigned wins: nothing being made
        ):
            self.assertEqual(self._classify(signed, head, nturns), want,
                             (signed, head, nturns))

    def test_the_JS_classifier_matches_this_test(self):
        """A python mirror of JS logic rots silently — pin the actual source
        text (the test_web.RoomTypeTest pattern)."""
        src = web_ui_loader.read_text()
        self.assertIn("function ledgerEmptyClass(signed, head, nturns)", src)
        self.assertIn('if (nturns > 0) return "rows"', src)
        self.assertIn('if (signed && head != null) return "mismatch"', src)
        self.assertIn('if (signed) return "fresh"', src)
        self.assertIn('return "off"', src)

    def test_the_cards_lead_with_plain_language(self):
        """The owner's acceptance test: "i dont really know what
...
class LedgerSurfaceAgreementTest(unittest.TestCase):
    ...
    Caught live by a cross-family review of the tip: the pulse tested
...
for (const k of Object.keys(cases)) {
  ...
  out[k] = {badge: ledgerBadges(t),
            pulse: cavePulse([t], t.chain_index, cases[k].now)};
}
...
class LedgerBadgeRuntimeTest(unittest.TestCase):
    ...
    The badge gated on t.consensus_final, a field only dregg PR #54 would
...
class SigningPulseRuntimeTest(unittest.TestCase):
    ...
    CardRuntimeTest pattern from test_web_lr.py): the assertion is about the
    words the owner sees, not about the function not throwing. The pulse is
    pure (turns + head + now in, string out) precisely so this test owns the
    clock. Skipped, not failed, where node is unavailable."""
...
const out = {};
for (const k of Object.keys(cases))
  out[k] = cavePulse(cases[k].turns, cases[k].head, cases[k].now);
process.stdout.write(JSON.stringify(out));
""")
        chk = subprocess.run([cls.node, "--check", cls.path],
                             capture_output=True, text=True)
    ...
    def test_a_solo_read_never_promises_a_wait_that_cannot_complete(self):
        """Every entry from a solo node was counted "awaiting finality"
    ...
    def test_certificate_absence_decides_before_any_finality_word(self):
        """The row a cross-family review caught: certificate ABSENT but finality
