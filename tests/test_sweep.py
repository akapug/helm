...
class ProposeTest(SweepBase):
    def test_same_slug_supersedes_across_a_supersedes_edge(self):
        self._registry(_proj("oldtool"), _proj("helm", [_edge("supersedes", "oldtool")]))
        self._prior("oldtool-rollover-policy", "on rollover re-home the oldtool session")
        self._prior("helm-rollover-policy", "on rollover helm re-homes the session")
        props = sweep.propose()
        self.assertEqual(len(props), 1)
        p = props[0]
        self.assertEqual((p["old_id"], p["new_id"]), ("oldtool-rollover-policy",
                                                      "helm-rollover-policy"))
        self.assertEqual(p["signal"], "same-slug")
        self.assertEqual((p["successor"], p["rel"], p["ancestor"]),
                         ("helm", "supersedes", "oldtool"))
    ...
    def test_descends_from_edge_also_drives_the_sweep(self):
        self._registry(_proj("oldtool"), _proj("helm", [_edge("descends-from", "oldtool")]))
        self._prior("oldtool-token-refresh", "refresh the oldtool token")
        self._prior("helm-token-refresh", "helm refreshes the token")
        props = sweep.propose()
        self.assertEqual([p["old_id"] for p in props], ["oldtool-token-refresh"])
    ...
    def test_high_overlap_pairs_when_base_differs(self):
        ...
        self._registry(_proj("oldtool"), _proj("helm", [_edge("supersedes", "oldtool")]))
        self._prior("oldtool-token-refresh", "swap when the account runs dry", kw)
    ...
    def test_reference_type_is_supersedable_too(self):
        self._registry(_proj("oldtool"), _proj("helm", [_edge("supersedes", "oldtool")]))
        self._reference("oldtool-doc-link", "the oldtool handbook")
    ...
    def test_same_slug_ranks_before_high_overlap(self):
        kw = "rollover,account,freshest,headroom,session"
        self._registry(_proj("oldtool"), _proj("helm", [_edge("supersedes", "oldtool")]))
        self._prior("oldtool-rollover-policy", "oldtool rollover", "rollover,policy")
        self._prior("helm-rollover-policy", "helm rollover", "rollover,policy")
        self._prior("oldtool-token-refresh", "swap when dry", kw)
...
class WeakSignalTest(SweepBase):
    def test_no_edge_no_proposal_even_with_same_slug(self):
        self._registry(_proj("oldtool"), _proj("helm"))   # NO succession edge
        self._prior("oldtool-rollover-policy", "oldtool rollover")
    ...
    def test_term_mention_without_a_twin_never_fires(self):
        self._registry(_proj("oldtool"), _proj("helm", [_edge("supersedes", "oldtool")]))
        self._prior("oldtool-only-note", "a lone oldtool-era note, no successor twin")
        self._prior("helm-unrelated-thing", "a totally different helm topic")
        self.assertEqual(sweep.propose(), [])

    def test_below_overlap_threshold_does_not_fire(self):
        self._registry(_proj("oldtool"), _proj("helm", [_edge("supersedes", "oldtool")]))
        self._prior("oldtool-widget", "one", "apple,banana,cherry,date,elder")
    ...
    def test_entry_tagged_with_both_eras_is_excluded(self):
        ...
        self._prior("helm-uses-oldtool-bridge", "helm still reads the oldtool bridge")
        self._prior("helm-uses-x-bridge", "helm reads the x bridge")
        props = sweep.propose()
        self.assertNotIn("helm-uses-oldtool-bridge", [p["old_id"] for p in props])
...
class ApplyTest(SweepBase):
    ...
    def test_apply_tombstones_old_keeps_file_and_is_idempotent(self):
        ...
        old = self._by_slug("oldtool-rollover-policy")
        ...
        self.assertTrue(os.path.isfile(
            os.path.join(self.adopted, "prior-oldtool-rollover-policy.md")))  # never deleted
    ...
    def test_cmd_apply_reports_receipts(self):
        ...
        self.assertIn("oldtool-rollover-policy -> helm-rollover-policy", out)
