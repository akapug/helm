...
from helm import seats, seats_gc
...
class FacadeCompletenessTest(unittest.TestCase):
    def test_every_name_a_caller_reaches_for_resolves_on_seats(self):
        ...
        # KNOWN OFFENDERS, pinned rather than ignored, and the staleness
        # asserts below are what keep the set honest: a pin whose caller
        # disappears or whose name starts resolving FAILS loudly and demands
        # deletion, so the list cannot become the place findings go to be
        # forgotten. Its one historical entry worked exactly as designed:
        # is_rostered_name (called at work/_gc.py:241, a name that never
        # existed anywhere) was pinned here, cured on lane
        # gc-triage-rostered-check-never-ran, and this assert forced the pin
        # out in the same landing.
        KNOWN = set()
        self.assertTrue(
            KNOWN <= set(callers),
    ...
    def test_the_facade_only_ever_shrinks(self):
        ...
        # ONE BUDGET, TWO READERS. The numbers live in the RUNG
        # (helm/splitbudget.py) and are imported here, so the commit-time
        # check and this backstop cannot drift into disagreeing about what
        # the budget is — the shape docref_guard established for its citation
        # registry. Raising either one raises both, deliberately and in one
        # place.
        from helm import splitbudget
        CEILING = splitbudget.CEILING   # the facade's ratchet; seats.py is 772
        FINISH = splitbudget.FINISH     # an EXTRACTED module's budget
...
# flagging a bound module on 3.14 (false positives, every case
...
# A probe measured that claim false: nine exact old repros still
...
class ExtractedModuleHealthTest(unittest.TestCase):
    ...
    def test_no_extracted_module_references_an_undefined_global(self):  # noqa: VACUOUS_ASSERTION — absence observable; the probe-table test above proves the same checker flags every known defect class, and checked>=3 proves this scan visited the modules
        ...
        helm/work/_gc.py called seats.is_rostered_name, a name that never
        existed, inside `except Exception`, and the whole suite passed 9271
        tests around it until the gc-triage cure replaced the call. Only a
        scan that asks the resolution question directly can see this class
        before a production path dies."""
        checked, bad = 0, {}
        for fn in sorted(os.listdir(HELM)):
...
class OldDefectRuntimeWitnessTest(unittest.TestCase):
    ...
    model. A probe measured the gap this closes: the deletion commit
    ...
    witness. The declared floor is 3.9 (scripts/install.sh, README) and the
    gate box runs 3.14; every arm below was measured on all six interpreters
    from 3.9 through 3.14 before it was written down."""
    ...
    def test_future_annotations_are_strings_and_the_checker_knows_from_310(self):
        ...
        for src in (modvar, fnann):
            ...
            if sys.version_info >= (3, 14) and src is modvar:
                ...
                # stringized (a review's case, measured: 3.14 tracks which
    ...
    def test_a_hand_written_load_of_the_synthetic_cell_is_flagged_and_raises(self):
        """THE MUST-HIT — the round-1 repro, kept red, now carrying
        ...
        contains the load — a synthetic top entry won the
    ...
    def test_every_shape_the_excusal_lied_about_is_screened_by_the_ban(self):
        """The round-2 false-positive table, rendered impossible
...
class FacadeAndSiblingsAgreeTest(unittest.TestCase):
    ...
    MEASURED after the split: owner_name() returned the owner's own name while
    ...
    def test_the_owner_name_pin_is_one_cell_not_two(self):
        ...
        A version of this test was itself a leak: it wrote
...
class FacadeMachineryTest(unittest.TestCase):
    ...
    serve. Both arms are findings from review, reproduced before they were
    ...
    def test_the_MOVED_mute_owner_is_reached_by_the_fanout(self):  # noqa: VACUOUS_ASSERTION — the flagged absence is the restore assertion; its unconditional positive control is the sentinel's own value asserted inside the patch, which an unregistered sibling could never return
        """AN EXPORT IS NOT A REGISTRATION, AND ONLY ONE OF THEM IS CHECKED.

        The facade can re-export a moved function from a new sibling and
        still not list that sibling in the fanout's module set — the names
        resolve, every import works, the exports-completeness arms pass, and
        a patch of `seats.roster` never reaches the moved owner's globals.
        So a caller that has always patched the facade to feed a fixture
        silently feeds the REAL roster to the one function that moved.

        THE OBSERVABLE IS THE SENTINEL'S OWN VALUE, so this cannot pass on a
        seat that happens to have no mutes: an unregistered sibling answers
        from the real roster and can never return this list.
        """
        from helm import seats_incarnation
        self.assertIn("seats_incarnation", seats._IMPL_MODULES,
                      "the extracted identity-generation owner is outside "
                      "the fanout, so a patch of its `roster` reaches it "
                      "and nothing else")
        from helm import seats_mute
        self.assertIn("seats_mute", seats._IMPL_MODULES,
                      "the moved owner is not in the fanout's module set")
        planted = {"planted-seat": {"mute": ["only-via-the-sentinel"]}}
        before = seats_mute.__dict__["roster"]
        with mock.patch.object(seats, "roster", lambda: planted):
            self.assertIsNot(seats_mute.__dict__["roster"], before,
                             "the fanout did not reach the moved owner")
            self.assertEqual(seats.mutes("planted-seat"),
                             ["only-via-the-sentinel"])
        # AND IT IS PUT BACK, which is the half a fanout gets wrong quietly.
        self.assertIs(seats_mute.__dict__["roster"], before,
                      "the restore left the moved owner patched")
    ...
    def test_a_DIVERGED_sibling_keeps_its_own_direct_patch(self):
        ...
        (measured by deleting them). The suite saw the CLASS and had
...
def _module_defined(tree):
    """Module-level DEFINITIONS only — what another file could take from here.

    Imports are excluded on purpose: two siblings importing `roster` from
    seats_common share a dependency, not a seam. Counting that as a
    cross-reference would make every module in the family look coupled to
    every other and the arm would mean nothing."""
    out = set()
    for n in tree.body:
        if isinstance(n, (ast.FunctionDef, ast.AsyncFunctionDef, ast.ClassDef)):
            out.add(n.name)
...
def _takes_from(src_a, src_b):
    """Names module A LOADS that only module B defines — A's debt to B."""
    a, b = ast.parse(src_a), ast.parse(src_b)
    theirs, mine = _module_defined(b), _module_bound(a)
    return {x.id for x in ast.walk(a)
            if isinstance(x, ast.Name) and isinstance(x.ctx, ast.Load)
            and x.id in theirs and x.id not in mine}


class ReportGcSeamTest(unittest.TestCase):
    """seats_gc came out of seats_report at 999 of the 1000-line budget.

    A split done for a line count is the kind that decays: the next author
    reaches back across the new file boundary, the import resolves through
    the facade, and within a few lanes the two files are one module wearing
    two names — with the budget pressure back and nothing to show for the
    move. These two arms are what make the seam a property instead of a
    historical accident.
    """

    def test_the_report_and_the_gc_share_no_module_level_name(self):
        report = _read(os.path.join(HELM, "seats_report.py"))
        gc = _read(os.path.join(HELM, "seats_gc.py"))

        # MUST-HIT FIRST: the detector has to be able to say yes. Absence is
        # the finding below, and an absence from a scanner that cannot find
        # anything is not evidence (this pair is the same shape as the real
        # one -- a def in B, a call to it from inside a function in A).
        self.assertEqual(
            _takes_from("def a():\n    return gc_roster()\n",
                        "def gc_roster():\n    return 1\n"),
            {"gc_roster"},
            "the cross-reference detector cannot see a reference it is "
            "pointed at -- every emptiness below is worthless")
        self.assertEqual(
            _takes_from("from x import gc_roster\ndef a():\n"
                        "    return gc_roster()\n",
                        "def gc_roster():\n    return 1\n"),
            set(),
            "an IMPORTED name is a shared dependency, not a seam crossing")

        self.assertEqual(
            _takes_from(gc, report), set(),
            "the GC reached back into the report half -- the seam these two "
            "files exist to keep is gone, and the budget that forced the "
            "split will be back with nothing to show for it")
        self.assertEqual(
            _takes_from(report, gc), set(),
            "the report half reached into the GC. A report is a READ: the "
            "auto-reap that once rode roster_report is exactly the defect "
            "this direction of the seam prevents")

    def test_the_moved_gc_is_reached_by_the_fanout(self):  # noqa: VACUOUS_ASSERTION — the unconditional positive control is the planted seat's own name asserted out of `rows` inside the patch, which the real roster can never produce; the absence assertions (no leftover name in seats_report, nothing pruned by a dry run) are read only after it
        """AN EXPORT IS NOT A REGISTRATION -- the same gap the mute owner's
        arm above pins, for the module that moved most recently.

        gc_roster resolves `roster` in its OWN globals. Re-exporting it from
        the facade makes every import work and every exports-completeness arm
        pass while a patch of `seats.roster` reaches nothing -- so a caller
        feeding a fixture roster to the GC would silently be feeding it the
        REAL one, and `apply=True` deletes rows off whatever it was given.

        THE OBSERVABLE IS THE PLANTED SEAT'S OWN NAME, which the real roster
        can never produce.
        """
        self.assertIn("seats_gc", seats._IMPL_MODULES,
                      "the moved GC is outside the fanout's module set")
        for name in ("REAP_S", "_unlink_seat_state", "_transcript_exists",
                     "_transcript_hit", "_COULD_HOST_SEAT",
                     "_live_process_evidence", "_gc_keep_reason",
                     "gc_roster"):
            self.assertIs(getattr(seats, name), getattr(seats_gc, name),
                          "%s does not reach the facade from its new home"
                          % name)
            self.assertFalse(
                hasattr(_seats_report(), name),
                "%s was left behind in seats_report as well -- two owners of "
                "one name is what the fanout's divergence guard then has to "
                "arbitrate" % name)

        planted = {"planted-gc-seat": {"last_seen": 1.0}}
        before = seats_gc.__dict__["roster"]
        with tempfile.TemporaryDirectory() as d:
            with mock.patch.object(seats, "roster", lambda *a, **k: planted):
                self.assertIsNot(seats_gc.__dict__["roster"], before,
                                 "the fanout did not reach the moved GC")
                rows, pruned = seats.gc_roster(apply=False, roots=[d],
                                               proc_dir=d, now=1000000.0)
            self.assertEqual([r["seat"] for r in rows], ["planted-gc-seat"],
                             "the GC answered from a roster the fixture did "
                             "not supply")
            self.assertEqual(pruned, [], "a dry run pruned something")
        # AND IT IS PUT BACK, which is the half a fanout gets wrong quietly.
        self.assertIs(seats_gc.__dict__["roster"], before,
                      "the restore left the moved GC patched")
