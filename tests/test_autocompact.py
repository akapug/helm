...
class FakeAdapter(harness._CLIAdapter):
    ...
    def send(self, handle, text, enter=True):
        ...
        if not enter:
            ...
            return
        placed = self.typed.pop(handle, None)
        compact = (text == "/compact" or placed == "/compact"
                   or (text == "" and placed is None))
        if not compact:
            return
...
class CountingAdapter(FakeAdapter):
    def __init__(self, panes, tail="❯ "):
        super().__init__(panes=panes, tail=tail)
        self.list_calls = 0
        self.read_calls = 0
...
class AutocompactBase(unittest.TestCase):
    """The autocompact fixture: a scratch HELM_HOME and chat dirs, the spawn
    and submit delays at zero, one codex seat spawned with its transcript
    project dir, and the helpers that plant seats and transcripts (`spawn`,
    `unbind_session`, `plant`, `preview_case`).
    ...
    def preview_case(self, previews, tail="❯ "):
        names = ("codex", "codex-2", "codex-3")[:len(previews)]
        panes, rows = [], []
        for i, (name, preview) in enumerate(zip(names, previews), 1):
            handle = "h%d" % i
            self.spawn(handle=handle, seat_name=name)
            pane = {"handle": handle, "title": name, "status": "connected"}
            if preview is not None:
                pane["preview"] = preview
            panes.append(pane)
            rows.append({"seat": name, "family": "codex",
                         "ctx_tokens": int(CODEX_WINDOW * 0.50),
                         "pct": 50.0, "source": "transcript",
                         "model": "gpt-5.3-codex", "session": SID,
                         "registered_session": SID,
                         "pane_handle": handle, "pane_harness": "fake",
                         "age_s": 1.0, "window": CODEX_WINDOW,
                         "window_src": "test", "headroom_tokens": 1,
                         "status": "ok", "pane_live": True})
        return names, rows, CountingAdapter(panes, tail=tail)


class AutocompactTest(AutocompactBase):
    """The autocompact arms, on AutocompactBase's fixture.
    ...
    class that only needs the fixture subclasses AutocompactBase."""
    ...
    def test_read_pct_from_transcript(self):
        ...
        self.assertEqual(row["pane_harness"], "fake")
    ...
    def test_a_ZERO_usage_record_is_skipped_not_reported_as_zero_context(self):
        ...
        Measured live: one seat read 0.0% of 360k against a 33-minute-old
        transcript of 2363 lines and 755 usage-bearing records, THIRTY-THREE
        MINUTES AFTER being rescued from 102%. Another read 0.0% while sitting
        at 115%. Both were wedged and invisible to the surface built to catch them;
    ...
    def test_window_pinned_from_family_max_context(self):
        # ds4pro declares max_context (1M, probe-backed) and no budget -> the
        # pinned window is the gauge, NOT CC's 200k default. 500k of a 1M
        # window reads 50%. (This arm read kimi until task/2944 gave kimi a
        # context_budget; kimi's own reading is the next arm.)
        proj = os.path.join(seat.seat_dir("ds4pro"), "claude", "projects", "-p")  # noqa: SEAT_NAME — the catalog FAMILY key whose window IS the subject of this arm (and "claude" is the config dir name)
        os.makedirs(proj, exist_ok=True)
        with open(os.path.join(proj, SID + ".jsonl"), "w") as f:
            f.write(usage_line(500000, model="ds4-pro") + "\n")
        row = autocompact.read("ds4pro")  # noqa: SEAT_NAME — the catalog FAMILY key whose window IS the subject of this arm
        self.assertEqual(row["window"], 1000000)
        self.assertEqual(row["window_src"], "FAMILIES.max_context")
        self.assertAlmostEqual(row["pct"], 50.0)

    def test_kimi_reads_against_its_context_budget(self):
        # task/2944: kimi's 1M window is TAUGHT as its budget, and the gauge
        # reads the same number the launch line teaches CC. Half the budget
        # reads 50%; against the 1M window it would read under 20%, and the
        # /compact at 80% would wait until 800k.
        budget = seat.FAMILIES["kimi"]["context_budget"]
        self.assertLess(budget, seat.FAMILIES["kimi"]["max_context"])
        proj = os.path.join(seat.seat_dir("kimi"), "claude", "projects", "-p")
        os.makedirs(proj, exist_ok=True)
        with open(os.path.join(proj, SID + ".jsonl"), "w") as f:
            f.write(usage_line(budget // 2, model="kimi-k3") + "\n")
    ...
    def clear_fallback(self, current):
        self.plant(180000)
        evidence = (
            "API Error: 400 prompt is too long: context exceeds maximum\n"
            "API Error: 400 prompt is too long: context exceeds maximum")
        ad = FakeAdapter(tail=evidence)
        ad.read = mock.Mock(side_effect=(evidence, current))
        with mock.patch.object(autocompact, "_prune_context",
                               return_value=(None, "cv prune failed: fixture", True)):
            got = autocompact.check(seats=["codex"], post=False, adapter=ad)
        self.assertEqual(ad.read.call_args_list, [
            mock.call("h1", limit=12000), mock.call("h1", limit=2000)])
        return got, ad

    def test_repeated_400_with_held_composer_refuses_clear(self):
        current = (
            "API Error: 400 prompt is too long: context exceeds maximum\n"
            "❯ Continue with the OPEN Codex dispatch.")
        got, ad = self.clear_fallback(current)
    ...
    def test_repeated_400_with_pending_clear_keeps_pending_behavior(self):  # noqa: VACUOUS_ASSERTION — exact legacy mode/detail and zero fresh reads positively prove the pending-command fast path before the prohibited empty send
        tail = (
            "API Error: 400 prompt is too long: context exceeds maximum\n"
            "API Error: 400 prompt is too long: context exceeds maximum\n"
            "❯ /clear")
        ad = FakeAdapter(tail=tail)
        ad.read = mock.Mock(side_effect=AssertionError(
            "pending /clear must return before a fresh composer read"))
        mode, detail = autocompact._fire_clear(
            "codex", ad, "h1", "identity proven", tail)
        self.assertEqual(mode, "clear-pending")
        self.assertEqual(detail, "pane h1 already has /clear queued")
        self.assertEqual(ad.sent, [])
        ad.read.assert_not_called()

    def test_repeated_400_with_uncertain_composer_refuses_clear(self):
        got, ad = self.clear_fallback(
            "API Error: 400 prompt is too long: context exceeds maximum\n"
            "assistant output continued without a current prompt")
        self.assertEqual(got["fired"][0]["mode"], "recovery-manual")
        self.assertIn("current composer is cannot-tell",
                      got["fired"][0]["detail"])
        self.assertIn("no current composer could be located",
                      got["fired"][0]["detail"])
        self.assertEqual(ad.sent, [])
    ...
    def test_healthy_inventory_previews_batch_overflow_probe_without_reads(self):
        names, rows, ad = self.preview_case(("ready\n❯ ",) * 3)
        with mock.patch.object(autocompact, "scan", return_value=rows), \
                mock.patch.object(autocompact, "_live_seat_names",
                                  return_value=set(names)):
            res = autocompact.check(seats=list(names), post=False, adapter=ad)
        self.assertEqual(res["fired"], [])
        self.assertEqual((ad.list_calls, ad.read_calls), (1, 0))

        # Named mutation witness: deleting the negative-preview decision restores
        # the old N inventory + N read shape, so the call-count assertion above
        # distinguishes the optimization rather than merely exercising it.
        _, _, mutated = self.preview_case(("ready\n❯ ",) * 3)
        with mock.patch.object(autocompact, "scan", return_value=rows), \
                mock.patch.object(autocompact, "_live_seat_names",
                                  return_value=set(names)), \
                mock.patch.object(autocompact, "_overflow_preview_excludes",
                                  return_value=False):
            autocompact.check(seats=list(names), post=False, adapter=mutated)
        self.assertEqual((mutated.list_calls, mutated.read_calls), (4, 3))

    def test_repeated_400_preview_requires_fresh_confirmation(self):
        tail = ("API Error: 400 prompt is too long: context exceeds maximum\n"
                "API Error: 400 prompt is too long: context exceeds maximum")
        names, rows, ad = self.preview_case((tail,), tail=tail)
        with mock.patch.object(autocompact, "scan", return_value=rows), \
                mock.patch.object(autocompact, "_live_seat_names",
                                  return_value=set(names)):
            res = autocompact.check(seats=list(names), fire=False, post=False,
                                    adapter=ad)
        self.assertTrue(res["rows"][0]["overflow"])
        self.assertTrue(res["rows"][0]["would_recover"])
        self.assertEqual((ad.list_calls, ad.read_calls), (2, 1))

    def test_unusable_inventory_preview_falls_back_to_fresh_read(self):
        for preview in (None, "", "   "):
            with self.subTest(preview=preview):
                names, rows, ad = self.preview_case((preview,))
                with mock.patch.object(autocompact, "scan", return_value=rows), \
                        mock.patch.object(autocompact, "_live_seat_names",
                                          return_value=set(names)):
                    res = autocompact.check(
                        seats=list(names), fire=False, post=False, adapter=ad)
                self.assertFalse(res["rows"][0].get("overflow", False))
                self.assertEqual((ad.list_calls, ad.read_calls), (2, 1))

        names, rows, ad = self.preview_case(("ready\n❯ ",))
        ad.panes.append(dict(ad.panes[0]))
        with mock.patch.object(autocompact, "scan", return_value=rows), \
                mock.patch.object(autocompact, "_live_seat_names",
                                  return_value=set(names)):
            autocompact.check(seats=list(names), fire=False, post=False,
                              adapter=ad)
        self.assertEqual((ad.list_calls, ad.read_calls), (2, 1))

        names, rows, ad = self.preview_case(("ready\n❯ ",) * 2)
        rows[1]["pane_handle"] = rows[0]["pane_handle"]
        self.spawn(handle=rows[0]["pane_handle"], seat_name=names[1])
        with mock.patch.object(autocompact, "scan", return_value=rows), \
                mock.patch.object(autocompact, "_live_seat_names",
                                  return_value=set(names)):
            autocompact.check(seats=list(names), fire=False, post=False,
                              adapter=ad)
        self.assertEqual((ad.list_calls, ad.read_calls), (2, 2))

        class FailingInventoryAdapter(CountingAdapter):
            def list(self):
                self.list_calls += 1
                if self.list_calls == 1:
                    raise harness.HarnessError("inventory unavailable")
                return self.panes

        names, rows, healthy = self.preview_case(("ready\n❯ ",))
        ad = FailingInventoryAdapter(healthy.panes)
        with mock.patch.object(autocompact, "scan", return_value=rows), \
                mock.patch.object(autocompact, "_live_seat_names",
                                  return_value=set(names)):
            autocompact.check(seats=list(names), fire=False, post=False,
    ...
    def test_pending_clear_rebriefs_only_after_session_changes(self, _prune):
        ...
        tail = ("API Error: 400 prompt is too long: context exceeds maximum\n"
                "API Error: 400 prompt is too long: context exceeds maximum")
        ad = FakeAdapter(tail=tail)
        ad.read = mock.Mock(side_effect=(tail, "❯ "))
        first = autocompact.check(seats=["codex"], post=False, adapter=ad)
    ...
    def test_injected_latch_never_ttl_retries_without_context_drop(self):
        ...
        ad = FakeAdapter()
        autocompact.check(seats=["codex"], post=False, adapter=ad)
        # THE TTL HAS PASSED, stamped rather than slept: `fired_at` is read
        # only by the TTL rung (`now - fired_at > TTL`) and a display, so a
        # latch stamped two TTLs ago is the state a real wait produced.
        path = autocompact._state_path()
        state = pk.read_json(path, {})
        self.assertEqual(state["codex"]["mode"], "injected")
        state["codex"]["fired_at"] -= 2
        pk.write_json(path, state)
        res = autocompact.check(seats=["codex"], post=False, adapter=ad)
        self.assertEqual(res["fired"], [])
        self.assertTrue(res["rows"][0].get("latched"))
        self.assertEqual(len(ad.sent), 1)
        # CONTROL on the same stamp: a MANUAL latch this old is released, so
        # the stamp really is past the TTL the rung measures.
        manual = dict(state["codex"], mode="manual")
        self.assertFalse(autocompact._latch_blocks(
            manual, res["rows"][0], time.time()))
    ...
    def test_mismatch_repairs_stale_orca_handle_from_live_transcript_session(self):
        ...
        with mock.patch.object(
            ...
            res = autocompact.check(
                seats=["codex"], post=False, adapter=ad)
        # The LIVE transcript session stays this arm's subject; the fourth
        # argument is the seat's spawn record, handed to the proof so the
        # session-record root follows the record being decided on.
        prove.assert_called_once_with(self.d, live, "codex", mock.ANY)
...
class TimerInstallSwitchTest(unittest.TestCase):
    """HELM_AUTOCOMPACT_TIMER off: ensure_timer writes no unit file and runs no
    systemctl, and each caller reads the skipped result as neither a failure
    nor an armed timer.

    ONE SEAM IN EVERY ARM: pk.atomic_write for the two unit files and
    subprocess.run for systemctl. systemctl resolves to a fixed path and the
    unit paths are fixed, so neither the host's PATH nor the checkout decides
    which branch runs. The MUST-HIT arms drive the SAME seam with the switch
    absent and see both writes and both systemctl calls, which is what makes
    a zero count in the switched-off arms mean something."""

    UNITS = ("/fixture/helm-autocompact.service", "SERVICE",
             "/fixture/helm-autocompact.timer", "TIMER")

    @contextlib.contextmanager
    def seam(self, switch, systemctl="/fixture/bin/systemctl"):
        done = subprocess.CompletedProcess([], 0, "", "")
        with mock.patch.dict(os.environ), \
                mock.patch.object(autocompact.shutil, "which",
                                  return_value=systemctl), \
                mock.patch.object(autocompact, "_timer_units",
                                  return_value=self.UNITS) as units, \
                mock.patch.object(pk, "atomic_write") as write, \
                mock.patch.object(subprocess, "run",
                                  return_value=done) as run:
            if switch is None:
                os.environ.pop(autocompact.TIMER_ENV, None)
            else:
                os.environ[autocompact.TIMER_ENV] = switch
            yield units, write, run

    def assert_installed(self, write, run):
        self.assertEqual(write.call_args_list,
                         [mock.call("/fixture/helm-autocompact.service",
                                    "SERVICE"),
                          mock.call("/fixture/helm-autocompact.timer",
                                    "TIMER")])
        self.assertEqual(
    ...
    def test_switch_off_writes_no_unit_and_runs_no_systemctl(self):  # noqa: VACUOUS_ASSERTION — the loop is over a non-empty literal tuple; assertIsNone(ok) and the detail assertIns are positive on every value, and the MUST-HIT arm sees both writes and both systemctl calls on the same seam. Mutation (switch check removed) reddens all six subtests
        for value in ("0", "off", "no", "false", "OFF", " 0 "):
            with self.subTest(value=value), self.seam(value) as (units, write,
                                                                 run):
                ...
                self.assertIn("install skipped by HELM_AUTOCOMPACT_TIMER=%s"
                              % value, detail)
                self.assertIn("no unit file written, no systemctl run",
                              detail)
                # a skipped install never claims a timer was enabled
                self.assertNotIn("enabled", detail)

    def test_MUST_HIT_switch_absent_or_on_installs_through_the_same_seam(self):  # noqa: VACUOUS_ASSERTION — the loop is over a non-empty literal tuple and every value asserts ok is True plus the exact write and systemctl call lists; this arm IS the positive control for the zero counts above
        for value in (None, "", "1", "on", "yes"):
            with self.subTest(value=value), self.seam(value) as (_u, write,
                                                                 run):
                ok, detail = autocompact.ensure_timer()
                self.assertIs(ok, True, detail)
                self.assertIn("timer enabled every 60s", detail)
                self.assert_installed(write, run)

    def test_switch_off_skips_even_where_systemctl_is_missing(self):
        """The switch is asked first, so a host with no systemctl reports the
        switch, not a missing binary it never needed."""
        with self.seam("0", systemctl=None) as (_u, write, run):
            ok, detail = autocompact.ensure_timer()
        self.assertIsNone(ok)
        self.assertIn("HELM_AUTOCOMPACT_TIMER=0", detail)
        self.assertEqual((write.call_count, run.call_count), (0, 0))
        # MUST-HIT: switch absent on the same host is the missing-binary refusal
        with self.seam(None, systemctl=None) as (_u, write, run):
            ok, detail = autocompact.ensure_timer()
        self.assertIs(ok, False)
        self.assertIn("systemctl unavailable", detail)
        self.assertEqual((write.call_count, run.call_count), (0, 0))

    def _child(self, env):
        """ensure_timer in a REAL child process, the shape of the defect: no
        mock reaches it. HOME is a tmp dir and PATH holds no systemctl, so
        even a child that ignores the switch writes nothing on this host and
        stops at the missing-binary refusal."""
        import json
        import sys
        root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        tmp = tempfile.mkdtemp(prefix="helm-timer-child-")
        self.addCleanup(shutil.rmtree, tmp, ignore_errors=True)
        empty = os.path.join(tmp, "bin")
        os.makedirs(empty)
        env = dict(env, HOME=tmp, PATH=empty)
        p = subprocess.run(
            [sys.executable, "-c",
             "import json; from helm import autocompact; "
             "print(json.dumps(autocompact.ensure_timer()))"],
            cwd=root, env=env, capture_output=True, text=True, timeout=60)
        self.assertEqual(p.returncode, 0, p.stderr)
        return json.loads(p.stdout)

    def test_a_child_process_inherits_the_suite_switch(self):
        """tests/__init__.py sets the switch for the suite, and a child that
        inherits this environment takes the switched-off branch."""
        import sys
        if "tests" not in sys.modules:
            self.skipTest("runner did not import the tests package, so its "
                          "suite-wide environment is not planted")
        self.assertEqual(sys.modules["tests"].PLANTED.get(
            autocompact.TIMER_ENV), "0")
        ok, detail = self._child(os.environ)
        self.assertIsNone(ok, detail)
        self.assertIn("install skipped by HELM_AUTOCOMPACT_TIMER=0", detail)
        # MUST-HIT: the same child without the variable reaches the install
        # and stops at the systemctl this PATH does not have
        env = dict(os.environ)
        env.pop(autocompact.TIMER_ENV, None)
        ok, detail = self._child(env)
    ...
    def _install(self, switch):
        ...
        with self.seam(switch) as (_u, write, run), \
                contextlib.redirect_stdout(out), \
                contextlib.redirect_stderr(err):
            rc = autocompact._install_timer(["--apply"])
        return rc, out.getvalue(), err.getvalue(), write, run

    def test_install_timer_verb_switched_off_is_rc1_on_stderr(self):
        """`--install-timer --apply` asked for a timer and none was enabled:
        rc 1 on stderr naming the variable, never a success line on stdout."""
        rc, out, err, write, run = self._install("0")
        self.assertEqual(rc, 1)
        self.assertEqual(out, "")
        self.assertIn("helm seat autocompact: install skipped by "
                      "HELM_AUTOCOMPACT_TIMER=0", err)
        self.assertEqual((write.call_count, run.call_count), (0, 0))
        # MUST-HIT: the same verb through the same seam installs and says so
        rc, out, err, write, run = self._install(None)
        self.assertEqual(rc, 0, err)
        self.assertIn("helm seat autocompact: timer enabled every 60s", out)
        self.assert_installed(write, run)

    def _lifecycle(self, switch, systemctl="/fixture/bin/systemctl"):
        from helm import seat_lifecycle_sessions
        err = io.StringIO()
        with self.seam(switch, systemctl) as (_u, write, run), \
                contextlib.redirect_stderr(err):
            ok = seat_lifecycle_sessions._ensure_autocompact_timer()
        return ok, err.getvalue(), write, run

    def test_lifecycle_seam_switched_off_says_note_not_warn(self):  # noqa: VACUOUS_ASSERTION — each absence is paired on the same stderr: NOT WARN beside the asserted NOTE line, the empty stderr of the installed branch beside assert_installed on its writes and calls
        """The seat verbs' wrapper: a switched-off install is one NOTE naming
        the variable, not a WARN, and it hands back a falsy value."""
        ok, err, write, run = self._lifecycle("0")
        self.assertIsNone(ok)
        ...
        self.assertEqual((write.call_count, run.call_count), (0, 0))
        # MUST-HIT, both other branches through the same seam: a failure is
        # still a WARN, and an installed timer is silent
        ok, err, write, run = self._lifecycle(None, systemctl=None)
        self.assertIs(ok, False)
        self.assertIn("helm seat: WARN — autocompact timer not armed: "
                      "systemctl unavailable", err)
        ok, err, write, run = self._lifecycle(None)
        self.assertIs(ok, True)
        self.assertEqual(err, "")
        self.assert_installed(write, run)

    def test_the_callers_of_ensure_timer_are_the_two_this_class_covers(self):  # noqa: VACUOUS_ASSERTION — the empty non-Expr list is bracketed by assertTrue(wrapper_uses) on the same census and the exact caller-set equality
        """A THIRD VALUE IS A BUG IN EVERY CALLER THAT WAS NOT READ. The set
        of callers is pinned so a new one fails here until someone decides how
        it treats ok=None; and every call of the lifecycle wrapper is a bare
        statement, so no seat verb can refuse or report on its result."""
        import ast
        pkg = os.path.dirname(os.path.abspath(autocompact.__file__))
        callers, wrapper_uses = set(), []
        for dirpath, _dirs, files in os.walk(pkg):
            for name in files:
                ...
                with open(path, encoding="utf-8") as f:
                    text = f.read()
                # a call spells the name in its source; parse only those files
                if "ensure_timer" not in text and \
                        "_ensure_autocompact_timer" not in text:
                    continue
                ...
                for node in ast.walk(tree):
                    if isinstance(node, ast.ImportFrom) and \
                            (node.module or "").endswith("autocompact"):
                        self.assertNotIn(
                            "ensure_timer", [a.name for a in node.names],
                            "%s imports ensure_timer by name" % rel)
                    if not isinstance(node, ast.Call):
                        continue
                    fn = node.func
                    named = fn.attr if isinstance(fn, ast.Attribute) else \
                        getattr(fn, "id", None)
                    ours = (isinstance(fn, ast.Attribute)
                            and named == "ensure_timer"
                            and getattr(fn.value, "id", None) == "autocompact"
                            ) or (rel == "autocompact.py"
                    ...
                    if ours:
                        ...
                        while up is not None and not isinstance(
                            ...
                            up = parents.get(up)
                        callers.add((rel, up.name if up else "<module>"))
                    if named == "_ensure_autocompact_timer":
                        wrapper_uses.append(
                            (rel, node.lineno,
                             type(parents.get(node)).__name__))
        self.assertEqual(callers, {
            ("autocompact.py", "_install_timer"),
            ("seat_lifecycle_sessions.py", "_ensure_autocompact_timer")})
        self.assertTrue(wrapper_uses, "the census found no wrapper call")
        self.assertEqual(
            [u for u in wrapper_uses if u[2] != "Expr"], [],
            "a seat verb uses the wrapper's result; decide how it reads None")


class HotBlockedTest(AutocompactBase):
    ...
    def test_the_None_comes_from_a_BLIND_census_not_only_from_an_EXCEPTION(self):
        ...
        read every claude on the host (dispatch 90845108). A
...
class SilentBackstopTest(AutocompactBase):
...
class Ds4proWindowIsProbeBackedTest(unittest.TestCase):
    ...
    def test_a_measured_family_still_pins_its_own_window(self):
        ...
        _window that always answers the default: ds4pro's 1M is MEASURED
        (probed context_length off /v1/models) and must survive. It read kimi
        until task/2944 gave kimi a context_budget; kimi's measured window
        survives in its entry, and what the gauge reads is now its budget."""
        win, why = autocompact._window("ds4pro")  # noqa: SEAT_NAME — the catalog FAMILY key whose window IS the subject of this arm
        self.assertEqual(why, "FAMILIES.max_context")
        self.assertEqual(win, 1000000)
        self.assertEqual(seat.FAMILIES["kimi"]["max_context"], 1000000)
        self.assertEqual(autocompact._window("kimi"),
                         (seat.FAMILIES["kimi"]["context_budget"],
                          "FAMILIES.context_budget"))
...
class RefusalLoopEscalatesTest(AutocompactBase):
    ...
    def _age_the_alarm(self, seconds):
        """Backdate the live series AND its alarm by `seconds`.

        Both owners carry `since`; ageing only one of them would test the
        bookkeeping rather than the elapsed-time rung.
        """
        path = autocompact._state_path()
        state = pk.read_json(path, {})
        for key in (autocompact._REFUSAL_KEY, autocompact._REFUSAL_ALARM_KEY):
            for entry in (state.get(key) or {}).values():
                for owner in (entry, entry.get("evidence") or {}):
                    if isinstance(owner.get("since"), (int, float)):
                        owner["since"] -= seconds
        pk.write_json(path, state)

    def test_a_flat_wedge_past_the_envelope_escalates(self):  # noqa: VACUOUS_ASSERTION — every absence arm here is controlled unconditionally on the SAME observable: `wedged` is asserted empty before ageing and non-empty (["codex"]) after it, on the same fixture and the same flat percentage
        """THE MEASUREMENT THIS RUNG COMES FROM (the whole durable chat
        journal, seven weeks of it): 73 wedge episodes across 8 seats; 66 of
        them DISCHARGED THEMSELVES, median 32.1 min, 63 of those inside four
        hours. So n>=3 non-falling refusals does NOT mean the seat is doomed,
        and the alarm that said it did was wrong nine times in ten.

        What the population separates is DURATION. And it exposes the opposite
        hole from the one that was alleged: HOT_BUCKET only re-announces on
        another five points of CLIMB, so a wedge pinned FLAT re-announces
        never — the worst episode on record (a project's codex seat, FLAT at
        99.5% for 4.3 days) produced exactly ONE chat line in four days.

        POSITIVE: a flat series, already alerted, that outlives the envelope
        must escalate anyway — the climb gate must not be able to silence it.
        Its CONTROL is the next test: the same age, broken by one fall."""
        self.assertEqual(autocompact.WEDGE_STUCK_S, 4 * 3600.0)
        for pct in (1.12, 1.15, 1.20):
            res = self.refuse(pct)
        self.assertEqual(len(res["wedged"]), 1, "fixture never armed the wedge")
        self.assertFalse(res["wedged"][0].get("wedge_stuck"),
                         "a two-minute series is not stuck")
        self.assertEqual(self.refuse(1.20)["wedged"], [],
                         "the flat pass must be silent BEFORE it is aged — "
                         "otherwise the escalation below proves nothing")

        self._age_the_alarm(autocompact.WEDGE_STUCK_S + 60)
        stuck = self.refuse(1.20)          # still flat: HOT_BUCKET stays shut
        self.assertEqual([r["seat"] for r in stuck["wedged"]], ["codex"],
                         "a flat wedge past the envelope stayed silent")
        self.assertTrue(stuck["wedged"][0].get("wedge_stuck"))
        text = autocompact._wedge_text(stuck["wedged"][0], 80)
        self.assertIn("is STUCK BEHIND", text)
        self.assertIn("NEEDS ITS LEAD", text)
        alarm = pk.read_json(autocompact._state_path(),
                             {})[autocompact._REFUSAL_ALARM_KEY]["codex"]
        self.assertIsNotNone(alarm["stuck_at"], "the rung did not latch")
        self.assertEqual(self.refuse(1.20)["wedged"], [],
                         "the stuck rung re-announced every pass")

    def test_a_wedge_that_fell_once_never_reaches_the_stuck_rung(self):  # noqa: VACUOUS_ASSERTION — each absence arm has its positive twin on THIS fixture: `wedged` names codex when the series arms, and the aged alarm renders STUCK before the fall, so the silence after it is the fall's doing
        """THE CONTROL for the flat-wedge rung, and the whole point of it: a
        busy seat's percentage FALLS when its turn lands, and a fall RESETS the
        run. A seat that has been over the trigger for just as long on the wall
        clock, but fell once in the middle, must never reach the stuck rung.

        ITS OWN TEST, NEVER A SECOND setUp. This arm once shared the method
        above and got its clean world by calling self.setUp() again. That
        re-saved the environment from the one the first fixture had already
        rewritten, so tearDown "restored" the first fixture's temporary
        HELM_HOME and HELM_CHAT_DIR, and every later test in the process ran
        against a chat directory that did not exist; the second patch.start
        left SUBMIT_VERIFY_INTERVAL_S at 0 for good. The runner hands each test
        a fresh fixture, and tests/test_env_hygiene.py refuses a re-entrant
        setUp.

        BOTH HALVES ARE PROVED ON THIS FIXTURE, so a fall that reset nothing
        cannot pass for one that reset the run: before the fall the aged
        evidence renders STUCK — the ageing reached the rung's own input — and
        after it the same rendering must not."""
        def said(a):
            return autocompact._wedge_text(
                {"seat": "codex", "window": CODEX_WINDOW,
                 "refusal_wedge": a["evidence"]}, 80)

        def alarm():
            return pk.read_json(autocompact._state_path(),
                                {})[autocompact._REFUSAL_ALARM_KEY]["codex"]

        for pct in (1.12, 1.15, 1.20):
            armed = self.refuse(pct)
        self.assertEqual([r["seat"] for r in armed["wedged"]], ["codex"],
                         "fixture never armed the wedge")
        self._age_the_alarm(autocompact.WEDGE_STUCK_S + 60)
        self.assertIn("is STUCK BEHIND", said(alarm()),
                      "the ageing never reached the rung's input, so the "
                      "fall below would prove nothing")

        fell = self.refuse(1.10)           # its turn landed; still over trigger
        self.assertEqual(fell["wedged"], [], "a falling series announced")
        for pct in (1.11, 1.12):
            res = self.refuse(pct)
        self.assertFalse(any(r.get("wedge_stuck") for r in res["wedged"]),
                         "a seat that FELL was called stuck")
        self.assertIsNone(alarm()["stuck_at"])
        self.assertNotIn("is STUCK BEHIND", said(alarm()))

    def test_the_ordinary_wedge_alarm_claims_only_what_was_measured(self):
        """The first rung asked for a manual rescue at two minutes on evidence
        that, measured, clears itself 90% of the time. It must state the base
        rate and ask for nothing, or every reader learns to filter it."""
        for pct in (1.12, 1.15, 1.20):
            res = self.refuse(pct)
        text = autocompact._wedge_text(res["wedged"][0], 80)
        for false_claim in ("NEVER become eligible", "until it 400s",
                            "Nothing automatic will act on it",
                            "esc the pane by hand"):
            self.assertNotIn(false_claim, text,
                             "the alarm still asserts what the ledger refutes")
        self.assertIn("66 cleared", text)
        self.assertIn("No action is asked for here", text)
    ...
    def test_escalation_posts_to_chat_and_still_never_touches_the_pane(self):  # noqa: VACUOUS_ASSERTION — the empty ad.sent is controlled unconditionally by the IDLE pass below, which sends /compact through the same fixture
        ...
        bodies = [c.args[0] for c in post.call_args_list]
        self.assertTrue(any("REFUSING /compact BEHIND ITS OWN SAFETY CHECK" in b
                            for b in bodies), bodies)
    ...
    def test_recovery_retires_a_failed_wedge_notice_before_discharge(self):  # noqa: VACUOUS_ASSERTION — retained wedge and delivered discharge positively control the final no-retry assertion on the same durable outbox
        ...
        def post_notice(text, **_kw):
            calls.append(text)
            if "[watchdogs-correct-composition-holed]" in text:
    ...
    def test_recovery_cannot_overtake_an_inflight_wedge_notice(self):  # noqa: VACUOUS_ASSERTION — the blocked wedge append positively controls recovery deferral, followed by the same episode's ordered discharge
        ...
        self.assertIn("[watchdogs-correct-composition-holed]", calls[0])
    ...
    def test_wedge_notice_ignores_a_malformed_discharge_observation(self):  # noqa: VACUOUS_ASSERTION — the emitted wedge row proves the malformed notice survived normalization before its successful ack empties the outbox
        ...
        self.assertEqual(post.call_count, 1)
        self.assertIn("[watchdogs-correct-composition-holed]",
...
class Codex3HundredPercentPaneTest(AutocompactBase):
    ...
    def test_update_toast_below_composer_is_footer_chrome(self):  # noqa: VACUOUS_ASSERTION — the fixed non-empty toast tuple and exact IDLE result positively control each loop arm
        for toast in ("Update installed\nRestart to update",
                      "Update installed · Restart to update"):
            with self.subTest(toast=toast):
                tail = "❯\n%s" % toast
                self.assertEqual(seat._current_prompt_line(tail), "❯")
    ...
    def test_update_words_in_transcript_prose_are_not_footer_chrome(self):  # noqa: VACUOUS_ASSERTION — exact toast lines above positively control the must-miss prose
        for line in ("Update installed", "Restart to update",
                     ...
                     "The docs say Restart to update dependencies",
                     "● Update installed after the previous turn"):
            with self.subTest(line=line):
                self.assertIsNone(seat._PANE_CHROME.match(line))
                self.assertIsNone(
                    seat._current_prompt_line("❯ old submitted\n%s" % line))
                self.assertIsNone(
                    seat._current_prompt_line("❯ /compact\n%s" % line))
...
def _counter_fixture(name):
    """A pane viewport captured live off this fleet (task/2808).

    Verbatim in every row the classifier reads; only transcript prose bodies
    are shortened. `tests/test_seat_liveness.py` holds the classifier-level
    arms and the measurement these two panes come from.
    """
    with open(os.path.join(os.path.dirname(os.path.abspath(__file__)),
                           "fixtures", name), encoding="utf-8") as f:
        return f.read()


def _mid_repaint_tail():
    """A LIVE turn sampled after its output landed and before its counter
    repainted — helm-codex's CL96 tail (task/2808).

    Rebuilt from the two captures above rather than captured mid-paint, and
    that limit is stated rather than hidden: nobody caught a mid-paint
    viewport off this fleet read-only, so nothing is written to fixtures/.
    Every ROW is verbatim — the live pane whole, plus one real assistant
    bullet from the other capture inserted directly under its counter. The
    INSERTION is the modelled event, and the footer under it is the one the
    previous paint left standing. Derivation and its measurements at three
    tips: `tests/test_seat_liveness.py`.
    """
    live = _counter_fixture("live-counter-running-tail.txt").splitlines()
    counter = max(i for i, line in enumerate(live)
                  if "(1h 10m 28s · ↓ 86.4k tokens)" in line)
    bullet = next(line for line in
                  _counter_fixture("stranded-counter-idle-tail.txt").splitlines()
                  if line.startswith("● Checked live state:"))
    return "\n".join(live[:counter + 1] + [bullet] + live[counter + 1:]) + "\n"


class ABeaconIsNotATurnTest(AutocompactBase):
    """The watchdog refused a healthy idle seat forever — task/2808.

    Measured on helm-codex: three consecutive autocompact refusals while
    context climbed 80.2% -> 80.7% and never fell. The refusal is CORRECT
    given the state it was handed; the state was wrong. The pane was idle
    with an empty composer and its only activity was the seat's own armed
    beacon Monitor, whose firing repaints the pane and strands a spent
    elapsed-plus-token counter in the viewport — which RUNNING matched
    anywhere in the tail. Every seat in this fleet is instructed to keep that
    beacon armed, so the failure is fleet-wide, silent and one-way: a turn
    that never lands never yields the empty composer that would make the seat
    eligible on its own, and the seat eventually 400s with its context gone.
    """

    def test_an_idle_seat_with_only_a_stranded_counter_is_compacted(self):
        tail = _counter_fixture("stranded-counter-idle-tail.txt")
        # MUST-HIT seeds: the defect's two inputs are really in this pane.
    ...
    def test_a_genuinely_live_turn_is_still_refused(self):  # noqa: VACUOUS_ASSERTION — the exact RUNNING actuation state and its refusal reason are positive controls on the same pass
        """The control, and it is a DIFFERENT pane read in the same sweep."""
        tail = _counter_fixture("live-counter-running-tail.txt")
        self.assertEqual(seat._classify_pane_tail(tail)[0], "RUNNING")
        self.plant(int(CODEX_WINDOW * 0.96))
        ad = FakeAdapter(tail=tail)
        res = autocompact.check(seats=["codex"], post=False, adapter=ad)
        ...
        row = res["rows"][0]
        self.assertEqual(row["actuation_state"], "RUNNING")
        self.assertIn("open turn", row["actuation_reason"])

    def test_a_live_turn_sampled_mid_repaint_is_never_compacted(self):  # noqa: VACUOUS_ASSERTION — fired and sent are asserted EMPTY on a pass whose sibling arm fires and sends on the neighbouring fixture, and actuation_state is a positive control on this same pass
        """CL96, THE EXPENSIVE DIRECTION, THROUGH THE SHIPPED CHECK.

        A probe reproduced the false IDLE at BOTH layers, and the second is
        why it outranks the defect this lane was opened for: `_fire` reads the
        classifier ONCE and injects on IDLE with an empty composer, with no
        liveness re-read. So a false IDLE here does not delay a compaction, it
        destroys a live turn's work — and the classifier's word is not enough
        for this arm. What must be empty is what the watchdog SENT.
        """
        tail = _mid_repaint_tail()
        # MUST-HIT seeds: this tail passes BOTH halves of the production gate
        # if the classifier gets it wrong — a resolvable EMPTY composer is
        # what makes the false IDLE actuate rather than refuse.
        self.assertIn("(1h 10m 28s · ↓ 86.4k tokens)", tail)
        self.assertIn("● Checked live state:", tail)
        self.assertEqual(seat._current_prompt_line(tail), "❯")
        self.assertEqual(seat._classify_pane_tail(tail)[0], "RUNNING")
        self.plant(int(CODEX_WINDOW * 0.96))
        ad = FakeAdapter(tail=tail)
...
class ContextBudgetTest(unittest.TestCase):
    """task/2944: a family `context_budget` narrows the window a seat is
    TAUGHT, and the launch line and this watchdog read it through one helper
    (seat_catalog.taught_window). The launch-line half is pinned in
    tests/test_seat.py; this class pins the helper, the gauge, and the
    import-time refusal of an incoherent budget or fresh-session floor."""

    def test_kimi_is_taught_its_budget_and_keeps_its_measured_window(self):
        fam = seat.FAMILIES["kimi"]
        self.assertEqual(fam["max_context"], 1000000)
        self.assertLess(fam["context_budget"], fam["max_context"])
        self.assertEqual(autocompact._window("kimi"),
                         (fam["context_budget"], "FAMILIES.context_budget"))
        # CONTROL: a pinned family with no budget still reads its own window,
        # so the narrowing is kimi's declaration and not a table-wide cut.
        self.assertEqual(autocompact._window("ds4pro"),
                         (1000000, "FAMILIES.max_context"))  # noqa: SEAT_NAME — the catalog FAMILY key whose window IS the subject of this arm

    def test_kimi_budget_lets_the_measured_p95_row_finish(self):
        """Two owner goals, one number. The 1M window was set because CC,
        taught nothing, compacted kimi at 160k, about 5x too often. The budget
        exists because long-lived sessions spent the weekly allowance. With a
        fresh session between rows the budget only has to stop ONE runaway
        row, so it is sized from the MEASURED p95 per-row peak: the watchdog's
        compaction point (80% of the budget) must sit at or above it, the
        budget stays inside [200000, max_context], and the measurement carries
        its date and source."""
        m = seat_catalog.KIMI_ROW_PEAK_P95
        self.assertEqual((m["tokens"], m["percentile"], m["rows"],
                          m["measured"]), (300373, 95, 174, "2026-09-23"))
        self.assertIn("transcripts", m["source"])
        fam = seat.FAMILIES["kimi"]
        pct = int(seat_catalog.AUTOCOMPACT_PCT_OVERRIDE)
        self.assertGreaterEqual(fam["context_budget"] * pct / 100.0,
                                m["tokens"],
                                "the budget compacts the measured p95 row")
        self.assertGreaterEqual(fam["context_budget"], 200000)
        self.assertLessEqual(fam["context_budget"], fam["max_context"])

    def test_a_budget_only_narrows(self):
        tw = seat_catalog.taught_window
        self.assertEqual(tw({"context_budget": 200000}, 1000000), 200000)
        # a model window already under the budget is kept, never widened
        self.assertEqual(tw({"context_budget": 200000}, 76000), 76000)
    ...
    def test_an_incoherent_budget_or_floor_is_refused(self):
        ...
        # POSITIVE CONTROLS: the shipped table and kimi's own entry pass.
        self.assertIsNone(bad())
        self.assertIsNone(bad({"k": kimi}))
        self.assertIn("caps nothing",
                      bad({"k": dict(kimi, context_budget=1000000)}))
        self.assertIn("no max_context",
                      bad({"k": {"context_budget": 150000}}))
        self.assertIn("positive token count",
                      bad({"k": dict(kimi, context_budget=True)}))
        # the watchdog compacts at 80% of the taught budget, so a floor there
        # is never reached; one token under it is admitted
        trigger = kimi["context_budget"] * 80 // 100
        self.assertIn("never reached",
                      bad({"k": dict(kimi, fresh_session_floor=trigger)}))
        self.assertIsNone(bad({"k": dict(kimi, fresh_session_floor=trigger - 1)}))
        self.assertIn("positive token count",
                      bad({"k": dict(kimi, fresh_session_floor="100k")}))


class FreshSessionBetweenRowsTest(AutocompactBase):
    """task/2944: a seat whose family declares `fresh_session_floor` gets a
    fresh session BETWEEN rows, through the existing /clear -> SessionStart
    -> onboarding rungs. The codex fixture seat stands in for kimi: its
    family entry is given a floor for the test, and the real between-rows
    reads run against this fixture's empty dispatch and claims ledgers."""
    ...
    def setUp(self):
        super().setUp()
        floor = mock.patch.dict(seat.FAMILIES["codex"],
                                {"fresh_session_floor": self.FLOOR})
        floor.start()
        self.addCleanup(floor.stop)

    def idle(self, ctx=120000, quiet=True):
        extra = 60 if quiet else -60
        return self.plant(ctx, age_s=autocompact.FRESH_ROW_QUIET_S + extra)

    def state(self):
        return pk.read_json(autocompact._state_path(), {}) or {}
    ...
    def check(self, ad, **kw):
        with mock.patch("helm.chat.post") as post:
            res = autocompact.check(seats=["codex"], adapter=ad, **kw)
        return res, post

    def refused(self, ad):
        res, _post = self.check(ad)
        self.assertEqual(ad.sent, [])
        self.assertEqual(res["fired"], [])
        return res["rows"][0]["fresh_refused"]

    def test_an_idle_seat_between_rows_is_cleared_then_re_onboarded(self):  # noqa: VACUOUS_ASSERTION — the empty second pass and the released latch are the contract; the /clear send and the cleared mode on the same check() results are the positive controls
        self.idle()
        ad = FakeAdapter()
        first, post = self.check(ad)
        self.assertEqual(ad.sent, [("h1", "/clear", True)])
        row = first["fired"][0]
        self.assertEqual((row["mode"], row.get("fresh_row")),
                         ("clear-pending", True))
        self.assertIn("no owed dispatch row", row["detail"])
        entry = self.state()["codex"]
        self.assertEqual((entry["mode"], entry["why"]),
                         ("clear-pending", autocompact.FRESH_ROW))
        post.assert_not_called()          # routine: nothing in the room
        # nothing more until SessionStart binds the new session
        waiting, _post = self.check(ad)
        self.assertEqual(waiting["fired"], [])
        self.assertEqual(len(ad.sent), 1)
        # SessionStart bound the new session: the SAME onboarding rung the
        # context-wall recovery uses re-arms the beacon in a clean context
        self.plant(40000, sid=self.SID2)
        self.rebind(self.SID2)
        second, post = self.check(ad)
        row = second["fired"][0]
        self.assertEqual((row["mode"], row.get("fresh_row")),
                         ("cleared", True))
        self.assertEqual(len(ad.sent), 2)
        self.assertIn(seat.onboarding_prompt("codex", "main"), ad.sent[1][1])
        self.assertNotIn("codex", self.state())   # latch released
        post.assert_not_called()

    def test_an_onboarding_that_fails_after_the_clear_is_loud(self):
        self.idle()
        ad = FakeAdapter()
        self.check(ad)
        self.plant(40000, sid=self.SID2)
        self.rebind(self.SID2)
        ad.send = mock.Mock(side_effect=harness.HarnessError("pane gone"))
        res, post = self.check(ad)
        self.assertEqual(res["fired"][0]["mode"], "clear-needs-brief")
        post.assert_called_once()
        text = post.call_args[0][0]
        self.assertIn("FRESH SESSION", text)
        self.assertIn("NOT armed", text)
        self.assertEqual(self.state()["codex"]["why"], autocompact.FRESH_ROW)

    def test_an_owed_row_under_any_spelling_refuses(self):
        self.idle()
        with mock.patch("helm.dispatches.open_recipients",
                        return_value=({"@Codex": 1}, None)):
            why = self.refused(FakeAdapter())
        self.assertEqual(why, "1 owed dispatch row(s) name this seat")

    def test_an_unreadable_dispatch_ledger_refuses(self):
        self.idle()
        with mock.patch("helm.dispatches.open_recipients",
                        return_value=(None, "torn line 7")):
            why = self.refused(FakeAdapter())
        self.assertIn("could not be read (torn line 7)", why)

    def test_a_held_or_unattributable_claim_refuses(self):  # noqa: VACUOUS_ASSERTION — the loop is over a literal three-tuple, so every subTest assertion runs
        self.idle()
        for claims, want in (
                ({"worktree:helm:lane": {"holder": "codex"}},
                 "holds live claim worktree:helm:lane"),
                ({"worktree:helm:lane": {"holder": ""}},
                 "no readable holder"),
                (None, "claims ledger could not be read")):
            with self.subTest(claims=claims), \
                    mock.patch("helm.seats._live_claims",
                               return_value=claims):
                self.assertIn(want, self.refused(FakeAdapter()))

    def test_another_seats_claim_does_not_refuse(self):
        self.idle()
        ad = FakeAdapter()
        with mock.patch("helm.seats._live_claims", return_value={
                "worktree:helm:lane": {"holder": "seat-b"}}):
            self.check(ad)
        self.assertEqual(ad.sent, [("h1", "/clear", True)])

    def test_a_failed_ledger_read_is_unknown_and_the_pass_survives(self):
        self.idle()
        with mock.patch("helm.dispatches.open_recipients",
                        side_effect=OSError("ledger vanished")):
            why = self.refused(FakeAdapter())
        self.assertIn("between-rows read failed: OSError", why)

    def test_an_open_turn_or_a_draft_refuses(self):  # noqa: VACUOUS_ASSERTION — the loop is over a literal two-tuple, so every subTest assertion runs
        self.idle()
        for tail, want in (
                ("working\n  ⏵⏵ bypass permissions on · esc to interrupt\n❯ ",
                 "refused-not-idle"),
                ("❯ half a sentence the owner typed", "recovery-manual")):
            with self.subTest(tail=tail):
                self.assertTrue(self.refused(FakeAdapter(tail=tail))
                                .startswith(want))

    def test_the_cheap_gates_quiet_floor_and_declaration(self):
        self.idle()
        self.assertTrue(autocompact._fresh_candidate(
            autocompact.read("codex")))                    # the control
        self.idle(quiet=False)
        self.assertFalse(autocompact._fresh_candidate(
            autocompact.read("codex")))
        self.idle(ctx=self.FLOOR - 1)
        self.assertFalse(autocompact._fresh_candidate(
            autocompact.read("codex")))
        self.idle()
        with mock.patch.dict(seat.FAMILIES["codex"]):
            del seat.FAMILIES["codex"]["fresh_session_floor"]
            self.assertFalse(autocompact._fresh_candidate(
                autocompact.read("codex")))

    def test_a_second_clear_waits_out_the_spacing(self):  # noqa: VACUOUS_ASSERTION — the refused pass sends nothing by contract; the spacing reason and the later /clear send are the positive controls
        self.idle()
        now = time.time()
        pk.write_json(autocompact._state_path(), {"_fresh": {"codex": now}})
        self.assertIn("spacing", self.refused(FakeAdapter()))
        pk.write_json(autocompact._state_path(), {"_fresh": {
            "codex": now - autocompact.FRESH_ROW_SPACING_S - 1}})
        ad = FakeAdapter()
        self.check(ad)
        self.assertEqual(ad.sent, [("h1", "/clear", True)])
        self.assertGreater(self.state()["_fresh"]["codex"], now)

    def test_a_dry_run_reports_and_touches_nothing(self):  # noqa: VACUOUS_ASSERTION — a dry run writing nothing IS the contract; would_fresh and the rendered line are the positive controls on the same result
        self.idle()
        ad = FakeAdapter()
        res, _post = self.check(ad, fire=False)
        self.assertTrue(res["rows"][0]["would_fresh"])
        self.assertIn("would start a fresh session",
