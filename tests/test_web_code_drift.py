...
Measured live: the owner's console had been serving current HTML against
20-hour-old web.py, including an honest-presence fix ("a roster dot may only
report evidence about the seat it names"). Half of an honesty fix is not
honest, and the owner had no way to know.
...
class CodeDriftTest(unittest.TestCase):
    ...
    def test_source_newer_than_the_load_stamp_REPORTS(self):
        ...
        # this test went red on a 2.24h-old lane worktree).
