...
ENV_KEYS = ("HELM_HOME", "HELM_CHAT_DIR", "HELM_CHAT_NAME", "HELM_CHAT_ROOM",
            ...
            # set to "0" in setUp: see the comment there
...
class NewAgentGuideTest(unittest.TestCase):
    def setUp(self):
        ...
        # Every helm child below inherits this environment, and one of them is
        # a real `helm seat launch`. Without the switch that child installs
        # helm-autocompact.timer in the HOST's ~/.config/systemd/user and runs
        # the real systemctl: HELM_HOME does not move that path and no mock
        # reaches a child. tests/__init__.py sets the same value for the whole
        # suite; this line keeps it when the module runs as a script.
    ...
    def test_launch_line_instance_port_and_token_file(self):
        ...
        self.assertEqual(p.returncode, 0, p.stderr)
        # THE FENCE REACHED THE CHILD: its ensure_timer took the switched-off
        # branch and said so, so no unit file was written and no systemctl ran.
        self.assertIn("autocompact timer not armed: install skipped by "
                      "HELM_AUTOCOMPACT_TIMER=0", p.stderr)
        line = p.stdout.strip()
