...
THE DEFECT, measured on the live fleet 2026-07-29. `helm seat doctor` rendered:
...
class SeatRowRendersUnknownTest(unittest.TestCase):
    ...
    def test_a_never_minted_seat_reads_unminted_and_keeps_UNKNOWN_out(self):
        """ONE WORD FOR "nothing is minted here", and it is not UNKNOWN.

        The model column reads a seat's launch.sh. ds4pro has none, and the
        word helm renders for that is `unminted` — a measured absence. The
        round-2 spelling was `unlaunched`, and before that it was UNKNOWN,
        which turned a healthy api-key seat into an alarm these arms cannot
        tell apart from the cred fact they guard. Pinned here so the word
        cannot drift without this suite saying so. BLAST RADIUS: this arm and
        the two api-key arms beside it — a reader that answered UNKNOWN for an
        absent script reddens all three, and a rename of the word reddens only
        this one."""
        row = self._row("ds4pro", {}, mode="proxy-key")  # noqa: SEAT_NAME — the live api-key family with no launch.sh is this arm's subject, exactly as in the two arms below
        self.assertIn("unminted", row)
        self.assertNotIn("UNKNOWN", row)
...
class CredRemainderAndRollingTest(unittest.TestCase):
    ...
    Measured 2026-08-03 at 22:25Z: `helm seat doctor` rendered gemini as
    ...
    A reader who acts on it is responding to an integer truncation. I very
    nearly announced a seat about to die mid-lane on the strength of it, and
