...
class ChainBase(unittest.TestCase):
    def setUp(self):
        ...
        # An empty fake proc tree for the census readers. ADMISSION never read
        # HELM_PROC, so the gate.run fixtures are pinned to a fixture box too
        # (task/1740) — without it they were red whenever the node was full.
...
class DuplicateSuccessorWarningTest(ChainBase):
    ...
    def test_an_OPEN_successor_refuses_a_second_child_without_force(self):  # noqa: VACUOUS_ASSERTION — the same warning first positively names the OPEN child, then the HELD child; both calls also prove the refusal returns no row
        parent = self.root()
        first, why = self.child(parent["id"], lane="round-2")
        self.assertIsNone(why)

        second, open_warning = self.child(parent["id"], lane="round-2-again")
        self.assertIsNone(second)
        self.assertIn("%s (OPEN)" % first["id"][:12], open_warning)
        self.assertNotIn("dispatch list --held", open_warning)
        self.assertIn(parent["id"][:12], open_warning)
        self.assertIn("--force", open_warning)

        held, why = dispatches.mark_hold(first["id"], "waiting on dependency")
        self.assertIsNone(why)
        self.assertEqual(held["status"], "held")
        second, held_warning = self.child(parent["id"], lane="round-2-again")
        self.assertIsNone(second)
        self.assertIn("%s (HELD)" % first["id"][:12], held_warning)
        self.assertNotIn("%s (OPEN)" % first["id"][:12], held_warning)
        self.assertIn("dispatch list --held", held_warning)
        self.assertEqual(len(dispatches.rows()), 2)
    ...
    def test_NEW_WORK_with_an_OPEN_same_lane_refuses_unless_forced(self):
        ...
        self.assertIn("%s (OPEN)" % first["id"][:12], open_warning)
        self.assertNotIn("dispatch list --held", open_warning)
        self.assertIn("born-wrong", open_warning)
    ...
    def test_a_CLOSED_successor_does_not_block_the_next_round(self):  # noqa: VACUOUS_ASSERTION — the forced sibling below unconditionally proves _WRITE_WARNINGS is observable
        parent = self.root()
        closed, why = self.child(parent["id"], lane="closed-round")
        self.assertIsNone(why)
        # APPROVE closes the work identity; FIX closes only its review turn.
        with mock.patch.object(
                dispatches.gate, "bind",
                return_value=("VERIFIED", "a" * 16, "fixture receipt")):
            verdict, why = dispatches.mark_verdict(
                closed["id"], closed["tip"], "gate:" + "a" * 16,
                polarity="approve")
        self.assertIsNone(why, why)
        self.assertEqual(verdict["polarity"], "approve")
        next_round, why = self.child(parent["id"], lane="next-round")
        self.assertIsNone(why)
        self.assertNotIn(dispatches._WRITE_WARNINGS, next_round)

        control, why = self.child(next_round["id"], lane="open-control")
        self.assertIsNone(why)
        forced, why = self.child(
            next_round["id"], lane="forced-control", force=True)
        self.assertIsNone(why)
        self.assertIn(dispatches._WRITE_WARNINGS, forced)
        self.assertIn(control["id"][:12], self.warnings(forced))
...
class DischargeWalksTheChainTest(ChainBase):
    ...
    def approve(self, tip, lane, supersedes=None):
        ...
        # The shared process seam preserves the canonical gate command. Every
        # fixture caller commits `tip` immediately before approving, so the repo
        # HEAD equals the reviewed tip and the receipt binds.
        with serial_process():
            g, gerr = gate.run(repo=self.repo)
        self.assertIsNone(gerr, gerr)
        from tests._verdict import native_author
        with native_author(self):
            got, verr = dispatches.mark_verdict(
...
class DischargedCloseDoorTest(ChainBase):
    ...
    agree, or the ledger answers differently on reload (a named risk).
    ...
    def _opened_a_second_ago(self):
        """Stamp the rows opened before the gate run one whole second in the
        past, and return the STARTED patch: the caller stops it right after
        those rows, so the receipt keeps the real clock (the cleanup only
        covers an add that raises). gmtime floors, so floor(t - 1) is
        exactly one second before any `pk.now_ts` stamp taken at or after t.
        The build row takes it too, so a build never reads as opened after
        its own review.

        NOT WHAT MAKES THESE ARMS PASS TODAY, MEASURED. The receipt's head IS
        the reviewed tip, and gate.bind verifies an exact-tip receipt before
        its postdate rung (minted <= opened refuses) is reached. With every
        stamp frozen to one second, all seven arms still passed on fab. This
        keeps the precondition true should the receipt ever bind through the
        descendant rung, where a same-second receipt refuses. It replaces a
        1.1s sleep that bought the same order."""
        p = mock.patch.object(pk, "now_ts",
    ...
    def _build_row(self, lane="build-lane"):
        opened = self._opened_a_second_ago()
    ...
    def _landed_review(self, build, lane="review-lane"):
        ...
        writer did."""
        # Opened a second before the receipt is minted (see the helper for
        # why that order is kept, and why it is not what binds the receipt).
        opened = self._opened_a_second_ago()
        ...
        # (self.b is the tip commit). The shared process seam preserves the
        # canonical gate command and interpreter.
        with serial_process():
            g, gerr = gate.run(repo=self.repo)
        self.assertIsNone(gerr, gerr)
    ...
    def test_a_forged_event_dies_on_REPLAY_not_just_at_the_writer(self):
        """The named risk, from the other side: an event appended OUTSIDE
    ...
    def test_replay_refuses_a_discharge_whose_landed_leg_is_UNPROVEN(self):  # noqa: VACUOUS_ASSERTION — the positive control is the sibling test: the SAME fixture with a landed close DOES close (same observable, opposite verdict), so this refusal is the landed leg and not a door that never opens
        ...
        build = self._build_row()
        opened = self._opened_a_second_ago()   # see the helper
        review, why = dispatches.add("codex-3", "review-lane", ref=self.b,
                                     repo=self.repo, kind="review",
                                     notify=False, supersedes=build["id"],
                                     _reason=True)
...
class SpiralGateKeysOnTheLedgersNameTest(unittest.TestCase):
    """A two-name probe, as a regression fixture.
