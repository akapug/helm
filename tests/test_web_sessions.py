...
class TestWebSessions(unittest.TestCase):
    ...
    def test_catalog_shape_and_refresh(self):
        ...
        for key in ("rows", "stats", "scanned_at"):
            self.assertIn(key, d, "predecessor /api/catalog shape: missing %s" % key)
    ...
    def test_search_scoped_grep_hides_synthetic(self):
        ...
        for key in ("harness", "id8", "date", "title", "snippet", "sid", "cvid", "syn"):
            self.assertIn(key, hit, "predecessor /api/search hit shape: missing %s" % key)
    ...
    def test_search_failure_keeps_production_api_shape_and_never_claims_empty(self):
        issue = {"stage": "session-list", "kind": "timeout",
                 "message": "scoped grep session list timed out"}
        with mock.patch.object(transcripts, "_grep_sessions", return_value={
        ...
        self.assertEqual(d["unavailable"], [issue])
        self.assertTrue(d["source"].startswith("scoped-grep"))
        self.assertEqual(d["scope"], "alpha")

    def test_search_unexpected_exception_is_structured_unavailable(self):
        with mock.patch.object(transcripts, "deep_search",
                               side_effect=OSError("forced search failure")):
            status, d = self.req("/api/search?q=marker")
        self.assertEqual(status, 200)
        self.assertEqual(d["hits"], [])
        self.assertIn("error", d)
        self.assertEqual(d["unavailable"][0]["stage"], "deep-search")
        self.assertEqual(d["unavailable"][0]["kind"], "exec")
    ...
    def test_cmd_omitted_account_degrades_to_default(self):
        # broken provider => no creds => "(default)" (the predecessor's degraded handling)
        status, d = self.req("/api/cmd?sid=%s" % SID_A[:12])
        self.assertEqual(status, 200, d)
        for key in ("cmd", "preflight", "warnings", "session"):
            self.assertIn(key, d, "predecessor /api/cmd shape: missing %s" % key)
