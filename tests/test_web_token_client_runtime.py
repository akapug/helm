...
class TestTokenClientRuntime(unittest.TestCase):
    ...
    def test_non_403_never_refreshes(self):
        ...
        # THE 400 THREW LOUDLY. Asked for the three tiers rather than the
        # whole string: this arm is about the RETRY PATH, and pinning the
        # rendering here blocked task/2650's cure while testing nothing the
        # scenario is about.
        msg = r["detail"]["message"]
        for tier in ("body wants a JSON object", "400", "/api/chat"):
    ...
    def test_homepost_error_object_contract(self):
        ...
        # The CONTRACT is an {error: ...} OBJECT and never a throw; the
        # sentence inside it carries all three tiers (task/2650). Asserting
        # the whole dict pinned the one-tier rendering.
        self.assertEqual(list(r["detail"]["badValue"]), ["error"],
                         "homePost must resolve with an {error: ...} object")
        for tier in ("forbidden (mutations always require the bearer token "
                     "the UI carries)", "403", "/api/homes"):
            self.assertIn(tier, r["detail"]["badValue"]["error"])
