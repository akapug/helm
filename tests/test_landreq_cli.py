#!/usr/bin/env python3
"""`helm.landreq_cli` — the VERB SURFACE of the land ledger.
...
class TheVerbSurfaceIsPublishedByTheLedgerTest(unittest.TestCase):
    """The declaration, the definition and the binding are one thing."""

    def _declared(self):
        for module, names in landreq._OWNER_NAMES:
            if module == "landreq_cli":
                return names
        self.fail("landreq_cli is not declared in landreq._OWNER_NAMES; without the "
    ...
    def test_the_published_object_IS_this_modules_object(self):
        ...
        self.assertIs(getattr(landreq, _ANCHOR), getattr(landreq_cli, _ANCHOR),
        ...
        same = sorted(n for n in declared
                      if getattr(landreq, n, None) is getattr(landreq_cli, n, None))
