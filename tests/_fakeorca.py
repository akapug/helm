...
class FakeDaemon:
    ...
    def __init__(self, tmp, reply=None, raw=None, token="tok", hang=False,
                 error=None):
        self.dir = tmp
        # The runtime metadata goes in `tmp` (the pinned userData); the socket
        # goes in tests.socket_dir(), because a path under TMPDIR can exceed
        # sun_path. The metadata names the endpoint, so nothing else moves.
        self.sock_path = os.path.join(socket_dir(), "d.sock")
