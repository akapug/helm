...
signature: one seat's drops cleared on restart and MAY involve stale inputs,
while another's high-rate defect-hunting drops persist on a fresh proxy and
track task shape. This feature promises only what identity proves: whether the running
...
class ProxyIdentityTest(StaleBase):
    ...
    def test_up_records_the_inputs_given_to_popen(self):
        ...
        with mock.patch.object(seat, "_running_pid", return_value=None), \
             ...
             mock.patch.object(seat, "_port_open", side_effect=[False, True, True]), \
