...
from helm import seat, seats_integrator, silent_drop
...
def nudge():
    ...
    proven by this nudge following the thinking-only prefix row (kimi mislabel
    fix): text that arrives WITHOUT a nudge is a normal continuation."""
    return json.dumps({
...
class SilentDropTest(unittest.TestCase):
    ...
    def test_thinking_prefix_continuation_is_NOT_a_drop(self):
        # the mislabel (kimi 2026-07-24): claude-code records a thinking-only
    ...
    def test_a_real_alert_still_fires_after_a_dry_run(self):
        """THE FILED BUG (fable QC 2026-07-29): a --dry-run stamped
    ...
    def test_alerts_post_to_helm_room_not_main(self):
        # owner 2026-07-24: silent-drop alerts in #main woke every #main-homed
...
def _roster(*names):
    """The live roster the integrator is resolved from, and nothing else."""
    return mock.patch("helm.seats_roster.roster_checked",
                      return_value=({n: {"seat": n} for n in names}, False))


class _integrator_env:
    """HELM_INTEGRATOR_SEAT set to NAME, or unset for None; restored after."""
    ...
    def __enter__(self):
        self.prior = {k: os.environ.pop(k, None) for k in self.KEYS}
        if self.name is not None:
            os.environ["HELM_INTEGRATOR_SEAT"] = self.name
...
class TheAlertAddressesTheResolvedIntegratorTest(unittest.TestCase):
    """The silent-drop alert names the integrator the roster resolves, never a
    seat name written into this module. A spelled name keeps being addressed
    after no seat carries it, and the alert reaches nobody while it reads as
    sent."""
...
class UnscannableTest(unittest.TestCase):
    ...
    MEASURED 2026-07-29: codex-2's newest transcript was 141 HOURS old and the
    watchdog called it clean every ninety seconds, straight through the week
    the owner kept reporting that codex was still dropping. Quiet and healthy
