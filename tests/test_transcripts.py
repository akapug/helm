...
class TranscriptsTest(unittest.TestCase):
    ...
    def test_scoped_search_dash_query_is_literal_not_a_flag(self):
        # a query beginning with '-' must reach the pinned grep as a PATTERN
        # (after `--`), never as a flag; both list and snippet stages must use
        # the real executable rather than a PATH-resolved shell function/shim.
        ...
        for call in run.call_args_list:
            ...
            self.assertEqual(argv[argv.index("--") + 1], "--evil-flag")

    def _forced_scoped_grep_failure(self, stage, exc):
        marker = "--planted-marker"
        self._plant_claude(SID_A, self.alpha, "the %s must hit" % marker)
        self._fresh_catalog()
        control = transcripts.deep_search(marker, scope="alpha")
        self.assertEqual([h["sid"] for h in control["hits"]], [SID_A],
                         "must-hit control did not reach the planted marker")
        self.assertIn(marker, control["hits"][0]["snippet"])
        real_run = subprocess.run

        def fail(argv, *args, **kwargs):
            is_list = "-rilF" in argv
            if (stage == "session-list") == is_list:
                raise exc
            return real_run(argv, *args, **kwargs)

        with mock.patch("subprocess.run", side_effect=fail):
            return transcripts.deep_search(marker, scope="alpha")

    def test_scoped_search_missing_catalog_path_is_unavailable_not_empty(self):
        path = self._plant_claude(SID_A, self.alpha, "the planted marker must hit")
        self._fresh_catalog()
        os.remove(path)
        res = transcripts.deep_search("planted marker", scope="alpha")
        self.assertEqual(res["hits"], [])
        self.assertIn("error", res)
        self.assertEqual(res["unavailable"][0]["stage"], "session-list")
        self.assertEqual(res["unavailable"][0]["kind"], "missing")
        self.assertEqual(res["unavailable"][0]["sid"], SID_A)

    def test_scoped_search_list_timeout_is_unavailable_not_empty(self):
        res = self._forced_scoped_grep_failure(
            "session-list", subprocess.TimeoutExpired(["grep"], 60))
        self.assertEqual(res["hits"], [])
        self.assertIn("error", res)
        self.assertEqual(res["unavailable"][0]["stage"], "session-list")
        self.assertEqual(res["unavailable"][0]["kind"], "timeout")

    def test_scoped_search_list_exec_oserror_is_unavailable_not_empty(self):
        res = self._forced_scoped_grep_failure(
            "session-list", OSError("forced exec failure"))
        self.assertEqual(res["hits"], [])
        self.assertIn("error", res)
        self.assertEqual(res["unavailable"][0]["stage"], "session-list")
        self.assertEqual(res["unavailable"][0]["kind"], "exec")

    def test_scoped_search_snippet_timeout_marks_partial_result_unavailable(self):
        res = self._forced_scoped_grep_failure(
            "snippet", subprocess.TimeoutExpired(["grep"], 10))
        self.assertEqual([h["sid"] for h in res["hits"]], [SID_A])
        self.assertIn("error", res)
        self.assertEqual(res["unavailable"][0]["stage"], "snippet")
        self.assertEqual(res["unavailable"][0]["kind"], "timeout")
        self.assertEqual(res["hits"][0]["snippet_unavailable"], res["unavailable"][0])
        self.assertIn("unavailable", res["hits"][0]["snippet"])

    def test_scoped_search_snippet_exec_oserror_marks_partial_result_unavailable(self):
        res = self._forced_scoped_grep_failure(
            "snippet", OSError("forced exec failure"))
        self.assertEqual([h["sid"] for h in res["hits"]], [SID_A])
        self.assertIn("error", res)
        self.assertEqual(res["unavailable"][0]["stage"], "snippet")
        self.assertEqual(res["unavailable"][0]["kind"], "exec")
    ...
    def test_cmd_search_propagates_unavailable_without_claiming_zero_hits(self):
        issue = {"stage": "session-list", "kind": "timeout",
                 "message": "scoped grep session list timed out"}
        err = io.StringIO()
        out = io.StringIO()
        with mock.patch.object(transcripts, "deep_search", return_value={
                "hits": [], "source": "scoped-grep (1 session in alpha)",
                "scope": "alpha", "unavailable": [issue],
                "error": "scoped transcript search unavailable"}), \
             contextlib.redirect_stdout(out), contextlib.redirect_stderr(err):
            rc = transcripts.cmd_search(["marker", "--scope", "alpha"])
        self.assertEqual(rc, 1)
        self.assertEqual(out.getvalue(), "")
        self.assertIn("unavailable", err.getvalue())
        self.assertNotIn("0 hits", out.getvalue() + err.getvalue())
