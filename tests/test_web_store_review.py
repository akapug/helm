...
class TestWebStoreReview(unittest.TestCase):
    ...
    def test_ui_review_markup_and_js(self):
        ...
        # the read is BOUNDED and both halves of the panel are rewritten on
        # every (re)load, so a hung fetch can never leave a stale count above
        # a permanent spinner: the header names the failure too
        self.assertIn('j("/api/store/review", SREV_TIMEOUT_MS)', body)
        self.assertIn('$("#storerevmeta").textContent = force ? "refreshing…" : "loading…"', body)
        self.assertIn('$("#storerevmeta").textContent = "✗ not loaded — "', body)
        # CONTROL: the unbounded spelling is gone
        self.assertNotIn('j("/api/store/review")', body)
