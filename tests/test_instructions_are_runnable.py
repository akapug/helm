...
def _subverbs(verb):
    ...
    constants used by those comparisons all count. Nested functions do not —
    their argv belongs to a different parser scope.
    """
    if verb in _SUBVERBS:
        return _SUBVERBS[verb]
    handler = _handler(verb)
    tree = ast.parse(textwrap.dedent(inspect.getsource(handler))).body[0]
    ...
    def add(comparator):
        if isinstance(comparator, ast.Constant) \
            ...
            value = handler.__globals__.get(comparator.id)
...
class InboxCureRunsTest(InstructionBase):
    ...
    def test_catchup_without_apply_parks_nothing(self):
        ...
        seats.join(session="s-inbox", seat="probe-seat", cwd=self.tmp)
        chat.post("@probe-seat please look at this", who="someone-else",
                  sign=False)
    ...
    def test_the_inbox_block_names_apply(self):
        ...
        mentioned it."""
        from helm import chat
        seats.join(session="s-block", seat="probe-seat", cwd=self.tmp)
        chat.post("@probe-seat this is an obligation", who="someone-else",
                  sign=False)
        blocks, _warns = seats.stop_guard(session="s-block", room="main",
                                          seat="probe-seat", cwd=self.tmp)
...
class PrintedCommandGrammarTest(InstructionBase):
    ...
    def test_non_token_grammar_misstatements_do_not_regrow(self):  # noqa: VACUOUS_ASSERTION — required forms are positive controls
        ...
        # PER FAMILY. @codex-3 broke my first version by adding
        ...
        for pat, kind in families:
            ...
            if kind == "family":
                ...
                # @codex-3's break (helm/meld_status.py carrying a forbidden
