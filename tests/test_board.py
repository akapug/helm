...
`def test_*(tmp_path)` functions. The gate runs `python -m unittest discover`,
which collects TestCase subclasses ONLY, so `python3 -m unittest tests.test_board`
...
class KeyOwnershipTest(BoardBase):
    ...
    (helm-claude-2's sharpening): two seats appending two findings is two
