...
@codex-2 bound a FIX: the reviewed commit added 27 production JS lines and
...
class TestNavRevealSource(unittest.TestCase):
    ...
    def test_the_badge_writer_re_reveals_the_active_tab(self):
        """navBadge changes the ACTIVE tab's width after showView already
        scrolled — measured at 390px, "?" and "99+" pushed the selected tab
        back out by ~41px. Without this call the reveal is correct and stale."""
        body = _extract_fn(web_ui_loader.read_text(), "navBadge")
        self.assertIn("revealActiveTab()", body)
        self.assertIn('classList.contains("on")', body)
    ...
    def test_showView_is_the_only_place_a_tab_becomes_selected(self):
        ...
        EXISTS. @codex-2: the harness never executes the initial `/#ledger`
...
class TestNavRevealRuntime(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        template = open(HARNESS, encoding="utf-8").read()
        ...
        fn = "\n\n".join(_extract_fn(src, n) for n in
                          ("revealActiveTab", "navBadge", "canonView", "showView"))
        ...
        # only way to bind REACHABILITY, which no source check can (@codex-2
    ...
    def test_a_tab_off_the_right_edge_is_revealed(self):
        """@codex-2's exact repro: strip 14..376, selected tab at content 640,
    ...
    def test_a_non_scrollable_strip_is_left_alone(self):  # noqa: VACUOUS_ASSERTION — the first assertion is an unconditional positive control on the SAME `moved` field from the SAME harness run (hidden_right moved); the rung cannot span the dict key, but a constant-False field or an unrun harness fails that line before reaching the absence assertions
        """Desktop: .navtabs is display:contents, so it generates no box and
        scrollWidth == clientWidth == 0. The reveal must return immediately
        rather than compute against a zero-width rect."""
        ...
        # assertion below while proving nothing about display:contents.
    ...
    def test_a_badge_that_widens_the_active_tab_re_reveals_it(self):
        """@codex-2: the badge arm was SOURCE-ONLY and order-blind. Moving the
    ...
    def test_booting_on_a_hash_view_reveals_its_tab(self):
        """THE ORIGINAL REPRO, EXECUTED. The page OPENS on #ledger with the
        selected tab off-screen; boot must leave it visible.

        This RUNS the real boot block rather than asserting about its text,
        because @codex-2 broke three string-level arms in a row — the last with
        ...
        # NOTHING IS PRE-SATISFIED (@codex-2's fifth finding): no tab is
        # selected before boot, so showView must do the selecting, and the
        # ledger tab really is off the strip's right edge to begin with.
        self.assertFalse(r["anySelectedBefore"], "fixture pre-selected a tab")
        self.assertTrue(r["ledgerWasOffscreen"], "fixture: ledger was not off-screen")
        self.assertEqual(r["selected"], "ledger", "showView did not select the hash view")
        self.assertTrue(r["visible"], "boot did not reveal the selected tab")
        self.assertGreater(r["scrollLeft"], 0)

    def test_a_badge_on_an_inactive_tab_scrolls_nothing(self):  # noqa: VACUOUS_ASSERTION — the arm above is the unconditional positive control on the same harness run: it proves navBadge CAN move the strip, so this stillness is the classList.contains("on") guard and not an inert call
        """MUST-MISS: only the ACTIVE tab's geometry can invalidate the reveal.
        Re-revealing on every badge would yank the strip whenever chat's unread
        count ticked, which is a worse defect than the one being fixed."""
        self.assertTrue(self.out["badge_widens_active"]["visible"])
