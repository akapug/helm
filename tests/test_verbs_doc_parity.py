...
On 2026-07-25 four shipped verbs — `capabilities`, `fleet`, `lr`,
...
class VerbsDocParityTest(unittest.TestCase):
    ...
    def test_seat_lifecycle_subverbs_stay_documented(self):  # noqa: VACUOUS_ASSERTION — exact section control plus exact nested grammar assertion; no loop can skip either
        self.assertIn("A note on `helm seat`", self.text,
                      "the canonical seat reference section moved")
        self.assertIn("seat lifecycle show|record", self.text)
