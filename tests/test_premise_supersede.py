...
class CaptureGuardTest(SupBase):
    ...
    def test_remint_over_tombstone_drops_the_stale_gloss(self):
        """codex round 3 of lane gloss-fires-full-entry-stays: a re-minted
        premise kept firing its OLD gloss beside NEW, contradictory text,
        because this path carried its own hard-coded stale-field copy and
        `gloss` was added only to the other one. The scrub now has ONE
        spelling (store STALE_ON_REMINT), shared with store add."""
        self.capture_v1()
        self.run_verb(premise.cmd_premise,
                      ["--supersede", "law-v1", "law-v2 | truth two"])
...
# a fixed stamp for the revise arms below; this module had no TS of its own
_STAGE_TS = "2026-08-06T09:00:00Z"


class StagedRevisionAcrossCaptureTest(SupBase):
    """T1: the cure shipped with NO ARM and was VACUOUS TO THE GATE.

    `helm premise` legally rewrites a LIVE statement whenever the entry is
    unattested — the orphaning guard above only refuses the ATTESTED case —
    and it carried `pending_revision` straight through, leaving a correction
    staged against text that no longer existed. `confirm` would then install a
    sharpening of a sentence nobody can read.

    I measured both directions with a throwaway probe and never turned it into
    a test, so ReviseLiveEntryTest passed identically at the parent commit
    where the bug still lived. A cure the suite cannot see is a cure the next
    refactor deletes for free.

    EXACTLY THE GLOSS RULE ONE FIELD OVER (see its sibling arm above): a gloss
    is DERIVED FROM the statement and is dropped rather than guessed at when
    the statement changes; a staged revision is a proposed REPLACEMENT for the
    statement, a stronger dependency, so it cannot survive what a gloss does
    not.

    RE-RUN THE DISCRIMINATION WITHOUT A SHA. The evidence that these arms see
    the cure at all was first recorded as "revert helm/premise/_capture.py to
    <parent>", and a rebase turns that parent into an object the branch no
    longer reaches — a recipe nobody can replay is the same failure as a probe
    left outside the repository. The mutation stated by CONTENT survives any
    rebase: in `_capture_store`, delete the two-line guard

        if canonicalize(e.get("statement")) != canonicalize(statement):
            e.pop("pending_revision", None)

    which is the whole cure. test_a_CHANGED_statement_DROPS_the_staged_revision
    then FAILS with the defect verbatim — the staged dict is still present
    after the statement it was written against was rewritten — and passes again
    when the guard is restored. test_an_UNCHANGED_statement_KEEPS_it stays
    GREEN across that mutation BY DESIGN: it is the control against an
    over-broad cure that drops the stage on every write, so it is the arm that
    would go red if someone widened the guard, not narrowed it. Clear
    __pycache__ between the two runs; a same-second edit of an equal-length
    file leaves a stale .pyc and the measurement means nothing."""

    def live_unattested(self, pid="stage-law", text="the original mechanism holds"):
        """A LIVE entry with NO attestation, BUILT THROUGH THE PUBLIC WRITER.

        capture_v1 attests, and an attested in-place edit is refused earlier by
        the orphaning guard, so an arm built on it is green while never
        reaching the code under test.

        MY FIRST CUT ATTESTED AND THEN STRIPPED THE attest_* KEYS, and a review
        named exactly what that costs: it proves a HAND-SHAPED EQUIVALENT, not
        REACHABILITY. A state I assembled by deleting fields is not evidence
        that helm can produce it, so the arm could have been defending a
        configuration no writer ever emits. `--no-attest` is the real public
        path (its own arm, test_never_attested_old_starts_chain_honestly, uses
        it), and the preconditions are asserted rather than assumed."""
        rc, _out, err = self.run_verb(premise.cmd_premise,
                                      ["%s | %s" % (pid, text), "--no-attest"])
        self.assertEqual(rc, 0, err)
        path = os.path.join(home.global_dir(), "premises", "prior-%s.md" % pid)
        raw = self.entry_raw(pid)
        self.assertIn("  status: live", raw)
        for k in ("attest_record", "attest_payload", "attest_ts", "attest_by"):
            self.assertNotIn("  %s:" % k, raw,
                             "--no-attest left %s behind, so this fixture is "
                             "not the unattested state it claims" % k)
        return pid, path

    def staged(self, pid):
        return (store._pick_candidate(pid)[0] or {}).get("pending_revision")

    def test_a_CHANGED_statement_DROPS_the_staged_revision(self):  # noqa: VACUOUS_ASSERTION — the unconditional positive control is three lines above the absence, on the SAME field: assertEqual(self.staged(pid).get('statement'), <literal>) proves the stage EXISTS and names WHICH correction it holds, before the edit that must drop it. The rung reads it as a different observable because of the .get(); it is the same value one accessor down, and asserting the whole dict equal to a literal would pin a timestamp this test does not own. A positive on the identical expression is impossible here: `staged(pid)` being falsy IS the fact under test.
        pid, _path = self.live_unattested()
        _r, err = store.revise(pid, _STAGE_TS, "the original mechanism holds, clause fixed")
        self.assertIsNone(err, err)
        # UNCONDITIONAL POSITIVE CONTROL ON THE SAME OBSERVABLE, and an
        # EQUALITY against a literal rather than an isinstance: the vacuity
        # rung is right that "it is a dict" is not proof of presence, and the
        # stronger form also pins WHICH correction was staged — so the
        # assertFalse below means "this exact stage was dropped" rather than
        # "some falsy value is there now".
        self.assertEqual(self.staged(pid).get("statement"),
                         "the original mechanism holds, clause fixed")
        rc, _out, err = self.run_verb(
            premise.cmd_premise, ["%s | a COMPLETELY different mechanism now" % pid])
        self.assertEqual(rc, 0, err)
        self.assertIn("a COMPLETELY different mechanism now", self.entry_raw(pid))
        self.assertFalse(self.staged(pid),
                         "a correction staged against the OLD statement "
                         "survived a rewrite of that statement")

    def test_an_UNCHANGED_statement_KEEPS_it(self):  # noqa: VACUOUS_ASSERTION — this arm asserts PRESENCE, not absence: the staged statement must equal a non-empty literal after the re-mint. The `or {}` the rung reads as an absence handler is there so a DROPPED stage fails on the equality with a readable message rather than raising AttributeError on None. Its sibling arm above is the negative half, and the re-mint's own change (widget alpha in the entry) is asserted too, so this cannot pass on a write that did nothing.
        """The half that makes the cure usable rather than merely safe. Without
        this arm, dropping the stage on EVERY premise write would pass the test
        above while making `revise` useless on exactly the entries that get
        re-minted most — and re-minting is a routine agent lane."""
        pid, _path = self.live_unattested()
        _r, err = store.revise(pid, _STAGE_TS, "the original mechanism holds, clause fixed")
        self.assertIsNone(err, err)
        rc, _out, err = self.run_verb(
            premise.cmd_premise,
            ["%s | the original mechanism holds | widget alpha,widget beta" % pid])
        self.assertEqual(rc, 0, err)
        keep = self.staged(pid) or {}
        self.assertEqual(keep.get("statement"),
                         "the original mechanism holds, clause fixed",
                         "a re-mint of the SAME sentence discarded a stage "
                         "that nothing had invalidated")
        # and the re-mint's OWN change landed, so this is not a no-op write
        self.assertIn("widget alpha", self.entry_raw(pid))
