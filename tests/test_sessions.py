...
class AdversarialIdentityTest(unittest.TestCase):
    """The two classes a read-only review pass confirmed uncovered: a recycled
...
class SnapshotScopeTest(unittest.TestCase):
    """`sessions.snapshot()` — resolve many sessions against ONE reading.

    The defect it cures: `credhome_for` indexes into three WHOLE-MACHINE reads
    that do not depend on the sid asked about, and a caller resolving a page
    repeated all three per row. The live-pane read walks every process on the
    box, so the repetition was not merely the caller's cost — it is pure CPU and
    holds the GIL for every other thread in the process.

    Every count assertion below is paired with the UNSCOPED count in the same
    test, so a counter that could never rise above one would fail the pair."""

    def setUp(self):
        self.tmp = tempfile.mkdtemp()
        self.calls = {"live": 0, "homes": 0, "bind": 0}

        def live():
            self.calls["live"] += 1
            return {"sid-live": 4242}
        ...
        def bind():
            self.calls["bind"] += 1
            return {"sid-latched": (os.path.join(self.tmp, "home"), "environ")}

        self.p = [mock.patch.object(sessions, "_read_live_sids", live),
                  mock.patch.object(sessions, "_read_cred_homes", homes),
                  mock.patch.object(sessions, "_read_bindings", bind),
                  mock.patch.object(sessions, "BINDINGS",
                                    os.path.join(self.tmp, "b.tsv"))]
        for p in self.p:
            p.start()
    ...
    def test_a_scope_reads_each_input_once_and_unscoped_reads_them_every_time(self):
        for _ in range(8):
            sessions.live_sids()
            sessions.cred_homes()
            sessions._bindings()
        self.assertEqual(self.calls, {"live": 8, "homes": 8, "bind": 8})
        self.calls.update(live=0, homes=0, bind=0)
        with sessions.snapshot():
            for _ in range(8):
                sessions.live_sids()
                sessions.cred_homes()
                sessions._bindings()
        self.assertEqual(self.calls, {"live": 1, "homes": 1, "bind": 1})

    def test_the_scope_closes_and_the_next_caller_reads_again(self):
        with sessions.snapshot():
            sessions.live_sids()
        self.assertEqual(self.calls["live"], 1)
        sessions.live_sids()
        self.assertEqual(self.calls["live"], 2)

    def test_a_nested_scope_joins_the_outer_reading(self):
        with sessions.snapshot():
            sessions.live_sids()
            with sessions.snapshot():
                sessions.live_sids()
            sessions.live_sids()          # outer still open after the inner exits
        self.assertEqual(self.calls["live"], 1)

    def test_a_scope_is_not_shared_with_another_thread(self):
        import threading
        seen = []

        def other():
            sessions.live_sids()
            seen.append(getattr(sessions._scope, "box", None))
        ...
        with sessions.snapshot():
            ...
            t.start()
            t.join()
        self.assertEqual(seen, [None])     # the other thread saw no scope
        self.assertEqual(self.calls["live"], 2)   # so it did its own read

    def test_a_write_inside_a_scope_is_visible_to_the_rest_of_it(self):
        # without carrying the append into the memo, the second record re-reads
        # the pre-append world, appends a duplicate row, and counts it as new
        with sessions.snapshot():
            self.assertTrue(sessions.record_binding("sid-new", "/h/A", "environ"))
            self.assertFalse(sessions.record_binding("sid-new", "/h/A", "environ"))
            self.assertEqual(sessions._bindings()["sid-new"], ("/h/A", "environ"))
        with open(sessions.BINDINGS) as f:
            rows = [ln for ln in f if ln.strip()]
        self.assertEqual(len(rows), 1)
        # positive control on the SAME observable: a different sid does append
        with sessions.snapshot():
            self.assertTrue(sessions.record_binding("sid-two", "/h/A", "environ"))
        with open(sessions.BINDINGS) as f:
            self.assertEqual(len([ln for ln in f if ln.strip()]), 2)

    def test_scoped_and_unscoped_resolve_to_the_same_home(self):  # noqa: VACUOUS_ASSERTION — the lone absence arm is controlled by the three real-home arms beside it, read out of the same mapping the same loop filled in the same call
        home = os.path.join(self.tmp, "home")
        os.makedirs(os.path.join(home, "session-env"), exist_ok=True)
        open(os.path.join(home, "session-env", "sid-env"), "w").close()
        seen = {}
        for sid in ("sid-live", "sid-latched", "sid-env", "sid-absent"):
            with mock.patch.object(sessions, "proc_home", return_value=home):
                loose = sessions.credhome_for(sid, latch=False)
                with sessions.snapshot():
                    tight = sessions.credhome_for(sid, latch=False)
            self.assertEqual(loose, tight, sid)
            seen[sid] = tight
        # UNCONDITIONAL POSITIVE CONTROL on the same observable: the agreement
        # above is vacuous if every rung answers None, so name what each rung
        # actually returned. One sid per rung resolves to a REAL home — the
        # live pane, the latch index and the session-env claim — and only the
        # sid no rung can answer is absent.
        self.assertEqual(seen["sid-live"], home)
        self.assertEqual(seen["sid-latched"], home)
        self.assertEqual(seen["sid-env"], home)
        self.assertIsNone(seen["sid-absent"])


class SessionsRouteScopeTest(unittest.TestCase):
    """The console's sessions page mints one resume command per row, and every
    one of them has to name a credential home. The route must resolve the whole
    page against one reading rather than one per row."""

    def _rows(self, n):
        return [_row("sid-%04d" % k) for k in range(n)]

    def test_the_route_reads_the_live_pane_map_once_for_the_whole_page(self):  # noqa: ORPHANED_MOCK — the page loop reaches these doubles through the `_memo` indirection the walker does not model, and this test COUNTS the calls, so a double that never fired would fail its own assertion
        from helm import web_core
        calls = []
        with mock.patch.object(sessions, "_read_live_sids",
                               lambda: calls.append(1) or {}), \
             mock.patch.object(sessions, "_read_cred_homes", lambda: []), \
             mock.patch.object(sessions, "_read_bindings", lambda: {}), \
             mock.patch.object(sessions, "rows_for",
                               lambda **kw: self._rows(25)):
            out = web_core._api_sessions()
            scoped = len(calls)
            # POSITIVE CONTROL, same counter, same stubs: the pre-cure loop
            # shape, so a counter that could not rise above one would fail here
            del calls[:]
            for r in sessions.rows_for(limit=25):
                sessions.resume_command(r)
            unscoped = len(calls)
        self.assertEqual(len(out["sessions"]), 25)
        self.assertEqual(scoped, 1)
        self.assertEqual(unscoped, 25)

    def test_the_scoped_page_is_field_for_field_what_the_unscoped_loop_built(self):  # noqa: ORPHANED_MOCK — the page loop reaches these doubles through the `_memo` indirection the walker does not model, and this test COUNTS the calls, so a double that never fired would fail its own assertion
        from helm import web_core
        rows = self._rows(25)
        home = tempfile.mkdtemp()
        with mock.patch.object(sessions, "_read_live_sids", lambda: {}), \
             mock.patch.object(sessions, "_read_cred_homes", lambda: [home]), \
             mock.patch.object(sessions, "_read_bindings",
                               lambda: {r["i"]: (home, "environ") for r in rows}), \
             mock.patch.object(sessions, "rows_for", lambda **kw: rows):
            scoped = web_core._api_sessions()["sessions"]
            loose = [dict({k: r.get(k) for k in web_core.SESSION_KEYS},
                          cmd=sessions.resume_command(r)) for r in rows]
        self.assertEqual(scoped, loose)
        self.assertTrue(all(home in r["cmd"] for r in scoped))   # not vacuous
        self.assertNotEqual(scoped, loose[:-1])                  # comparator works
