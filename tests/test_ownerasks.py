...
class ClarityRefusesBeforeTheWriteTest(AsksBase):
    """task/353, filed by me after tripping it myself. The clarity die ran
    AFTER the append, so a 103-word ask printed "max 20 in owner mode" and
    landed anyway — and this ledger has no edit, amend or supersede verb, so
    the row sat on the owner's queue permanently in the form the guard had
    just said he could not read. There was no cure available after the write.
    """
    ...
    def test_a_LIBRARY_caller_cannot_file_a_row_the_owner_cannot_read(self):
        """task/373 — the guard must sit on the WRITE, not on one door.
    ...
    def test_hold_kind_is_typed_at_the_write_boundary(self):
        row, err = ownerasks.add("relogin", needs="owner device auth",
                                 hold_kind="usage-reset")
        self.assertIsNone(err)
        self.assertEqual(row["hold_kind"], "usage-reset")
        refused, err = ownerasks.add("relogin", needs="owner device auth",
                                     hold_kind="device auth from prose")
        self.assertIsNone(refused)
        self.assertIn("hold kind must be one of", err)
        self.assertEqual(len(ownerasks.rows()), 1)
    ...
    def test_an_empty_ask_and_a_dead_ledger_do_not_report_the_same_reason(self):
        ...
        "the disk declined the write" indistinguishable to every caller — the
        exact conflation this seat bound FIX on twice in one night in other
        seats' code, and then shipped here under cover of the module's
        never-raise law. UNWRITABLE is the only one compared by VALUE, because
        it is the only one that is an operational fault rather than input."""
...
class NeedsRequiredTest(AsksBase):
    ...
    Measured 2026-07-26: the live queue held 19 open asks, the oldest four days
    ...
    human can supply. The hold kind is separately typed at the writer; ordinary
    owner input is the default and usage-reset must be explicit.
    """
    ...
    def test_a_named_human_only_input_records_AND_renders(self):
        rc, _out, err = self.asks("add", "reconnect the Linear connector",
                                  "--needs", "Daria's Linear OAuth login")
        self.assertEqual(rc, 0, err)
        rc, out, _ = self.asks("list")
        self.assertIn("reconnect the Linear connector", out)
        ...
        self.assertIn("needs: Daria's Linear OAuth login", out)

    def test_usage_reset_kind_is_explicit_at_the_cli(self):
        rc, _out, err = self.asks("add", "relogin codex", "--needs",
                                  "owner device auth", "--kind", "usage-reset")
        self.assertEqual(rc, 0, err)
        row = next(iter(ownerasks.rows().values()))
        self.assertEqual(row["hold_kind"], "usage-reset")
