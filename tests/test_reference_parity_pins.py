...
class HooksEstatePinTest(unittest.TestCase):
    ...
    def test_the_stated_estate_count_is_the_specs_length(self):
        ...
        # law, verbatim): "9 entries per home" moves when SPECS does.
        self.assertIn("%d entries per home" % len(hooks.SPECS),
                      _doc("HOOKS.md"))
    ...
    def test_the_gate_paragraph_names_every_gate_and_the_rc124_leg(self):
        ...
        # and the doc's arms are the CODE's arms — a leg added to spec_command
        # without touching this paragraph fails here.
        gate_cmd = hooks.spec_command(gates[0])
        for arm in ("2)", "124)", "127)", "*)"):
            self.assertIn(arm, gate_cmd,
                          "the doc pins an arm the generated gate lost")
