...
class DeadReflexTest(BehaviorBase):
    ...
    def test_a_nested_spawn_reflex_is_not_ledger_dead(self):
        """task/2971: a nested-spawn reflex is said at SubagentStart and
        never rides the inject ledger, so zero ledger fires says nothing
        about it."""
        self.live("rb")
        reflex.write({"id": "fanout", "steer": "steer-fanout",
                      "signal": "nested-spawn", "stated_ts": TS})
        self.plant([self.turn()] * 200)
        what = self.dead_props()[0][1]
        self.assertIn("1 live reflex with zero ledger fires", what)  # control
        self.assertNotIn("fanout", what)

    def test_an_act_reflex_is_not_ledger_dead(self):
        """task/2980: an act reflex is said by argv-guard at the act and
        never rides the inject ledger, so zero ledger fires says nothing
        about it — evolve must not propose retiring it."""
        self.live("rb")
        reflex.write({"id": "at-the-act", "steer": "steer-act",
                      "signal": reflex.ACT_SIGNAL, "stated_ts": TS})
        self.plant([self.turn()] * 200)
        what = self.dead_props()[0][1]
        self.assertIn("1 live reflex with zero ledger fires", what)  # control
        self.assertNotIn("at-the-act", what)
