...
class IdentityTest(unittest.TestCase):
    ...
    def test_a_HISTORY_sid_does_not_authorize_an_address(self):
        """Repro 1/2 on dispatch 1b4039cc, reproduced at exact tip
    ...
    def test_a_roster_with_HISTORY_BUT_NO_CURRENT_sid_authorizes_nothing(self):
        """The round-3 finding 2 (dispatch a4051c69), reproduced at
...
class DeliveryTest(unittest.TestCase):
    ...
    class Adapter(harness._CLIAdapter):
        ...
        The round-3 finding 1 — it rested on pid equality across two
...
class ChildArgvTest(unittest.TestCase):
    ...
    def test_the_child_parses_the_stamp_back_out_of_argv(self):
        ...
        def fake_child(seat_name, session, text, delay, adapter=None,
                       pids=None, transcript=None, since=None,
                       record_key=None, attempt=None, owed_room=None):
            # THE DOUBLE MUST ACCEPT WHAT PRODUCTION PASSES, and this is the
            # THIRD time a whole-suite gate has said so here: the withdrawal
            # evidence crosses this call, then the parent's episode namespace
            # (`record_key`), then the SPELL the child was launched for, and
            # now the ROOM the alarm was raised on (`owed_room`). A
            # double whose signature stops short turns a wiring change into a
            # TypeError that reads as an unrelated crash. It is written out
            # rather than swallowed by **kwargs deliberately — a double that
            # accepts anything cannot tell you the wiring changed at all, and
            # this arm exists to notice.
            seen["pids"] = pids
            seen["transcript"] = transcript
            seen["since"] = since
            seen["record_key"] = record_key
            seen["attempt"] = attempt
            seen["owed_room"] = owed_room
            return "resumed", "ok"
        ...
        # NO --record-key ON THIS ARGV MEANS THE COMPACTION KEY, which is the
        # right default for this leg: the adopted-pane deliver is the
        # compaction child, and only a caller with its OWN episode namespace
        # passes one.
        self.assertIsNone(seen["record_key"])
        # AND NO --attempt EITHER: a compaction child closes no repair episode,
        # so it names none.
        self.assertIsNone(seen["attempt"])
        # AND NO --owed-room: the compaction child answers about the home
        # room, and only a deaf-leg child pinned to its alarm's room carries one.
        self.assertIsNone(seen["owed_room"])
