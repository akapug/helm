...
review of that exact file. A seat found the first one only because a
...
def _redefined_names(path):
    """[(name, count)] for each top-level function or class that `path`
    defines more than once. Python keeps the LAST definition, so an earlier
    one is silently shadowed and every caller that still passes its argument
    list breaks only when it runs."""
    import ast
    import collections
    ...
    seen = collections.Counter(
        ...
        if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef,
                             ast.ClassDef)))
    return sorted((name, n) for name, n in seen.items() if n > 1)


class NoTopLevelRedefinitionTest(unittest.TestCase):
    """A module defines each top-level function and class once. A second
    `def` of the same name in one file shadows the first for every caller in
    the tree, and a focused run chosen by the lane's own symbols never selects
    the first definition's consumers: two lanes each adding a helper with a
    common name reach the whole-suite gate before anything notices."""
    ...
    def test_the_detector_finds_a_planted_redefinition(self):
        import tempfile
        ...
        self.assertEqual(_redefined_names(fh.name), [("f", 2)])
