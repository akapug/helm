#!/usr/bin/env python3
"""What a RESTART is obliged to restore (two obligations, one seam).

THE WAKE PATH. A seat's inbox beacon is a PER-SESSION Monitor process:
...
SessionStart arm directive. Measured: six of seven restored seats
...
THE CWD. Resume respawned at whatever cwd the newest transcript
...
# ---------------------------------------------------------------------------
# a restart re-arms the wake path or refuses to call itself complete
# ---------------------------------------------------------------------------
...
class ResumeRearmsTheWakePathTest(ResumeFixture):
    ...
    def test_the_rearm_rides_the_boot_grace_knob(self):
        ...
        self.assertEqual(rc, 0, err)
        zzz.assert_called_once_with(0.01)
...
# ---------------------------------------------------------------------------
# --cwd overrides; a stale recorded cwd refuses loudly
# ---------------------------------------------------------------------------
...
class ResumeCwdOverrideTest(ResumeFixture):
    ...
    def test_cwd_override_wins_over_the_sniffed_cwd(self):  # noqa: VACUOUS_ASSERTION — positive controls on the spawned cwd and the spawn.json worktree; mutation 4 (override ignored) reddens both
        ...
        # exactly that during a mutation pass (it refreshed a live seat's
        # home). A test must fail RED, never reach live state.
