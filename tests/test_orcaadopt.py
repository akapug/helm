...
from helm import harness, orcaadopt        # noqa: E402
        ...
        with open(os.path.join(self.tmp, "orca-runtime.json"), "w") as f:
            json.dump({"runtimeId": "rt1", "authToken": "t",
                       "transports": [{"kind": "unix",
                                       "endpoint": os.path.join(self.tmp,
                                                                "gone.sock")}]},
                      f)
        result, err = self.ad.rpc("terminal.list", {})
        self.assertIsNone(result)
...
class SessionJoinTest(unittest.TestCase):
    ...
    MEASURED 2026-07-30: helm-claude-2, the seat driving fleet recovery, showed
    ...
    def test_another_seats_session_is_never_claimed(self):
        ...
        # injection: the 2026-07-22 incident where a rebrief hit codex-3).
...
class SessionAddressingRefusalTest(unittest.TestCase):
    ...
    REPRODUCED 2026-07-30 against the first draft of the join (cross-family FIX,
    dispatch 9f04b633): a pane declaring HELM_CHAT_NAME=codex-3 and resumed on a
    sid still listed in helm-claude-2's roster HISTORY was folded in, supplied
    the handle, and send_to_pane injected /compact into codex-3 and returned
    ...
    def test_the_handle_binds_to_the_AUTHORIZED_pid_not_the_first_in_order(self):
        """@codex's scenario 1 on dispatch 9f04b633, the MIRROR of the shape
...
class SendTimeSnapshotTest(unittest.TestCase):
    ...
    @codex's repro 2/2 on dispatch 1b4039cc, reproduced at the exact lane tip.
...
_SEND_AUTHORITIES = {
    ...
    ("seat.py", "_spawn"): (
        "fresh-spawn", "onboarding into the pane ad.spawn() just returned"),
    ("seat.py", "_resume"): (
        ...
        "process, and only an injected turn can re-arm it)"),
    ...
    ("harness.py", "submit"): (
        "delegated", "the TURN verb itself — it owns the split (text, settle, "
        "bare Enter) and the read-back, and types into whatever handle its "
        "caller already proved; every entry above is one of those callers"),
}
...
# MEASURED, not estimated: 11 — autocompact's three /clear+/compact sends,
# harness.submit's OWN two legs (the text, then the bare Enter), sessions'
# resume kick, and the four turn-verb callers (both adopted legs,
# resumeturn.deliver, seat._spawn, seat._resume).
_SEND_SITES = 11
...
def _helm_sources(root):
    ...
    work/ (@codex round 3, finding 3). A new `ad.send` in any of them was
...
class SendSiteCensusTest(unittest.TestCase):
    ...
    def test_every_ADOPTED_send_routes_through_the_one_primitive(self):
        ...
        for mod, fn, lineno, src, _params in _send_sites():
            ...
            self.assertIn("authorized_handle(", src,
                          ...
                          % (mod, fn, lineno))
        self.assertEqual(checked, 2, "the adopted send sites moved; the census "
                         "must still cover both addressing modes")
    ...
    def test_the_census_REACHES_A_NESTED_PACKAGE(self):
        ...
        same hole. @codex's round-3 finding 3: the census walked `os.listdir`,
...
class ProcIncarnationTest(unittest.TestCase):
    ...
    @codex's round-3 finding 1 (dispatch a4051c69), reproduced at the lane tip:
    ...
    def test_a_RECYCLED_pid_REFUSES_instead_of_matching(self):
        """THE ADVERSARIAL MUST-HIT, and it is @codex's shape exactly. Snapshot
...
class RosterIdentityTest(unittest.TestCase):
    ...
    agent's pane, silently. Measured on the live roster 2026-07-30: codex
    carried 7 historical sids and opus-integrator 8.
    """
...
class _TearingProc:
    ...
    # gone (measured 2026-07-30, argv `--resume "helm coordinator 7-22"`). A
...
class TornSnapshotTest(unittest.TestCase):
    ...
    @codex's round-4 finding (dispatch ad157755, reproduced at the lane tip):
...
class CommOutsideTheWindowTest(unittest.TestCase):
    ...
    @codex's round-5 finding (dispatch ad2aed0f, reproduced at the lane tip):
    ...
    def test_a_NON_CLAUDE_pid_still_costs_ZERO_stat_reads(self):
        """The performance invariant round 5 bought and this must not sell:
...
class UnreadableIsNotGoneTest(unittest.TestCase):
    ...
    @codex's round-7 finding (dispatch 90845108): "Inner
    ...
    def test_a_WALK_THAT_NEVER_RAN_is_blind_not_an_empty_host(self):
        ...
        the same fail-open, one level up from where round 7 found it.
...
class OneBracketTest(unittest.TestCase):
    ...
    def test_every_proc_read_lives_inside_the_bracket(self):
        ...
        self.assertEqual(
            ...
            "errno split, which is round 7: %r" % (outside,))
...
class UnreadableRivalTest(unittest.TestCase):
    ...
    @codex's round-7 finding (dispatch 90845108): "Still
    ...
    Rounds 4-6 made the collector HONEST — it now reports the pids it could not
...
# nothing about which returns it guards — @codex's round 8, exactly.
#
...
def _p_live_seats(blind):
    ...
    with mock.patch.object(orcaadopt, "claude_processes",
                           return_value=_census(blind, procs)):
        _names, flag = proxywatch._live_seats()
...
class CensusConsumerCensusTest(unittest.TestCase):
    ...
    guard built in round 7 to end that sequence checked SPELLING, so it passed
...
class ResumeTokenShapeTest(unittest.TestCase):
    ...
    MEASURED 2026-07-30. opus-integrator compacted and rung 2 posted to the
    room: "every addressable process for opus-integrator holds a different
...
class PaneRowsWalksTheSessionJoin(unittest.TestCase):
    ...
    MEASURED 2026-08-04: `seat where helm-claude` answered `orca-adopted — LIVE`
    ...
    def test_a_process_declaring_another_seat_names_no_pane(self):
        ...
        out, _ = orcaadopt.pane_rows(
            ...
            procs=[_proc(1, seat="codex-3", pane_key="pk-a",
                         resume_sid="sid-a")],
            unreadable=[], roster_sids={"helm-claude": "sid-a"})
        # It is codex-3's by its own declaration, and NEVER helm-claude's.
