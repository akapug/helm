...
class RefusalPrescribesTheRunnableClaimFormTest(RungBase):
    ...
    def work_cmd(self, *args):
        ...
        # A LANE LEASE IS ACTOR-ATTRIBUTED (task/994): the identity layer
        # refuses a name minted from session+cwd, which is what a hermetic
        # fixture resolves to. `--seat S` names the seat this call acts as;
        # the bare-positional arm below gets the default and still proves its
        # point, because a POSITIONAL seat binds nothing either way.
