...
**Live composition + watchdog verbs:**
...
**BETTER, WHEN AVAILABLE — `lsof /dev/pts/N`**: lists the
...
```
seat-a  env term_<old> -> term_<new>   double-blind, see below
seat-b  env term_<old> -> term_<new>
seat-c  env term_<old> -> term_<new>
seat-d  env term_<old> -> term_<new>   re-run clean after the
...
Fresh helm-native write (a prior harness's maintenance and cred-rescue
runbooks are the ancestors — reboot tiers, monitored-pair relaunch,
cred/session decoupling all inherited as principles; their verbs were
harness-specific so none port literally). Seat-level mechanics: the
intervention ladder in §2 above.
