...
4. **Capture immediately.** Update the plan, PRD, task, or chat thread. Do not
...
When refinement involves another seat, ask in chat rather than through terminal
state:

```bash
helm chat post "@<peer> REFINE request: <subject, invariant, options, recommendation, open fork>"
```

The reply should cite the request's row id. Capture the resolved fork in the
artifact and, when useful, post a compact receipt in the same room.

Read `helm work list` and `helm chat claims` before convening if ownership or
claims matter.

## Success Criteria

- The highest-leverage ambiguity was resolved, not merely discussed.
- The living artifact or chat thread was updated with the convergence.
...
- `decision-spirit` when the fork is technical composition risk, and
  `council-of-models` for a cross-family read of it.
