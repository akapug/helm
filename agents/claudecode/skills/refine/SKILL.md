...
description: >
  ...
  shape in the living artifact or MC message before building.
license: MIT
metadata:
  author: mc
  version: "1.0.0"
...
4. **Capture immediately.** Update the plan, PRD, task, or MC message. Do not
...
## MC Coordination Form

When refinement involves another cell, use MC comms rather than TUI state:

```bash
mc comms send --from pane:<you> --to pane:<peer> --conv <lane> \
  --priority action-required \
  --payload '<REFINE request: subject, invariant, options, recommendation, open fork>'
```

The reply should cite the request message or seq. Capture the resolved fork in the
artifact and, when useful, post a compact receipt on the same conv.

Use `mc cockpit get --ambient` before convening if ownership or claims matter.

## Success Criteria

- The highest-leverage ambiguity was resolved, not merely discussed.
- The living artifact or MC message was updated with the convergence.
