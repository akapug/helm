#!/usr/bin/env bash
# Deterministic guard tests for scripts/mc-recall.sh — the safety-critical invariant is that EVERY
...
MC_RECALL_ECHO=1 "$SUT" "memgc garbage collection" >"$tmpout" 2>/dev/null
...
MC_RECALL_ECHO=1 MC_RECALL_MEMORY_MAX=2G "$SUT" "q" >"$tmpout" 2>/dev/null
...
MC_RECALL_ECHO=1 CLUSTERVISION_HOME="$sem" "$SUT" "q" >"$tmpout" 2>/dev/null
grep_out "semantic on when index present" "--semantic"
MC_RECALL_ECHO=1 CLUSTERVISION_HOME="$sem/nope" "$SUT" "q" >"$tmpout" 2>/dev/null
if grep -qF -- "--semantic" "$tmpout"; then echo "FAIL: keyword fallback when no index"; fails=$((fails+1)); fi
rm -rf "$sem"

# 4. arg validation
check "missing query exits 2" 2 env MC_RECALL_ECHO=0 "$SUT"
...
MC_RECALL_ECHO=1 "$SUT" --limit 5 q >"$tmpout" 2>/dev/null
...
MC_RECALL_CV_BIN=/bin/false "$SUT" --scope global --limit 2 "anything" >/dev/null 2>"$tmpout"; got=$?
...
if [ "$fails" -eq 0 ]; then echo "mc-recall guard tests: PASS"; else echo "mc-recall guard tests: $fails FAILURE(S)"; fi
exit "$fails"
