...
1. **Ground the objective.** Read the source: user message, PRD, issue, task, or
   chat record. Quote the actual requested outcome.
...
3. **Make each exit surfaceable.** The final report must show evidence the
   ...
   response, chat row, or commit sha.
4. **Attach real-surface verification.**
   ...
   - Coordination: prove the send, its delivery (`helm chat receipts`), the
     pending state, and the recipient's reply, as appropriate.
...
Sub-goals:
- <id>: <exit condition>; owner=<seat>; deps=<ids>; verify=<gate>; stop=<blocker>
...
- `helm dispatch` for handing a sub-goal to another seat.
- `xchk` for claim verification before declaring done.
