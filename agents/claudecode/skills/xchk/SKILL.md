...
description: >
  ...
  conflict. Given a topic or claim, /xchk runs one disciplined sweep across web
  or primary sources, local docs, memory, implementation, git history, chat
  messages and live state, and cross-family refutation. It returns a cited verdict
...
a cited verdict, not a file dump. helm's state is readable from the command line:
use local files, git, `helm who`, `helm work list`, and `helm chat read` as
sources; do not depend on a client view.
...
| 6 | chat / live state | relevant `helm chat read` or `helm chat pending` row cited by row id, or a `helm who` / `helm work list` reading, or `NO_CHAT_RECORD` |
| 7 | cross-family refute | a verdict from a different model family (a dispatched review or `council-of-models`), or `CROSS_FAMILY DEGRADED: <reason>` |
...
- chat: <row id or live reading / NO_CHAT_RECORD>
- cross-family: <verdict / CROSS_FAMILY DEGRADED: ...>
...
- Coordination facts come from `helm` reads (chat, claims, roster), not terminal
  state.

## Cross-Refs

- `council-of-models` for the cross-family refute.
