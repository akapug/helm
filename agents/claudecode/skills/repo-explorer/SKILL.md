...
5. **Check active work.** Claims, worktrees, dirty state, and chat messages tell you
   who owns what now.
6. **Cite before claiming.** File/line, command output, commit, or chat row id beats
   inference.

## Coordination Context

When coordination matters, read `helm work list`, `helm chat claims`, and the
relevant room (`helm chat read --room <room>`). Do not inspect a client view as
source of truth.
